...
namespace GI_PSMissileHit {
    ...
    void LoadMissileHitAnimationPaths() {
        ...
        std::int32_t Index{};
        pas::WideString Text{};
        Block = GR_Main::GameDataConfig->GetBlockByPath(u"SE.Weapon.MissileHit.Palettes"_wref.get());
        ...
        MissileHitAnimationPaths.set_length(Count);
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, Count - 1); cpp_range_2.next(Index); ) {
            ...
            if (Block->CountBlocks(Text) != 0) {
                ...
                if (PaletteBlock->CountParams(u"GAI"_wref.get()) > 0) {
                    MissileHitAnimationPaths[Index][0] = PaletteBlock->GetParam(u"GAI"_wref.get());
                }
            }
        }
    }
