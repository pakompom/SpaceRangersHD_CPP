...
namespace CrcUnit {
    ...
    std::uint32_t ExtendCrc32(std::uint32_t Crc, void* BufferPtr, std::int32_t ByteCount) {
        Crc = CrcUnit::InvertCrc32(Crc);
        Crc = CrcUnit::UpdateCrc32Bytes(Crc, BufferPtr, ByteCount);
    ...
    void WriteCrc32Correction(std::uint32_t CurrentCrc, std::uint32_t TargetCrc, void* Dest) {
        ...
        *Cursor = static_cast<std::uint8_t>(B3 ^ State);
        State = State >> 8 ^ Crc32Table[B3];
        ++Cursor;
        *Cursor = static_cast<std::uint8_t>(B2 ^ State);
        State = State >> 8 ^ Crc32Table[B2];
        ++Cursor;
        *Cursor = static_cast<std::uint8_t>(B1 ^ State);
