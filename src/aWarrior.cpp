...
namespace aWarrior {
    ...
    void TWarrior::InitGenerated(aPlanet::TPlanet* Planet, std::int32_t InitialMoney, TWarriorType Kind) {
        ...
        if (GR_Main::ModShipNameConfig != nullptr && GR_Main::ModShipNameConfig->CountBlocks(u"Warrior"_wref.get()) > 0) {
            Config = GR_Main::ModShipNameConfig->GetBlock(u"Warrior"sv);
            if (aConst::RaceToOwner(PilotRace) == OwnerId) {
                Config = Config->GetBlock(pas::view(aConst::OwnerToSys(OwnerId)));
            } else {
                if (Config->CountBlocks(aConst::OwnerToSys(OwnerId)) > 0) {
                    Config = Config->GetBlock(pas::view(aConst::OwnerToSys(OwnerId)));
                }
                if (Config->CountBlocks(aConst::OwnerToSys(aConst::RaceToOwner(PilotRace))) > 0) {
                    Config = Config->GetBlock(pas::view(aConst::OwnerToSys(aConst::RaceToOwner(PilotRace))));
                }
            }
            LastNameIndex = 0;
        ...
        if (GetName().length() == 0) {
            Config = GR_Main::LanguageDataConfig->GetBlock(u"ShipName"sv)->GetBlock(u"Warrior"sv);
            if (aConst::RaceToOwner(PilotRace) == OwnerId) {
                Config = Config->GetBlock(pas::view(aConst::OwnerToSys(OwnerId)));
            } else {
                if (Config->CountBlocks(aConst::OwnerToSys(OwnerId)) > 0) {
        ...
        if (aPlayer::GetPlayer() != nullptr) {
            ...
            if (!aGalaxy::Galaxy->IsZeroStartingExperienceEnabled()) {
                GainExperience(System::Round(aMyFunction::RemapClamped(Rank, 0.0, 1.0E+3, aConst::TotalSkillTrainingCost / 6, aConst::TotalSkillTrainingCost / 2)), aGalaxyStruct::esUnscaled);
                {
        ...
        if (WarriorType == wtFlagship) {
            ...
            if (GetSlotCount(aConst::sskWeapon) > WeaponCount) {
                Weapon = CreateAndEquipWeapon(aConst::t_Flux, 2 * aConst::WeaponInfos[aConst::t_Flux].AverageSize, 1, OwnerId);
            ...
            if (GetSlotCount(aConst::sskWeapon) > WeaponCount) {
                CreateAndEquipWeapon(aConst::t_Flux, aConst::WeaponInfos[aConst::t_Flux].AverageSize, 1, OwnerId);
            }
            if (GetSlotCountForItemType(aConst::t_Radar) > 0) {
    ...
    void TWarrior::SaveToBuffer(EC_Buf::TBufEC* Buffer) {
        ...
        Buffer->AddAnsiChar(static_cast<std::uint8_t>(WarriorType));
    }
    ...
    void TWarrior::LoadFromBuffer(EC_Buf::TBufEC* Buffer, aGalaxy::TGalaxy* Galaxy) {
        ...
        if (GlobalsV::LoadedSaveVersion >= 130) {
            WarriorType = static_cast<TWarriorType>(EC_Buf::TBufEC_GetByte(Buffer));
        } else {
    ...
    void TWarrior_NextDay(TWarrior* Self) {
        ...
        try {
            ...
            if (Self->WarriorType == wtFlagship) {
                Self->SetMoney(Self->Money + std::max<std::int32_t>(1000, std::min<std::int32_t>(5000, aGalaxy::Galaxy->MaxRangerWealth / 15)));
            }
            ...
            if (pas::Exception* E = pas::class_cast_if<pas::Exception*>(cpp_exception)) {
                ...
                pas::raise(pas::make_exception<pas::Exception>(static_cast<pas::AnsiString>(pas::concat_wide({u"Error in procedure TWarrior.NextDay ", Self->GetFullName(u" "_wref.get())}))));
            } else {
                throw;
            }
        }
    ...
    void TWarrior_NextDayLogic(TWarrior* Self) {
        static const pas::Set<0, 255> FriendlyStationMask = pas::constant_set<pas::Set<0, 255>>({{aGalaxyStruct::ssCoalitionMilitary}, {aGalaxyStruct::ssCoalitionActive}});
        aPlanet::TPlanet* Planet{};
        aShip::TShip* Station{};
        std::int32_t Stage = 0;
        if (Self->WarriorType == wtFlagship) {
            aWarrior::TWarrior_NextDayFlagshipLogic(Self);
        } else {
            try {
                if (Self->CurrentPlanet != nullptr) {
                    Stage = 1;
                    if (pas::contains(aConst::PlanetOwnerMasks.Coalition, Self->CurrentPlanet->OwnerId)) {
                        Stage = 2;
                        ...
                        if (Self->LiberationGroup != nullptr) {
                            Self->ProcessLiberationGroupRoute();
                        }
                        if (Self->CurrentPlanet != Self->HomePlanet || Self->LiberationGroup != nullptr || Self->IsHomePatrolTurn()) {
                            Self->OrderTakeoff();
                        }
                    } else {
                        Self->OrderTakeoff();
                    ...
                    if (Self->DockedTo->TypeId == aGalaxyStruct::rstMilitaryBase && reinterpret_cast<aRuins::TRuins*>(Self->DockedTo)->FlyToStar != nullptr && reinterpret_cast<aRuins::TRuins*>(Self->DockedTo)->FlyToStar != Self->DockedTo->CurrentStar) {
                        return;
                    }
                    if (Self->DockedTo->InNormalSpace()) {
                        Self->OrderTakeoff();
                    ...
                    if (Self->InFear) {
                        ...
                        if (Self->Order != aShip::soLand) {
                            ...
                            if (Planet != nullptr && Planet->CurrentStar == Self->CurrentStar) {
                                ...
                                Stage = 13;
                                Station = Self->FindNearestDockableStation(static_cast<aGalaxyStruct::TShipStandings>(FriendlyStationMask));
                                if (Station != nullptr) {
                                    Stage = 14;
                        ...
                        if (Self->LiberationGroup == nullptr) {
                            ...
                            Stage = 19;
                            if (Self->Order == aShip::soNone) {
                                if (Self->IsHomePatrolTurn()) {
                                    Self->MoveToRandomPlanetOrbit();
                                } else if (!Self->NavigateToHomePlanet()) {
                                    Self->MoveToRandomPatrolPoint();
                                }
                            }
                            Stage = 20;
                    ...
                    if (Self->LiberationGroup != nullptr) {
                        ...
                        if (Self->LiberationGroup == nullptr) {
                            ...
                            Self->SelectEnemyShipInStar();
                            Self->EngageEnemyShip();
                            if (Self->Order == aShip::soNone && static_cast<std::uint8_t>(Self->NavigateToHomePlanet() ^ 1)) {
                                Self->MoveToRandomPatrolPoint();
                            }
                        }
                    }
                ...
                if (pas::Exception* E = pas::class_cast_if<pas::Exception*>(cpp_exception)) {
                    GR_Main::AppendLogLineThreadSafe(pas::concat_ansi({static_cast<pas::AnsiString>(pas::class_name(pas::class_type(E))), " ", E->message}));
                    pas::raise(pas::make_exception<pas::Exception>(static_cast<pas::AnsiString>(pas::concat_wide({u"Error in procedure TWarrior.NextDayLogic ", Self->GetFullName(u" "_wref.get()), u" label = ", pas::wide_int_to_str(Stage)}))));
                } else {
                    throw;
                }
            }
    ...
    void TWarrior_NextDayFlagshipLogic(TWarrior* Self) {
        static const pas::Set<0, 255> FriendlyStationMask = pas::constant_set<pas::Set<0, 255>>({{aGalaxyStruct::ssCoalitionMilitary}, {aGalaxyStruct::ssCoalitionActive}});
        static const pas::Set<0, 255> AnyStationMask = pas::constant_set<pas::Set<0, 255>>({});
        aPlanet::TPlanet* Planet{};
        aShip::TShip* Station{};
        aShip::TShip* Ship{};
        pas::List* Stations{};
        ...
        auto RepairHullWithNodes = [&]() -> void {
            ...
            if (pas::is_one_of<aGalaxyStruct::oiMaloc, aGalaxyStruct::oiPeleng>(Self->PilotRace) && Self->InFear) {
                Available = Self->GetCarriedNodeCount();
                if (Available > 0) {
                    {
                        pas::Extended cpp_right = aMyFunction::RemapClamped(aGalaxy::Galaxy->TechLevel, 2.0, 8.0, 5.0, 1.0);
                        Needed = MathImports::Ceil(pas::real_divide(pas::real_divide((Self->GetHull()->Weight - Self->GetHull()->HullPoints) * (Self->GetCombatStatusStrength(aShip::cseBWRepairDebuff) * 0.002L + 1.0L), 1.0E+1L), 2.0L) * cpp_right);
                    }
                    if (Needed > 0) {
                        Available = std::min<std::int32_t>(Needed, Available);
                        Self->ConsumeNodes(Available);
                        Fraction = pas::real_divide(Available, Needed);
                        Restored = MathImports::Ceil(aMyFunction::RemapClamped(Fraction, 0.0, 1.0, 0.01, Self->GetHull()->Weight - Self->GetHull()->HullPoints));
                        Self->GetHull()->HullPoints += Restored;
                        Self->AddCombatStatusStrength(aShip::cseBWRepairDebuff, pas::real_divide(Restored, 2.0L), nullptr);
                        Self->RefreshDerivedStats(true);
                        if (aPlayer::GetPlayer()->CurrentStar == Self->CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
                            ...
                            Effect->SetEndpoints(nullptr, Self->Graphic);
                            Effect->SetHit(aConst::OwnerToFilmColor(Self->OwnerId), -Restored, false, true);
        ...
        auto RepairEquipmentWithNodes = [&]() -> void {
            ...
            float Fraction{};
            if (pas::is_one_of<aGalaxyStruct::oiFeyan, aGalaxyStruct::oiGaal>(Self->PilotRace)) {
                Available = Self->GetCarriedNodeCount();
                if (Available > 0) {
                    NeedsRepair = false;
                    TotalCost = 0;
                    Threshold = System::Round(aMyFunction::RemapClamped(Self->CargoFreeSpace, 0.0, Self->GetHull()->Weight / 4, 9.0E+1, 5.0E+1));
                    for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Self->Inventory) - 1); cpp_range.next(I); ) {
                        ...
                        if (aItem::TWeapon* weapon = pas::class_cast_if<aItem::TWeapon*>(Equipment); (!(weapon != nullptr) || weapon->GetWeaponInfo()->Availability != aGalaxyStruct::waNotSoldAndNodeRepair) && aShip::TShip_CanRepairEquipmentTech(Self, Equipment) && Equipment->ItemType != aConst::t_Hull && Equipment->EquippedFlag != 0) {
                            if (static_cast<long double>(Threshold) >= Equipment->ConditionPercent) {
                                NeedsRepair = true;
                            }
                            TotalCost += System::Round(aItem::TEquipment_CalculateRepairCost(Equipment));
                        }
                    }
                    if (NeedsRepair) {
                        {
                        ...
                        Self->ConsumeNodes(Available);
                        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Self->Inventory) - 1); cpp_range_2.next(I); ) {
                            Equipment = pas::list_at<aItem::TEquipment>(Self->Inventory, I);
                            if (aItem::TWeapon* weapon_2 = pas::class_cast_if<aItem::TWeapon*>(Equipment); (!(weapon_2 != nullptr) || weapon_2->GetWeaponInfo()->Availability != aGalaxyStruct::waNotSoldAndNodeRepair) && aShip::TShip_CanRepairEquipmentTech(Self, Equipment) && Equipment->ItemType != aConst::t_Hull && Equipment->EquippedFlag != 0) {
                        ...
                        if (Self->Order == aShip::soNone) {
                            if (aMyFunction::NextRandomIntRange(1, 100, Self->RandomState) > 80) {
                                ...
                                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Self->CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                                    ...
                                    if (pas::in_range(Ship->TypeId, static_cast<std::int32_t>(aGalaxyStruct::rstRangerCenter), static_cast<std::int32_t>(aGalaxyStruct::rstCustomStation)) && Ship->virtual_TShip_CanDock(Self) && pas::in_range(Ship->CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssCoalitionMilitary), static_cast<std::int32_t>(aGalaxyStruct::ssNeutral))) {
                                        if (static_cast<long double>(aMyFunction::PointDistanceSquared(Self->Position, Ship->Position)) >= pas::sqr(Self->CalculateSpeed())) {
                                            pas::list_add(Stations, reinterpret_cast<void*>(Ship));
                                        }
                                    }
                                }
                                if (pas::list_count(Stations) > 0) {
                                    Station = pas::list_at<aShip::TShip>(Stations, aMyFunction::NextRandomIntRange(0, pas::list_count(Stations) - 1, Self->RandomState));
                                }
                                pas::free(Stations);
                                if (Station != nullptr) {
                                    Self->OrderLanding(Station, false);
                                }
                            }
                            if (Self->Order == aShip::soNone) {
                    ...
                    if (Self->LiberationGroup == nullptr) {
                        ...
                        if (Self->Order == aShip::soNone) {
                            if (aMyFunction::NextRandomIntRange(1, 100, Self->RandomState) > 40) {
                                Station = Self->FindNearestDockableStation(static_cast<aGalaxyStruct::TShipStandings>(AnyStationMask));
                                if (Station != nullptr) {
                                    Self->OrderLanding(Station, false);
                                }
                            }
    ...
    std::uint8_t TWarrior_CanQueueReachablePlanet(TWarrior* Self, aPlanet::TPlanet* Planet) {
        return Planet->OwnerId != aGalaxyStruct::oiDominator && Planet->OwnerId != aGalaxyStruct::oiPirate;
    }
    ...
    aGalaxyStruct::TGreetingShipCategory TWarrior::GetGreetingShipCategory() {
    ...
    aGalaxyStruct::TPercent TWarrior::GetStrengthScaledPirateStatus() {
        return 0;
    }
    ...
    std::uint8_t TWarrior::RelationToNonRanger(aShip::TShip* Ship) {
        if (Ship->TypeId == aGalaxyStruct::stPirate) {
            return System::Round(pas::real_max<pas::Extended>(1.0E+1L, pas::real_min<pas::Extended>(2.0E+1L, aConst::OwnerRelations[aConst::RaceToOwner(PilotRace)][Ship->OwnerId] * (0.5L * aConst::PlanetRaceMarket[PilotRace].PirateRelationFactor))));
    ...
    void TWarrior::ReactToAttack(aShip::TShip* Attacker) {
        EnemyShip = Attacker;
        if (pas::contains(aConst::PlanetOwnerMasks.Coalition, HomePlanet->OwnerId) && Attacker->TypeId == aGalaxyStruct::stRanger) {
            ...
            if (aTranclucator::TTranclucator* tranclucator = pas::class_cast_if<aTranclucator::TTranclucator*>(Attacker); tranclucator != nullptr && tranclucator->OwnerShip != nullptr && tranclucator->OwnerShip->TypeId == aGalaxyStruct::stRanger) {
                HomePlanet->ChangeRelationToRanger(tranclucator->OwnerShip, -3);
            }
        }
    }
    ...
    std::uint8_t TWarrior_RecomputeFearState(TWarrior* Self) {
        ...
        if (Self->HasNoUsableWeapons() && Self->EnemyShip != nullptr && Self->EnemyShip->CurrentStar == Self->CurrentStar) {
            ...
            Result = Self->GetHull()->HullPoints < Self->GetHull()->Weight * 0.3L && Self->GetHull()->HullPoints < 200 || Self->GetHull()->HullPoints < 130 || Self->GetHull()->Weight * 0.5L * aConst::OwnerInfo[Self->OwnerId].FearThresholdScale > Self->GetHull()->HullPoints && Self->EnemyShip != nullptr && Self->EnemyShip->OrderTarget == Self && ([&] {
    ...
    std::uint8_t TWarrior_AcceptsRansomDemandFrom(TWarrior* Self, aShip::TShip* Ship) {
        return Self->GetHull()->Weight * 0.5L * aConst::OwnerInfo[Self->OwnerId].FearThresholdScale > Self->GetHull()->HullPoints && ([&] {
    ...
    std::uint8_t TWarrior::AcceptsAppealFrom(aShip::TShip* Ship) {
        pas::Extended cpp_right = aMyFunction::RemapClamped(Ship->Strength, 0.9L * Strength, Strength * 3.0L, 0.0, 1.0E+2);
        return aShip::TShip_RelationToShip(this, Ship) + cpp_right > 1.6E+2L;
    }
            ...
            try {
                ...
                for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range_2.next(I); ) {
                    ...
                    if (Ship->InNormalSpace() && Ship != this && EnemyShip != Ship) {
                        Stage = 32;
                        if (aShip::TShip_RelationToShip(this, Ship) < aGalaxyStruct::RelationBadMin || EnemyShip == Ship || Ship->EnemyShip == this) {
                            Stage = 33;
                            if (TruceShip != Ship) {
                                Stage = 34;
                ...
                if (aGalaxy::Galaxy->GetAIJunkToleranceLevel() * 1.5L < pas::list_count(CurrentStar->Items)) {
                    for (auto cpp_range_5 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range_5.next(I); ) {
                        ...
                        if ((Item->ItemType == aConst::t_Minerals || Item->OwnerId == aGalaxyStruct::oiDominator) && (Item->ScriptItem == nullptr || reinterpret_cast<aScript::TScriptItem*>(Item->ScriptItem)->Name == u"")) {
    ...
    void TWarrior_AssignFlagshipWeaponTargets(TWarrior* Self) {
            ...
            float Score = 0.0f;
            {
                ...
                if (1 <= cpp_last) {
                    for (J = 1; J <= cpp_last; ++J) {
                        ...
                        if (aShip::TShip_IsEquipmentUsable(Self, Weapon) && (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) || Weapon->Ammo > 0)) {
                            Range = aShip::TShip_GetWeaponRange(Self, Weapon);
        ...
        try {
            ...
            if (Self->EnemyShip != nullptr && Self->EnemyShip->CurrentStar == Self->CurrentStar && Self->EnemyShip->InNormalSpace()) {
                ...
                if (!IgnoreRanger) {
                    ScoreTarget(Self->EnemyShip);
                }
            }
            Stage = 2;
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Self->CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                Ship = pas::list_at<aShip::TShip>(Self->CurrentStar->Ships, I);
                if (Ship->InNormalSpace() && Self->EnemyShip != Ship && Self->TruceShip != Ship && Self->LiberationGroup == nullptr && aShip::TShip_GetRelationLevelToShip(Self, Ship) <= aGalaxyStruct::rlHostile) {
                    ScoreTarget(Ship);
                }
            }
                    ...
                    if (1 <= cpp_last_2) {
                        for (J = 1; J <= cpp_last_2; ++J) {
                            ...
                            if (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) && Weapon->Target == nullptr && aShip::TShip_IsEquipmentUsable(Self, Weapon)) {
                                if (static_cast<long double>(aMyFunction::PointDistanceSquared(Self->Position, Missile->Position)) <= pas::sqr(aShip::TShip_GetWeaponRange(Self, Weapon))) {
                                    Weapon->Target = Missile;
                                    ++Assigned;
                                    if (Assigned == Self->WeaponCount) {
                                        return;
            ...
            if (aGalaxy::Galaxy->GetAIJunkToleranceLevel() * 1.5L < pas::list_count(Self->CurrentStar->Items)) {
                for (auto cpp_range_4 = pas::for_to<std::int32_t>(0, pas::list_count(Self->CurrentStar->Items) - 1); cpp_range_4.next(I); ) {
                    ...
                    if ((Item->ItemType == aConst::t_Minerals || Item->OwnerId == aGalaxyStruct::oiDominator) && (Item->ScriptItem == nullptr || reinterpret_cast<aScript::TScriptItem*>(Item->ScriptItem)->Name == u"") && static_cast<std::uint8_t>(Self->AcceptPickupItem(Item) ^ 1)) {
                            ...
                            if (Self->CanSafelyDetonateItem(Item)) {
                                ...
                                if (1 <= cpp_last_4) {
                                    for (J = 1; J <= cpp_last_4; ++J) {
                                        Weapon = Self->Weapons[J];
                                        if (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) && Weapon->Target == nullptr && aShip::TShip_IsEquipmentUsable(Self, Weapon)) {
                                            if (static_cast<long double>(aMyFunction::PointDistanceSquared(Self->Position, Item->Position)) <= pas::sqr(aShip::TShip_GetWeaponRange(Self, Weapon))) {
                                                Weapon->Target = Item;
                                                ++Assigned;
                                                if (Assigned == Self->WeaponCount) {
                                                    return;
                ...
                if (1 <= cpp_last_5) {
                    for (J = 1; J <= cpp_last_5; ++J) {
                        ...
                        if ((static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) || Weapon->Ammo != 0) && Weapon->Target == nullptr && aShip::TShip_IsEquipmentUsable(Self, Weapon)) {
                            if (static_cast<long double>(aMyFunction::PointDistanceSquared(Self->Position, aPlayer::GetPlayer()->Position)) <= pas::sqr(aShip::TShip_GetWeaponRange(Self, Weapon))) {
                                Weapon->Target = aPlayer::GetPlayer();
                                ++Assigned;
                                if (Assigned == Self->WeaponCount) {
                                    return;
        ...
        if (InFlagshipRange) {
            ...
            MaxRange = -1;
            {
                ...
                if (1 <= cpp_last) {
                    for (I = 1; I <= cpp_last; ++I) {
                        Weapon = Weapons[I];
                        if (aShip::TShip_IsEquipmentUsable(this, Weapon) && (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) || Weapon->Ammo > 0)) {
                            Range = aShip::TShip_GetWeaponRange(this, Weapon);
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                Ship = pas::list_at<aShip::TShip>(CurrentStar->Ships, I);
                if (Ship->InNormalSpace() && aShip::TShip_RelationToShip(this, Ship) < aGalaxyStruct::RelationBadMin && TruceShip != Ship) {
                    {
                    ...
                    if (aKling::TKling* kling = pas::class_cast_if<aKling::TKling*>(Ship)) {
                        ...
                        if (kling->KlingType == aGalaxyStruct::ktKlig) {
                            Score = Score * 0.25L;
                        }
                    }
                    if (Score > BestScore) {
                ...
                for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range_2.next(I); ) {
                    ...
                    if (Ship->InNormalSpace() && aShip::TShip_RelationToShip(this, Ship) < aGalaxyStruct::RelationBadMin && TruceShip != Ship) {
                        if (pas::is_one_of<aGalaxyStruct::ssDominator, aGalaxyStruct::ssPirateMilitary>(Ship->CurrentStanding)) {
                            EnemyShip = Ship;
                            if (aShip::TShip_ChanceToWin(this, Ship) > 1.0L) {
                                return;
                            }
        ...
        if (EnemyShip != nullptr && EnemyShip->CurrentStar == CurrentStar) {
            ...
            if (EnemyShip->InNormalSpace()) {
                if (RetreatToFlagship) {
                    OrderFollowShip(Ship, aShip::fmFollowNear, false);
                } else {
            ...
            } else if (aShip::TShip_ChanceToWin(this, EnemyShip) > 0.5L) {
    ...
    void TWarrior::ManeuverFlagship() {
        ...
        auto EvaluatePosition = [&](EC_Struct::TPointF Point) -> float {
            ...
            if (GetCargoHook() != nullptr) {
                for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(this->CurrentStar->Items) - 1); cpp_range_3.next(I); ) {
                    ...
                    if (aItem::TProtoplasm* protoplasm = pas::class_cast_if<aItem::TProtoplasm*>(Item); protoplasm != nullptr && aItem::CanCargoHookHandleItem(Item, this) && static_cast<std::uint8_t>(aShip::TShip_IsItemInPickupRange(this, Item) ^ 1)) {
            ...
            if (this->PilotRace != aGalaxyStruct::oiFeyan) {
        ...
        auto BoostWithNodes = [&]() -> void {
            float Factor{};
            if (pas::is_one_of<aGalaxyStruct::oiMaloc, aGalaxyStruct::oiHuman, aGalaxyStruct::oiGaal>(this->PilotRace) && EnemiesInRange > 0) {
                {
        ...
        if (BestShip != nullptr && BestShipScore >= BestPositionScore) {
            if (aShip::TShip_GetRelationLevelToShip(this, BestShip) > aGalaxyStruct::rlHostile) {
                OrderFollowShip(BestShip, aShip::fmFollowNear, false);
            } else {
                OrderFollowShip(BestShip, aShip::fmMinWeaponRange, false);
            }
        } else {
            OrderMove(Destination, false);
        }
        ...
        Maximum = -1;
        {
            ...
            if (1 <= cpp_last) {
                for (I = 1; I <= cpp_last; ++I) {
                    ...
                    if (aShip::TShip_IsEquipmentUsable(Self, Weapon) && (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) || Weapon->Ammo > 0)) {
                        Range = aShip::TShip_GetWeaponRange(Ship, Weapon);
    ...
    void TWarrior::ReassignFlagshipHomePlanet() {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range.next(I); ) {
            ...
            if (Star->Status.ControlFaction == aGalaxyStruct::sfCoalition && Star->Status.CustomFaction == u"" && Star->ConstellationGraphIndex != 20) {
                ...
                for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Ships) - 1); cpp_range_2.next(J); ) {
                    ...
                    if (pas::in_range(Ship->TypeId, static_cast<std::int32_t>(aGalaxyStruct::rstRangerCenter), static_cast<std::int32_t>(aGalaxyStruct::rstCustomStation)) && pas::contains(aConst::NonTargetableStationStandingMasks[aGalaxyStruct::sfCoalition], Ship->CurrentStanding)) {
                        ++StationCount;
                    }
                }
                for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Planets) - 1); cpp_range_3.next(J); ) {
                    ...
                    if (pas::contains(aConst::PlanetOwnerMasks.Coalition, Planet->OwnerId)) {
                        ...
                        for (auto cpp_range_4 = pas::for_to<std::int32_t>(0, pas::list_count(Planet->Warriors) - 1); cpp_range_4.next(K); ) {
                            ...
                            if (Warrior->WarriorType == wtFlagship) {
                                if (Warrior->PilotRace == PilotRace) {
        ...
        if (BestStar != nullptr) {
            ...
            for (auto cpp_range_6 = pas::for_to<std::int32_t>(0, pas::list_count(BestStar->Planets) - 1); cpp_range_6.next(I); ) {
                ...
                if (pas::contains(aConst::PlanetOwnerMasks.Coalition, Planet->OwnerId) && Planet->RaceId == PilotRace) {
                    WarriorCount = 0;
                    FlagshipFactor = 1.0f;
            ...
            if (BestPlanet != nullptr && BestPlanet != HomePlanet) {
    ...
    std::uint8_t TWarrior_BuildMoneyExtortionResponse(TWarrior* Self, aShip::TShip* OtherShip, pas::WideString& Response, std::int32_t DemandedAmount) {
        ...
        if (aPlayer::GetPlayer() != OtherShip && (Self->EnemyShip == nullptr || Self->EnemyShip->CurrentStar != Self->CurrentStar)) {
    ...
    std::uint8_t TWarrior::BuildTrucePaymentResponse(aShip::TShip* OtherShip, pas::WideString& Response, std::int32_t OfferedAmount) {
        ...
        auto AcceptPayment = [&]() -> void {
            ...
            if (aNormalShip::TNormalShip* normalShip = pas::class_cast_if<aNormalShip::TNormalShip*>(OtherShip)) {
                normalShip->CurrentSystemKills.Normal = 0;
            }
            if (aRanger::TRanger* ranger = pas::class_cast_if<aRanger::TRanger*>(OtherShip)) {
                ...
                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(this->CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                    ...
                    if (TWarrior* warrior = pas::class_cast_if<TWarrior*>(Ship); warrior != nullptr && warrior->HomePlanet == Planet && this != Ship) {
        ...
        if (RefusesFactionNegotiation(OtherShip)) {
            Response = aShip::TShip_LookupVisibleTalkText(this, u"Talk.Refuse.Warrior"_wref.get(), OtherShip);
            return false;
            ...
            Response = aShip::TShip_LookupVisibleTalkText(this, u"Talk.Truce.WeAlreadyHavePact"_wref.get(), OtherShip);
            return Result;
        } else if (aNormalShip::TNormalShip* normalShip = pas::class_cast_if<aNormalShip::TNormalShip*>(OtherShip); normalShip != nullptr && OtherShip->OwnerId == aGalaxyStruct::oiPirate && OtherShip->CurrentStar->Status.ControlFaction == aGalaxyStruct::sfCoalition && normalShip->CurrentSystemKills.Normal > 0) {
    ...
    std::uint8_t TWarrior_BuildAttackRequestResponse(TWarrior* Self, aShip::TShip* Requester, pas::WideString& Response, aShip::TShip* Target) {
        ...
        if (Self->OrderTarget == Target && aShip::TShip_GetRelationLevelToShip(Self, Target) == aGalaxyStruct::rlHostile && HostileCount <= 1) {
            AcceptRequest();
        } else if (Self->LiberationGroup != nullptr && Requester->LiberationGroup != Self->LiberationGroup) {
        ...
        } else if (Self->virtual_TShip_AcceptsRansomDemandFrom(Target) || Self->virtual_TShip_RecomputeFearState()) {
        ...
        } else if (Self->HasLockedOrFollowOrder() && pas::class_cast_if<aNormalShip::TNormalShip*>(Requester) != nullptr && static_cast<aNormalShip::TNormalShip*>(Requester)->Rank < Self->Rank) {
            Response = aShip::TShip_LookupVisibleTalkText(Self, pas::concat_wide({u"Talk.Attack.", Self->GetTypeNameKey(), u"HaveBusiness"}), Requester);
    ...
    std::uint8_t TWarrior::RefusesFactionNegotiation(aShip::TShip* OtherShip) {
        std::uint8_t Result = false;
        if (OtherShip->CurrentStanding == aGalaxyStruct::ssPirateMilitary) {
            return true;
        }
        if ((CurrentStar->Status.ControlFaction != aGalaxyStruct::sfCoalition || CurrentStar->Status.CustomFaction != u"") && pas::is_one_of<aGalaxyStruct::ssPirateActive, aGalaxyStruct::ssPirateMilitary>(OtherShip->CurrentStanding)) {
            return true;
        }
        return Result;
    }
    ...
    float TWarrior::EvaluateStatBonus(aConst::TEquipmentBonusKind BonusKind, std::int32_t Value) {
        ...
        }
        {
            ...
            if (cpp_case == aConst::bonHull) {
                ...
                Result = Value * 3 + Value * 30 * CountWeaponsByDamageFlags(static_cast<aGalaxyStruct::TDamageFlagSet>(ScannerFlags));
                ...
                Result = Value * 12 * (0.1L + static_cast<std::int8_t>(GetRadarRange() > 0) * 0.9L) * (1 - (WarriorType == wtFlagship));
                ...
                if (GetSlotCount(aConst::sskRadar) == 0 && Value > 0) {
                    ...
                    Result = -WarriorSlotBonusWeights[BonusKind] - WarriorSlotBonusWeights[aConst::bonSlotWeapon] * CountMissileWeapons();
                ...
                if (GetSlotCount(aConst::sskScanner) == 0 && Value > 0) {
                    ...
                    Result = -WarriorSlotBonusWeights[BonusKind] - CountWeaponsByDamageFlags(static_cast<aGalaxyStruct::TDamageFlagSet>(ScannerFlags)) * 0.1L * WarriorSlotBonusWeights[aConst::bonSlotWeapon];
                ...
                if (Value > 0) {
                    Result = std::min<std::int32_t>(6 - GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false), Value) * WarriorSkillBonusWeights[BonusKind];
                }
                if (Value > 0 && Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) > 6) {
                    Result = Result + WarriorSkillBonusWeights[BonusKind] * 0.05L * (Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) - 6);
                }
                if (Value < 0) {
                    Result = std::min<std::int32_t>(static_cast<std::int32_t>(GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false)), -Value) * -WarriorSkillBonusWeights[BonusKind];
                }
                if (Value < 0 && Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) < 0) {
                    Result = Result + WarriorSkillBonusWeights[BonusKind] * 0.03L * (Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false));
                }
            } else {
                Result = 0.0f;
            }
    ...
    std::uint8_t TWarrior::AcceptPickupItem(aItem::TItem* Item) {
        ...
        if (WarriorType == wtFlagship) {
            ...
            if (pas::in_range(PilotRace, static_cast<std::int32_t>(aGalaxyStruct::oiPeleng), static_cast<std::int32_t>(aGalaxyStruct::oiFeyan)) && pas::class_cast_if<aItem::TUselessItem*>(Item) != nullptr) {
                if (static_cast<aItem::TUselessItem*>(Item)->IsDominatorRemains()) {
                    return true;
                }
            }
        }
    ...
    void TWarrior::virtual_TShip_NextDay() {
        aWarrior::TWarrior_NextDay(this);
    }

    void TWarrior::virtual_TShip_NextDayLogic() {
        aWarrior::TWarrior_NextDayLogic(this);
    }

    std::uint8_t TWarrior::virtual_TShip_CanQueueReachablePlanet(aPlanet::TPlanet* Planet) {
    ...
    std::uint8_t TWarrior::virtual_TShip_AcceptsRansomDemandFrom(aShip::TShip* Ship) {
        return aWarrior::TWarrior_AcceptsRansomDemandFrom(this, Ship);
    }
    ...
    std::uint8_t TWarrior::virtual_TShip_BuildMoneyExtortionResponse(aShip::TShip* OtherShip, pas::WideString& Response, std::int32_t DemandedAmount) {
        return aWarrior::TWarrior_BuildMoneyExtortionResponse(this, OtherShip, Response, DemandedAmount);
    }

    std::uint8_t TWarrior::virtual_TShip_BuildCargoExtortionResponse(aShip::TShip* OtherShip, pas::WideString& Response) {
        return aWarrior::TWarrior_BuildCargoExtortionResponse(this, OtherShip, Response);
    }

    std::uint8_t TWarrior::virtual_TShip_BuildAttackRequestResponse(aShip::TShip* Requester, pas::WideString& Response, aShip::TShip* Target) {
        return aWarrior::TWarrior_BuildAttackRequestResponse(this, Requester, Response, Target);
    }

    std::uint8_t TWarrior::virtual_TShip_AcceptPartnershipOffer(aShip::TShip* OtherShip, pas::WideString& Response, std::int32_t PaymentAmount) {
        return aWarrior::TWarrior_AcceptPartnershipOffer(this, OtherShip, Response, PaymentAmount);
    }
    ...
    void TWarrior::virtual_TShip_RefreshCurrentStanding() {
        aWarrior::TWarrior_RefreshCurrentStanding(this);
    }
