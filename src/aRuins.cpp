...
namespace aRuins {
    ...
    const pas::Array<pas::Array<aGalaxyStruct::TOwnerId, 0, 1>, 6, 12> StationPilotRaces = pas::Array<pas::Array<aGalaxyStruct::TOwnerId, 0, 1>, 6, 12>{{pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiFeyan, aGalaxyStruct::oiGaal}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiHuman, aGalaxyStruct::oiPeleng}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiMaloc, aGalaxyStruct::oiPeleng}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiFeyan, aGalaxyStruct::oiGaal}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiHuman, aGalaxyStruct::oiHuman}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiGaal, aGalaxyStruct::oiGaal}}, pas::Array<aGalaxyStruct::TOwnerId, 0, 1>{{aGalaxyStruct::oiFeyan, aGalaxyStruct::oiFeyan}}}};
    ...
    const pas::Array<pas::Array<aConst::TItemType, 0, 2>, 6, 12> StationWeaponTypes = pas::Array<pas::Array<aConst::TItemType, 0, 2>, 6, 12>{{pas::Array<aConst::TItemType, 0, 2>{{aConst::t_Flux, aConst::t_FlowBlaster, aConst::t_AtomicVision}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_Flux, aConst::t_WavePhaser, aConst::t_Disintegrator}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_Flux, aConst::t_Multiresonator, aConst::t_FlowBlaster}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_Flux, aConst::t_Treton, aConst::t_Turbogravitron}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_FragmentationCannon, aConst::t_ElectronicCutter, aConst::t_WavePhaser}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_IndustrialLaser, aConst::t_Treton, aConst::t_WavePhaser}}, pas::Array<aConst::TItemType, 0, 2>{{aConst::t_Flux, aConst::t_WavePhaser, aConst::t_Disintegrator}}}};
    ...
    void TRuins::Init(aGalaxyStruct::TStationType StationType, aGalaxy::TStar* Star, pas::WideString TypeNameOverride) {
        std::int32_t I{};
        aRanger::TRanger* Ranger{};
        aGalaxyEvent::TGalaxyEvent* Event{};
        std::uint8_t Good{};
        ...
        aGalaxyStruct::TOwnerId EquipmentOwner{};
        auto SelectStationName = [&](EC_BlockPar::TBlockParEC* Config) -> void {
            ...
            if (Config->CountBlocks(aConst::ShipTypeNames[this->TypeId].Name) == 0) {
                return;
            }
            std::int32_t LastName = Config->GetBlock(pas::view(aConst::ShipTypeNames[this->TypeId].Name))->GetParamCount() - 1;
            Index = aMyFunction::NextRandomIntRange(0, LastName, this->RandomState);
            for (auto cpp_range = pas::for_to<std::int32_t>(0, LastName); cpp_range.next(I); ) {
                this->Name = Config->GetBlock(pas::view(aConst::ShipTypeNames[this->TypeId].Name))->GetParamValue(Index);
                Used = false;
                for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range_2.next(J); ) {
                    ...
                    for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(OtherStar->Ships) - 1); cpp_range_3.next(K); ) {
                        ...
                        if (Ship->TypeId == this->TypeId && Ship->Name == this->Name && Ship != this) {
                            Used = true;
                            break;
                        }
                    }
                ...
                if (I == LastName) {
                    this->Name = pas::concat_wide({this->Name, u" ", u"-", pas::wide_int_to_str(aMyFunction::NextRandomIntRange(10, 99, this->RandomState)), u"-"});
                }
            }
        };
        ...
        if (StationType == aGalaxyStruct::rstCustomStation) {
            TypeId = aGalaxyStruct::rstRangerCenter;
        } else {
            TypeId = static_cast<aGalaxyStruct::TShipType>(StationType);
        }
        ...
        if (aGalaxy::Galaxy->CurrentTurn < aGalaxyStruct::GalaxyWarmupTurns && aPlayer::GetPlayer()->CurrentStar->Constellation == Star->Constellation) {
            PilotRace = StationPilotRaces[TypeId][0];
        } else if (aMyFunction::NextRandomUnitFloat(RandomState) < 0.5L) {
            PilotRace = StationPilotRaces[TypeId][0];
        } else {
        ...
        if (GetName().length() == 0) {
            SelectStationName(GR_Main::LanguageDataConfig->GetBlock(u"RuinName"sv));
        }
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Rangers) - 1); cpp_range.next(I); ) {
            Ranger = pas::list_at<aRanger::TRanger>(aGalaxy::Galaxy->Rangers, I);
        ...
        if (aPlayer::GetPlayer() != nullptr && aPlayer::GetPlayer()->RuinsProxy != this) {
        ...
        for (Good = 0; Good <= 7; ++Good) {
            ShopGoods[Good].Count = System::Round(static_cast<long double>(aConst::GoodsMarket[Good].BaseStock) * aConst::StationGoodsFactors[TypeId][Good].StockFactor);
            ShopGoods[Good].PriceState = aConst::GoodsMarket[Good].AveragePrice;
            ShopGoods[Good].PurchasePrice = System::Round(ShopGoods[Good].PriceState);
            ShopGoods[Good].BaseSalePrice = System::Round(ShopGoods[Good].PriceState * 0.98L - 1.0L);
        }
        if (CurrentStar->Status.ControlFaction == aGalaxyStruct::sfPirates) {
            EquipmentOwner = aGalaxyStruct::oiPirate;
        } else {
            EquipmentOwner = OwnerId;
        }
        {
            aGalaxyStruct::TOwnerId ownerId = OwnerId;
            std::uint8_t cpp_arg = CurrentStar->Status.ControlFaction == aGalaxyStruct::sfPirates;
            std::int32_t cpp_left_2 = aMyFunction::NextRandomIntRange(StationHullGeneration[TypeId].MinSize, StationHullGeneration[TypeId].MaxSize, RandomState);
            pas::Extended cpp_left = cpp_left_2 + aGalaxy::Galaxy->ScaleIntByTechLevel(0, StationHullGeneration[TypeId].TechSizeBonus);
            std::uint16_t roundAndTruncateToTens = aMyFunction::RoundAndTruncateToTens(cpp_left * aConst::HullCapacityScale);
            std::int32_t nextRandomIntRange = aMyFunction::NextRandomIntRange(StationHullGeneration[TypeId].MinLevel, StationHullGeneration[TypeId].MaxLevel, RandomState);
        ...
        {
            std::int32_t nextRandomIntRange_2 = aMyFunction::NextRandomIntRange(StationDefenseLevels[TypeId].Minimum, StationDefenseLevels[TypeId].Maximum, RandomState);
        ...
        {
            std::int32_t nextRandomIntRange_3 = aMyFunction::NextRandomIntRange(StationRepairLevels[TypeId].Minimum, StationRepairLevels[TypeId].Maximum, RandomState);
        ...
        if (aConst::WeaponInfos[StationWeaponTypes[TypeId][2]].TechLevel <= aGalaxy::Galaxy->TechLevel && aMyFunction::NextRandomUnitFloat(RandomState) > 0.6L) {
            Weapon = ([&] {
                std::int32_t nextRandomIntRange_4 = aMyFunction::NextRandomIntRange(StationWeaponGeneration[TypeId].AdvancedLevel + 1, 8, RandomState);
                std::int32_t advancedLevel = StationWeaponGeneration[TypeId].AdvancedLevel;
                ...
                std::int32_t randomStationEquipmentSize_5 = RandomStationEquipmentSize(aConst::WeaponInfos[StationWeaponTypes[TypeId][2]].AverageSize);
                aConst::TItemType cpp_arg_2 = StationWeaponTypes[TypeId][2];
        ...
        } else if (aConst::WeaponInfos[StationWeaponTypes[TypeId][1]].TechLevel <= aGalaxy::Galaxy->TechLevel && aMyFunction::NextRandomUnitFloat(RandomState) > 0.6L) {
            Weapon = ([&] {
                std::int32_t nextRandomIntRange_5 = aMyFunction::NextRandomIntRange(StationWeaponGeneration[TypeId].IntermediateLevel + 1, 8, RandomState);
                std::int32_t intermediateLevel = StationWeaponGeneration[TypeId].IntermediateLevel;
                ...
                std::int32_t randomStationEquipmentSize_6 = RandomStationEquipmentSize(aConst::WeaponInfos[StationWeaponTypes[TypeId][1]].AverageSize);
                aConst::TItemType cpp_arg_3 = StationWeaponTypes[TypeId][1];
            ...
            Weapon = ([&] {
                std::int32_t nextRandomIntRange_6 = aMyFunction::NextRandomIntRange(StationWeaponGeneration[TypeId].BasicLevel + 1, 8, RandomState);
                std::int32_t basicLevel = StationWeaponGeneration[TypeId].BasicLevel;
                ...
                std::int32_t randomStationEquipmentSize_7 = RandomStationEquipmentSize(aConst::WeaponInfos[StationWeaponTypes[TypeId][0]].AverageSize);
                aConst::TItemType cpp_arg_4 = StationWeaponTypes[TypeId][0];
        ...
        Weapon->Range = std::max<std::int32_t>(Weapon->Range, StationWeaponGeneration[TypeId].MinimumRange);
        if (TypeId == aGalaxyStruct::rstDominion) {
            CurrentStar->Dominion = this;
            {
                std::int32_t nextRandomIntRange_7 = aMyFunction::NextRandomIntRange(5, 8, RandomState);
        ...
        if (GetCargoFreeSpace() < 0) {
            GetHull()->Weight += pas::abs(GetCargoFreeSpace());
        }
    ...
    void TRuins::LoadFromBlock(EC_BlockPar::TBlockParEC* Block) {
        ...
        Text = Block->GetParam(pas::view(EC_Str::DecodeTextW(u"SihrolpaGloiordes"_w)));
        for (I = 0; I <= 7; ++I) {
            ShopGoods[aConst::GoodsTextOrder[static_cast<std::uint8_t>(I)]].Count = SysUtils::StrToInt(static_cast<pas::AnsiString>(EC_Str::ExtractDelimitedPartW(pas::view(Text), I, u","sv)));
        }
        Text = Block->GetParam(pas::view(EC_Str::DecodeTextW(u"SihrolpaGloiordesaSrakloe"_w)));
        for (I = 0; I <= 7; ++I) {
            ShopGoods[aConst::GoodsTextOrder[static_cast<std::uint8_t>(I)]].PurchasePrice = SysUtils::StrToInt(static_cast<pas::AnsiString>(EC_Str::ExtractDelimitedPartW(pas::view(Text), I, u","sv)));
        }
        Text = Block->GetParam(pas::view(EC_Str::DecodeTextW(u"SihrolpaGloiordesaBruhy"_w)));
        for (I = 0; I <= 7; ++I) {
            ShopGoods[aConst::GoodsTextOrder[static_cast<std::uint8_t>(I)]].BaseSalePrice = SysUtils::StrToInt(static_cast<pas::AnsiString>(EC_Str::ExtractDelimitedPartW(pas::view(Text), I, u","sv)));
        }
        ...
        Text = ShopBlock->GetParam(pas::view(EC_Str::DecodeTextW(u"AodEdrIstaelma"_w)));
        for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, EC_Str::CountDelimitedPartsW(pas::view(Text), u","sv) - 1); cpp_range_3.next(I); ) {
            Name = EC_Str::ExtractDelimitedPartW(pas::view(Text), I, u","sv);
            for (Kind = static_cast<std::uint8_t>(0); Kind <= static_cast<std::uint8_t>(75); ++Kind) {
                if (aConst::ItemTypeNames[static_cast<aConst::TItemType>(Kind)] == Name) {
    ...
    void TRuins_NextDayLogic(TRuins* Self) {
        ...
        try {
            ...
            Stage = 5;
            if (Self->TypeId == aGalaxyStruct::rstPirateBase) {
                Stage = 6;
            ...
            } else if (Self->TypeId == aGalaxyStruct::rstDominion) {
                Stage = 7;
    ...
    pas::WideString TRuins::GetColoredFullName(const pas::WideString& ColorTag) {
        if (TypeNameOverrideKey == u"") {
            return pas::concat_wide({aConst::LocalizedText(pas::concat_wide({u"ShipType.TypeName.", GetTypeNameKey(), u"Small"})), u" ", aMyFunction::WrapTextInColor(pas::view(Name), pas::view(ColorTag))});
        }
        return pas::concat_wide({aConst::LocalizedText(pas::concat_wide({u"ShipType.TypeName.", TypeNameOverrideKey, u"Small"})), u" ", aMyFunction::WrapTextInColor(pas::view(Name), pas::view(ColorTag))});
    }

    aGalaxyStruct::TGreetingShipCategory TRuins::GetGreetingShipCategory() {
    ...
    aGalaxyStruct::TPercent TRuins::GetStrengthScaledPirateStatus() {
        return 0;
    }
    ...
    void TRuins::RepairBrokenEquipmentAtLocation() {
        std::int32_t I{};
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
            ...
            if (aItem::TWeapon* weapon = pas::class_cast_if<aItem::TWeapon*>(Item); (!(weapon != nullptr) || weapon->GetWeaponInfo()->Availability != aGalaxyStruct::waNotSoldAndNodeRepair) && aShip::TShip_CanRepairEquipmentTech(this, Item) && (Item->BrokenFlag != 0 || Item->ConditionPercent < 1.0L)) {
                Item->BrokenFlag = 0;
    ...
    void TRuins::RefreshShopInventory() {
        std::int32_t I{};
        std::int32_t Attempts{};
        aItem::TEquipment* Item{};
        aConst::TItemType Kind{};
        if (pas::is_one_of<aGalaxyStruct::sumDisabled, aGalaxyStruct::sumGoodsOnly>(ShopUpdateMode)) {
            return;
        }
        if (aGalaxy::Galaxy->CurrentTurn > CreationTurn + 1 && static_cast<std::int32_t>(Seed + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn)) % 7 != 0) {
            return;
        }
        std::int32_t nextRandomIntRange = aMyFunction::NextRandomIntRange(0, pas::list_count(CurrentStar->Planets) - 1, RandomState);
        aMyFunction::TObjectList* planets = CurrentStar->Planets;
        ...
        {
            std::int32_t cpp_left = pas::list_count(EquipmentShop);
            if (cpp_left >= CalculateEquipmentShopTargetCount()) {
                if (Planet != nullptr) {
                    I = aMyFunction::SeededRandomIntRange(0, pas::list_count(EquipmentShop) - 1, Seed * static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn));
                    Item = pas::list_at<aItem::TEquipment>(EquipmentShop, I);
        ...
        std::int32_t Added = 0;
        while (([&] {
            pas::Extended cpp_left_2 = pas::list_count(EquipmentShop);
            return cpp_left_2 <= CalculateEquipmentShopTargetCount() * 0.7L;
        }()) || ([&] {
            std::int32_t cpp_left_3 = pas::list_count(EquipmentShop);
            return cpp_left_3 <= CalculateEquipmentShopTargetCount();
        }()) && Planet != nullptr) {
            if (Planet == nullptr) {
            ...
            do {
                ++Attempts;
                Kind = static_cast<aConst::TItemType>(aMyFunction::SeededRandomIntRange(aConst::t_Hull, aConst::WeaponCategoryItemType, Seed * static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn) * 175 + static_cast<std::uint32_t>(Attempts)));
            } while (!(Attempts > 20 || CountEquipmentShopItems(Kind) < pas::load_unaligned<std::int32_t>(pas::byte_offset(&aConst::StationEquipmentOfferQuotas[TypeId], (Kind - 42) * sizeof(std::int32_t)))));
            Item = aRuins::TRuins_GenerateEquipmentOffer(this, aPlayer::GetPlayer(), Planet, Kind);
            if (Item != nullptr) {
                pas::list_add(EquipmentShop, reinterpret_cast<void*>(Item));
    ...
    std::int32_t TRuins::CalculateEquipmentShopTargetCount() {
        aConst::TItemType Kind{};
        std::int32_t Count = 0;
        for (auto cpp_range = pas::for_to<aConst::TItemType>(aConst::t_Hull, aConst::WeaponCategoryItemType); cpp_range.next(Kind); ) {
    ...
    std::int32_t TRuins::CountEquipmentShopItems(aConst::TItemType ItemType) {
        std::int32_t I{};
        aItem::TItem* Item{};
        std::int32_t Count = 0;
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(EquipmentShop) - 1); cpp_range.next(I); ) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(EquipmentShop) - 1); cpp_range.next(I); ) {
            ...
            if (Existing->ItemType == Item->ItemType && Existing != Item && (Existing->ScriptItem == nullptr || reinterpret_cast<aScript::TScriptItem*>(Existing->ScriptItem)->Name == u"")) {
                switch (Existing->ItemType) {
                    ...
                    default: {
                        if (pas::in_range(Existing->ItemType, static_cast<std::int32_t>(aConst::t_IndustrialLaser), static_cast<std::int32_t>(aConst::t_CustomWeapon))) {
    ...
    std::int32_t TRuins::SelectEquipmentOfferSpecialMicroModule(aItem::TEquipment* Item, aPlanet::TPlanet* Planet) {
        ...
        std::int32_t Ceiling = System::Round(pas::real_divide(Planet->InventionLevels[aGalaxyStruct::piMainTech] * 100, 8.0L));
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, aConst::MicroModuleTemplateCount - 1); cpp_range.next(I); ) {
            do {
                ...
                } else if (!pas::contains(Template->OfferStationTypes, TypeId)) {
                    break;
                }
    ...
    void TRuins::AssignWeaponTargetsInStar() {
        std::int32_t I{};
        std::int32_t J{};
        aShip::TShip* Ship{};
        aItem::TWeapon* Weapon{};
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar || aMyFunction::NextRandomUnitFloat(RandomState) > 0.7L || aGalaxy::Galaxy->IsFullStationTargetingEnabled() || CurrentStar->Status.ControlFaction == aGalaxyStruct::sfDominators || CurrentStar->Status.CustomFaction != u"") {
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                ...
                if ((Ship->OwnerId == aGalaxyStruct::oiDominator || aShip::TShip_RelationToShip(Ship, this) < aGalaxyStruct::RelationBadMin) && Ship->InNormalSpace() && (static_cast<std::uint8_t>(HasIndependentScriptFaction() ^ 1) || static_cast<std::uint8_t>(Ship->HasIndependentScriptFaction() ^ 1) || reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText != reinterpret_cast<aScript::TScriptShip*>(Ship->ScriptShip)->StateText)) {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Missiles) - 1); cpp_range_2.next(I); ) {
            ...
            if (Missile->OwnerShip != this && Missile->Target == this) {
                const std::int32_t cpp_last_3 = static_cast<std::int32_t>(WeaponCount);
                if (1 <= cpp_last_3) {
                    for (J = 1; J <= cpp_last_3; ++J) {
                        Weapon = Weapons[J];
                        if (static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) && Weapon->Target == nullptr && aShip::TShip_IsEquipmentUsable(this, Weapon) && static_cast<long double>(aMyFunction::PointDistanceSquared(Position, Missile->Position)) <= pas::sqr(aShip::TShip_GetWeaponRange(this, Weapon))) {
        ...
        if (pas::list_count(CurrentStar->Items) < 10 && aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Asteroids) - 1); cpp_range_3.next(I); ) {
                ...
                DistanceSquared = aMyFunction::PointDistanceSquared(Position, Asteroid->Position);
                if (DistanceSquared <= pas::constant(static_cast<long double>(aGalaxyStruct::AsteroidTargetRangeSquared))) {
                    const std::int32_t cpp_last_4 = static_cast<std::int32_t>(WeaponCount);
                    if (1 <= cpp_last_4) {
                        for (J = 1; J <= cpp_last_4; ++J) {
                            Weapon = Weapons[J];
                            if ((aMyFunction::NextRandomUnitFloat(RandomState) <= 0.9L || GetHullIntegrityPercent() <= 90) && static_cast<std::uint8_t>(pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstAreaDamage), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) && aShip::TShip_IsEquipmentUsable(this, Weapon) && static_cast<long double>(pas::sqr(aShip::TShip_GetWeaponRange(this, Weapon))) >= DistanceSquared) {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range_2.next(I); ) {
            ...
            if (Star->Status.ControlFaction == aGalaxyStruct::sfCoalition && Star->Status.CustomFaction == u"") {
                for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Ships) - 1); cpp_range_3.next(J); ) {
                    ...
                    if (Ship->InNormalSpace() && Ship->Order == aShip::soJump && reinterpret_cast<aGalaxy::TStar*>(Ship->OrderTarget)->Status.ControlFaction == aGalaxyStruct::sfCoalition && reinterpret_cast<aGalaxy::TStar*>(Ship->OrderTarget)->Status.CustomFaction == u"" && Ship->EstimateOrderTravelTurns() >= 2 && pas::class_cast_if<aNormalShip::TNormalShip*>(Ship) != nullptr && Ship->OwnerId != aGalaxyStruct::oiPirate && pas::in_range(Ship->TypeId, static_cast<std::int32_t>(aGalaxyStruct::stRanger), static_cast<std::int32_t>(aGalaxyStruct::stTransport)) && Ship->ScriptShip == nullptr && Ship->AbsoluteScriptOrder <= 0) {
                        ++Count;
                    }
                }
            }
    ...
    void TRuins::TryAbductDepartingShip(aShip::TShip* Ship) {
        ...
        aNormalShip::TNormalShip* Candidate = static_cast<aNormalShip::TNormalShip*>(Ship);
        ...
        if (pas::in_range(Candidate->CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssPiratePassive), static_cast<std::int32_t>(aGalaxyStruct::ssPirateMilitary))) {
            return;
        }
        ...
        if (aPlayer::GetPlayer() == Candidate) {
            if (aGalaxy::Galaxy->CurrentTurn < pas::real_divide(2.0E+2L, aConst::GalaxyDifficultyTuning[aGalaxy::Galaxy->DifficultyLevels[7]].GoodsEventDurationFactor) + pas::constant(static_cast<long double>(aGalaxyStruct::GalaxyWarmupTurns))) {
                return;
            }
            {
    ...
    void TRuins::ReportAbductionOutcome() {
        pas::WideString Text{};
        std::int32_t I{};
        aShip::TShip* Ship{};
        ...
        Text = pas::concat_wide({aMyFunction::TextHighlightColorTag, GetFullName(u" "_wref.get()), aMyFunction::EndColorTag, u"\r\n"});
        if (Abducted == 0) {
            Text = pas::concat_wide_reverse({aShip::TShip_LookupTalkText(this, u"Talk.PirateClan.RuinTalkAfterAbduct.Failure"_wref.get()), Text});
        } else if (Pirates == 0) {
            Text = pas::concat_wide_reverse({aShip::TShip_LookupTalkText(this, u"Talk.PirateClan.RuinTalkAfterAbduct.SuccessNoPirate"_wref.get()), Text});
        } else {
            Text = pas::concat_wide_reverse({aShip::TShip_LookupTalkText(this, u"Talk.PirateClan.RuinTalkAfterAbduct.Success"_wref.get()), Text});
        }
        {
            auto& cpp_target = Globals::AddOrUpdatePlayerBubble(Globals::pmRadio, aGalaxy::Galaxy->CurrentTurn, Text, u""_wref.get())->Targets[0].ShipId;
            cpp_target = Id;
        }
    }
    ...
    float TRuins::EvaluateRelocationPosition(EC_Struct::TPointF Point) {
        std::int32_t I{};
        ...
        if (aShip::TShip_IsEquipmentUsable(this, GetCargoHook())) {
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range.next(I); ) {
                ...
                if (aItem::CanCargoHookHandleItem(Item, this) && static_cast<long double>(aMyFunction::PointDistanceSquared(Point, Item->Position)) <= RangeSquared && AcceptPickupItem(Item)) {
                    Result = static_cast<long double>(Result) + Item->Cost;
                }
            }
        }
        Result = pas::real_divide(Result, std::max<std::int32_t>(10, aGalaxy::Galaxy->ComputeScaledMiniMoney(aGalaxyStruct::oiHuman)));
        Result = static_cast<long double>(Result) * aMyFunction::RemapClamped(RelocationAge, 3.0E+1, 9.0E+1, 0.3, 1.0);
        if (CurrentStar->Status.Battle != 0) {
            return static_cast<long double>(Result) + EvaluateLocalForceBalance(Point);
        }
        return Result;
    }
    ...
    std::uint8_t TRuins::TryRepositionInStar() {
        static const pas::Set<0, 255> StationTypes = pas::constant_set<pas::Set<0, 255>>({{aGalaxyStruct::rstRangerCenter, aGalaxyStruct::rstCustomStation}});
        ...
        SavedPoint = Position;
        float InitialScore = EvaluateRelocationPosition(Position);
    ...
    std::uint8_t TRuins::AcceptPickupItem(aItem::TItem* Item) {
        ...
        } else if (!(pas::in_set<aConst::t_Food, aConst::t_Narcotics, aConst::t_Hull, aConst::t_CustomWeapon>(Item->ItemType) && Item->OwnerId != aGalaxyStruct::oiDominator)) {
            return Result;
        }
        return true;
    }
    ...
    void TRuins::ApplyInventoryMicroModulesToShopItems() {
        ...
        while (Applied) {
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
                ...
                if (aItem::TMicroModule* microModule = pas::class_cast_if<aItem::TMicroModule*>(Module)) {
                    for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(EquipmentShop) - 1); cpp_range_2.next(J); ) {
                        ...
                        if (microModule->CanInstallOn(Item)) {
                            Applied = true;
                            aItem::ApplyMicroModule(microModule->MicroModuleIndex - 1, Item);
    ...
    void TRuins::ChangeRelationToRanger(void* Ranger, std::int32_t Amount) {
        ...
        if (Amount > 0 && static_cast<aShip::TShip*>(Ranger)->GetEffectiveSkillLevel(aGalaxyStruct::psCharisma, false) > 0) {
            Amount += System::Round(static_cast<aShip::TShip*>(Ranger)->GetEffectiveSkillLevel(aGalaxyStruct::psCharisma, false) * Amount * 0.2L);
        }
    ...
    void TRuins::ReactToAttack(aShip::TShip* Attacker) {
        ...
        std::uint8_t Independent = static_cast<std::uint8_t>(pas::contains(aConst::FactionStandingMasks[CurrentStar->Status.ControlFaction], CurrentStanding) ^ 1) || CurrentStar->Status.CustomFaction != u"";
        ...
        if (static_cast<std::uint8_t>(Independent ^ 1) && pas::class_cast_if<aNormalShip::TNormalShip*>(Attacker) != nullptr && pas::load_unaligned<std::uint16_t>(pas::byte_offset(&static_cast<aNormalShip::TNormalShip*>(Attacker)->CurrentSystemKills, CurrentStar->Status.ControlFaction * sizeof(std::uint16_t))) == 0) {
            pas::store_unaligned<std::uint16_t>(pas::byte_offset(&static_cast<aNormalShip::TNormalShip*>(Attacker)->CurrentSystemKills, CurrentStar->Status.ControlFaction * sizeof(std::uint16_t)), static_cast<std::uint16_t>(1));
        }
    }
    ...
    std::uint8_t TRuins::AcceptsAppealFrom(aShip::TShip* Ship) {
        return false;
    }
    ...
    void TRuins::GenerateCombatSkills() {
        ...
        BaseSkills[aGalaxyStruct::psAccuracy] = Accuracy;
        BaseSkills[aGalaxyStruct::psManeuverability] = Maneuverability;
    }
    ...
    float TRuins::EvaluateStatBonus(aConst::TEquipmentBonusKind BonusKind, std::int32_t Value) {
        ...
        }
        {
            ...
            if (cpp_case == aConst::bonHull) {
                ...
                Result = static_cast<std::int8_t>(GetRadar() == nullptr);
            } else if (cpp_case == aConst::bonScan) {
                Result = Value * 20 * CountWeaponsByDamageFlags(static_cast<aGalaxyStruct::TDamageFlagSet>(ScannerFlags));
            } else if (cpp_case == aConst::bonDroid) {
                Result = pas::real_divide(Value * 20, pas::real_max<float>(0.1f, GetHull()->GetFragilityFactor(static_cast<aGalaxyStruct::TDamageFlagSet>(NoFlags))));
                ...
                if (Value > 0) {
                    Result = std::min<std::int32_t>(6 - GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false), Value) * StationSkillBonusWeights[BonusKind];
                }
                if (Value > 0 && Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) > 6) {
                    Result = Result + StationSkillBonusWeights[BonusKind] * 0.05L * (Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) - 6);
                }
                if (Value < 0) {
                    Result = std::min<std::int32_t>(static_cast<std::int32_t>(GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false)), -Value) * -StationSkillBonusWeights[BonusKind];
                }
                if (Value < 0 && Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false) < 0) {
                    Result = Result + StationSkillBonusWeights[BonusKind] * 0.03L * (Value + GetEffectiveSkillLevel(aConst::EquipmentBonusSkills[BonusKind - aConst::bonSkill1], false));
                }
            }
        }
        if (TypeId == aGalaxyStruct::rstDominion) {
    ...
    float TRuins::EvaluateWeaponDamage(aItem::TWeapon* Weapon, std::uint8_t IncludeAdditiveBonuses, float BaseDamage) {
        static const pas::Set<0, 255> ScannerFlags = pas::constant_set<pas::Set<0, 255>>({{aGalaxyStruct::dkScanBonus, aGalaxyStruct::dkDroidBlock}});
        ...
        if (pas::contains(Flags, aGalaxyStruct::dkShock)) {
            Result = Result * (1.05L + CountWeaponsByDamageFlags(static_cast<aGalaxyStruct::TDamageFlagSet>(ShockFlags)) * 0.05L);
        }
        ...
        {
            const std::int32_t cpp_last_2 = static_cast<std::int32_t>(CountEquippedWeapons());
            if (1 <= cpp_last_2) {
                for (I = 1; I <= cpp_last_2; ++I) {
                    if (!pas::in_range(Weapons[I]->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstAreaDamage), static_cast<std::int32_t>(aGalaxyStruct::wstRocket))) {
                        HasOtherWeapon = true;
                    }
                }
            }
    ...
    void TRuins_RefreshCurrentStanding(TRuins* Self) {
        ...
        if (StandingMode == aGalaxyStruct::ssmCustomFaction) {
            ...
            if (Self->TypeId != aGalaxyStruct::rstCustomStation) {
    ...
    aItem::THull* TRuins::GeneratePlanetHullOffer(pas::Object* Ship, aPlanet::TPlanet* Planet) {
        ...
        if (Planet != nullptr && Ship != nullptr && pas::class_cast_if<aShip::TShip*>(Ship) != nullptr) {
            Buyer = static_cast<aShip::TShip*>(Ship);
            Result = Planet->GenerateHullOffer(Buyer);
            if (Result != nullptr) {
    ...
    aItem::THull* TRuins::GenerateHullOffer(pas::Object* Ship, aPlanet::TPlanet* Planet) {
        ...
        aItem::THull* Result = nullptr;
        if (TypeId == aGalaxyStruct::rstCustomStation) {
            return GeneratePlanetHullOffer(Ship, Planet);
        }
        if (Ship != nullptr && pas::class_cast_if<aShip::TShip*>(Ship) != nullptr) {
            Buyer = static_cast<aShip::TShip*>(Ship);
            Count = 0;
            for (auto cpp_range = pas::for_to<aGalaxyStruct::THullType>(aGalaxyStruct::htRanger, aGalaxyStruct::htFlagship); cpp_range.next(HullType); ) {
            ...
            for (auto cpp_range_2 = pas::for_to<aGalaxyStruct::THullType>(aGalaxyStruct::htRanger, aGalaxyStruct::htFlagship); cpp_range_2.next(HullType); ) {
                if (pas::contains(StationOfferHullTypes[TypeId], HullType)) {
                    --Count;
                }
                if (Count == 0) {
            ...
            if (aPlayer::GetPlayer() == Buyer || Buyer->GetHull()->HullType == aGalaxyStruct::htSpecial || Buyer->GetHull()->HullType == HullType) {
                ...
                switch (aGalaxy::Galaxy->GetHullGrowthMod()) {
                    case 1: {
                        ...
                        if (pas::is_one_of<aGalaxyStruct::htTransport, aGalaxyStruct::htLiner>(HullType)) {
                            MaxSize = Size + (40 + 10 * (TypeId == aGalaxyStruct::rstBusinessCenter)) * aGalaxy::Galaxy->TechLevel;
                        } else {
                            switch (HullType) {
                        ...
                        MinSize = std::max<std::int64_t>(static_cast<std::int64_t>(MinSize), System::Round(static_cast<long double>(aConst::HullBaseSize) * aConst::EquipmentSizeFactors[5 - (TypeId == aGalaxyStruct::rstBusinessCenter)]));
                    ...
                    case 2: {
                        ...
                        MinSize = System::Round(static_cast<long double>(aConst::HullBaseSize) * aConst::EquipmentSizeFactors[5 - (TypeId == aGalaxyStruct::rstBusinessCenter)]);
                    ...
                    default: {
                        ...
                        MinSize = Size / 2;
                        if (pas::is_one_of<aGalaxyStruct::htTransport, aGalaxyStruct::htLiner>(HullType)) {
                            MaxSize = Size + 300 + 100 * (TypeId == aGalaxyStruct::rstBusinessCenter);
                        } else {
                            switch (HullType) {
                ...
                if (Buyer->GetHull()->OwnerId == Owner || aPlayer::GetPlayer() == Buyer) {
                    ...
                    if (Flagship) {
                        ...
                        Result->Init(nextRandomIntRange_2, nextRandomIntRange, Owner, aGalaxyStruct::htFlagship, Series, pirateBuilt);
                    } else {
    ...
    aItem::TWeapon* TRuins::GenerateWeaponOffer(pas::Object* Ship, aPlanet::TPlanet* Planet) {
        ...
        aGalaxyStruct::TOwnerId Owner{};
        aGalaxyStruct::TOwnerId CandidateOwner{};
        std::int32_t ModuleIndex{};
        aItem::TWeapon* Result = nullptr;
        if (Ship != nullptr && pas::class_cast_if<aShip::TShip*>(Ship) != nullptr) {
            Buyer = static_cast<aShip::TShip*>(Ship);
            Availability = pas::constant_set<aGalaxyStruct::TWeaponAvailabilityMask>({{aGalaxyStruct::waFree}});
            if (Buyer->TypeId == aGalaxyStruct::stKling && pas::contains(aConst::PlanetOwnerMasks.Dominators, OwnerId)) {
                Availability = Availability + pas::constant_set<aGalaxyStruct::TWeaponAvailabilityMask>({{aGalaxyStruct::waNotSoldAndNodeRepair}});
            }
            if (pas::is_one_of<aGalaxyStruct::stRanger, aGalaxyStruct::stPirate>(Buyer->TypeId) && pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfPirates], CurrentStanding) && (CurrentStar->Status.ControlFaction == aGalaxyStruct::sfPirates || static_cast<std::uint8_t>(pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfCoalition], CurrentStanding) ^ 1))) {
                Availability = Availability + pas::constant_set<aGalaxyStruct::TWeaponAvailabilityMask>({{aGalaxyStruct::waPirateOnly}});
            }
            if (pas::in_range(Buyer->TypeId, static_cast<std::int32_t>(aGalaxyStruct::stRanger), static_cast<std::int32_t>(aGalaxyStruct::stWarrior)) && pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfCoalition], CurrentStanding) && (CurrentStar->Status.ControlFaction == aGalaxyStruct::sfCoalition || static_cast<std::uint8_t>(pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfPirates], CurrentStanding) ^ 1))) {
                Availability = Availability + pas::constant_set<aGalaxyStruct::TWeaponAvailabilityMask>({{aGalaxyStruct::waCoalitionOnly}, {aGalaxyStruct::waMalocOnly, aGalaxyStruct::waGaalOnly}});
            }
            // The native counter guard has no back edge: only one offer is generated.
            Attempts = 0;
            if (Attempts <= 100) {
                ++Attempts;
                Info = aGalaxy::Galaxy->SelectWeaponInfo(RandomState, Availability, Planet->InventionLevels[aGalaxyStruct::piMainTech], 1);
                ...
                if (aWarrior::TWarrior* warrior = pas::class_cast_if<aWarrior::TWarrior*>(Buyer); warrior != nullptr && warrior->WarriorType == aWarrior::wtFlagship) {
                    MinSize *= 2;
                    MaxSize *= 2;
                }
                MinLevel = 1;
                MaxLevel = std::min<std::int32_t>(static_cast<std::int32_t>(Planet->InventionLevels[aGalaxyStruct::piMainTech]), static_cast<std::int32_t>(Planet->InventionLevels[Info->InventionIndex]));
                ...
                Owner = aConst::PickRandomEquipmentOwner(RandomState);
                if (CurrentStar->Status.ControlFaction == aGalaxyStruct::sfPirates && pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfPirates], CurrentStanding) && (aMyFunction::NextRandomIntRange(1, 100, RandomState) < 70 || aGalaxy::Galaxy->CoalitionDefeatedTurn != 0)) {
                    Owner = aGalaxyStruct::oiPirate;
                }
                for (auto cpp_range = pas::for_to<aGalaxyStruct::TOwnerId>(aGalaxyStruct::oiMaloc, aGalaxyStruct::oiPirate); cpp_range.next(CandidateOwner); ) {
                    if (aConst::OwnerWeaponAvailability[CandidateOwner] == Info->Availability) {
                ...
                if (aWarrior::TWarrior* warrior_2 = pas::class_cast_if<aWarrior::TWarrior*>(Buyer); warrior_2 != nullptr && warrior_2->WarriorType == aWarrior::wtFlagship) {
    ...
    aItem::TEquipment* TRuins_GenerateEquipmentOffer(TRuins* Self, pas::Object* Ship, aPlanet::TPlanet* Planet, aConst::TItemType ItemType) {
        ...
        aShip::TShip* Buyer = static_cast<aShip::TShip*>(Ship);
        if (pas::in_range(ItemType, static_cast<std::int32_t>(aConst::t_FuelTanks), static_cast<std::int32_t>(aConst::t_DefGenerator))) {
            if (static_cast<std::uint8_t>(pas::is_one_of<aConst::t_FuelTanks, aConst::t_Engine>(ItemType) ^ 1) && Buyer->GetSlotCountForItemType(ItemType) == 0 && aPlayer::GetPlayer() != Buyer) {
                return Result;
            }
            MinLevel = 1;
            ...
            if (Self->CurrentStar->Status.ControlFaction == aGalaxyStruct::sfPirates && pas::contains(aConst::FactionStandingMasks[aGalaxyStruct::sfPirates], Self->CurrentStanding) && (aMyFunction::NextRandomIntRange(1, 100, Self->RandomState) < 70 || aGalaxy::Galaxy->CoalitionDefeatedTurn != 0)) {
                Owner = aGalaxyStruct::oiPirate;
            }
            {
        ...
        switch (Self->TypeId) {
            case aGalaxyStruct::rstBusinessCenter: {
                Result->Cost = std::min<std::int64_t>(static_cast<std::int64_t>(aGalaxyStruct::MaxMonetaryValue), System::Round(Result->Cost * 1.2L));
        ...
        if (Result->CanImprove()) {
            if (Self->TypeId == aGalaxyStruct::rstScienceBase) {
            ...
            } else if (Self->TypeId == aGalaxyStruct::rstRangerCenter) {
        ...
        std::int32_t Attempts = 0;
        if (Self->TypeId == aGalaxyStruct::rstDominion && aMyFunction::NextRandomIntRange(0, 100, Self->RandomState) > 50) {
            do {
    ...
    aMyFunction::TObjectList* TRuins_GenerateEquipmentOfferBatch(TRuins* Self, aShip::TShip* Ship, std::uint8_t UnusedForceGeneratedOffers) {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, aConst::CountItemTypesInMask(pas::constant_set<aConst::TItemTypeSelection>({{43, 49}}))); cpp_range_2.next(I); ) {
            Kind = static_cast<aConst::TItemType>(aConst::GetItemTypeFromMask(pas::constant_set<aConst::TItemTypeSelection>({{43, 49}}), I));
        ...
        for (auto cpp_range_4 = pas::for_to<std::int32_t>(1, aConst::StationEquipmentOfferQuotas[Self->TypeId].Weapons); cpp_range_4.next(I); ) {
            Planet = static_cast<aPlanet::TPlanet*>(Self->CurrentStar->SelectRandomInhabitedPlanet());
            Item = aRuins::TRuins_GenerateEquipmentOffer(Self, Ship, Planet, aConst::WeaponCategoryItemType);
            if (Item != nullptr) {
                pas::list_add(Result, reinterpret_cast<void*>(Item));
            }
        }
    ...
    void TRuins::UpdateGoodsMarketState() {
        ...
        aGalaxyStruct::TOwnerId Race = PilotRace;
        for (Good = 0; Good <= 7; ++Good) {
    ...
    std::int32_t TRuins::CalculateRepairCost(aShip::TShip* Ship, std::int32_t& EquipmentCost) {
        ...
        if (TypeId == aGalaxyStruct::rstMilitaryBase) {
            EquipmentFactor = aMyFunction::RemapClamped(aPlayer::GetPlayer()->Rank, 0.0, 7.0, 0.9, 0.2);
        }
        if (TypeId == aGalaxyStruct::rstPirateBase) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Ship->Inventory) - 1); cpp_range.next(I); ) {
            ...
            if (aItem::TWeapon* weapon = pas::class_cast_if<aItem::TWeapon*>(Item); (!(weapon != nullptr) || weapon->GetWeaponInfo()->Availability != aGalaxyStruct::waNotSoldAndNodeRepair || CanRepairArtefactsAtLocation()) && aShip::TShip_CanRepairEquipmentTech(this, Item) && (Item->ItemType == aConst::t_Hull || Item->EquippedFlag != 0 && Item->ConditionPercent < 9.0E+1L)) {
        ...
        if (CanRepairArtefactsAtLocation()) {
            ArtefactFactor = 1.0f;
            if (TypeId == aGalaxyStruct::rstScienceBase) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range.next(I); ) {
            ...
            if (Star->Status.Battle == 0 && Star->Status.ControlFaction != aGalaxyStruct::sfDominators && Star->Status.CustomFaction == u"" && Star->ShipTypeCounts[aGalaxyStruct::rstPirateBase] != 0) {
                for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Ships) - 1); cpp_range_2.next(J); ) {
                    Ship = pas::list_at<aShip::TShip>(Star->Ships, J);
                    if (Ship->TypeId == aGalaxyStruct::rstPirateBase && Ship->TypeNameOverrideKey == u"" && Ship->NodeReserve > 0) {
    ...
    std::uint8_t TRuins_CanDock(TRuins* Self, aShip::TShip* Ship) {
        std::uint8_t Result = false;
        if (Self->InNormalSpace() && Ship->EnemyShip != Self && aShip::TShip_GetRelationLevelToShip(Self, Ship) > aGalaxyStruct::rlHostile && static_cast<std::uint8_t>(Self->NoLanding ^ 1) && (static_cast<std::uint8_t>(pas::in_range(Self->CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssPirateActive), static_cast<std::int32_t>(aGalaxyStruct::ssPirateMilitary)) ^ 1) || pas::in_range(Ship->CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssPiratePassive), static_cast<std::int32_t>(aGalaxyStruct::ssPirateMilitary)))) {
            return true;
        }
        return Result;
    }
    ...
    std::uint8_t TRuins::CheckDockingPermission(aShip::TShip* Ship, pas::WideString& Response) {
        ...
        if (NoLanding || static_cast<std::uint8_t>(InNormalSpace() ^ 1)) {
            ...
            return Result;
        } else if (pas::in_range(CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssPirateActive), static_cast<std::int32_t>(aGalaxyStruct::ssPirateMilitary)) && static_cast<std::uint8_t>(pas::in_range(Ship->CurrentStanding, static_cast<std::int32_t>(aGalaxyStruct::ssPiratePassive), static_cast<std::int32_t>(aGalaxyStruct::ssPirateMilitary)) ^ 1)) {
