...
namespace aShip {
    ...
    void FinishChainExplosionFilm(std::int32_t& StepIndex, pas::List*& Ships, pas::List*& Damages, pas::List*& Colors, pas::List*& Films);

    void EquipType(aConst::TItemType ArtefactType, TShip* Self, std::int32_t& RemainingSlots);

    void AccumulateEquipmentBonus(float Value, float& Positive, float& Nonpositive);

    std::uint8_t FitsMicroModuleCapacity(TShip* Self, aItem::TEquipment*& Candidate, aItem::TEquipment*& Module);

    aItem::TItem* SelectCheapItem(TShip* Self, std::int32_t& Index);

    aItem::TItem* SelectValuableItem(TShip* Self, std::int32_t& Index);

    std::int32_t GetRetainedCapacity(TShip* Self);

    std::uint8_t FitsGeneratedModuleCapacity(TShip* Self, aItem::TEquipment*& Item, std::int32_t& ModuleIndex);

    std::uint8_t AcceptDuplicate(TShip* Self, std::int32_t& ModuleIndex);

    aItem::TWeapon* SelectReplacedWeapon(TShip* Self);

    void TemporarilyUnequipWeapon(aItem::TWeapon* Weapon, TShip* Self, aItem::TWeapon*& ReplacementWeapon, pas::Array<aItem::TWeapon*, 1, 5>& SavedWeapons, pas::Object*& SavedTarget);

    void RestoreWeapon(TShip* Self, aItem::TWeapon*& ReplacementWeapon, pas::Array<aItem::TWeapon*, 1, 5>& SavedWeapons, pas::Object*& SavedTarget);

    std::uint8_t AcceptPickupTarget(aItem::TItem* Item, TShip* Self, std::uint8_t& IgnoreRange);

    void InitializeFilm(std::uint8_t Alpha, TShip* Self, std::int32_t& StartStepIndex);

    void AdvancePath(TShip* Self, std::int32_t& StepIndex, std::uint8_t& RecordFilm, aPath::PSPathNode& Node, std::uint8_t& Angle);

    std::uint8_t IsArtefactBoostEquipment(aItem::TEquipment* Item);

    void SelectOrdinal(std::int32_t Ordinal, std::uint8_t Equipped, TShip* Self, aItem::TEquipment*& Item);

    void SelectRepairable(std::int32_t Count, std::uint8_t Equipped, TShip* Self, aItem::TEquipment*& Item);

    pas::WideString GetTalkContextPrefix(TShip* Ship);

    std::uint8_t IsBetterInterceptorTarget(TShip* Current, TShip* Candidate, aItem::TInterceptorTargetingStrategy Strategy, TShip* Self);

    void AppendStatusLine(pas::WideString TextKey, std::int32_t& Count, std::uint8_t& ShowStrength, pas::WideString& Result, std::int32_t& Strength);
    ...
    void TShip::LoadFromBlock(EC_BlockPar::TBlockParEC* Block) {
        ...
        }
        {
            ...
            for (auto cpp_range_6 = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range_6.next(I); ) {
                Item = pas::list_at<aItem::TItem>(Artefacts, I);
                Text = pas::concat_wide_reverse({pas::wide_int64_to_str(static_cast<std::int64_t>(static_cast<std::uint32_t>(Item->Id))), EC_Str::DecodeTextW(u"ImtreamrIodo"_w)});
                Item->LoadFromBlock(cpp_with_3->GetBlockByPath(Text));
            }
        ...
        }
        {
            ...
            for (auto cpp_range_8 = pas::for_to<std::int32_t>(0, pas::list_count(GuaranteedDeathDropItems) - 1); cpp_range_8.next(I); ) {
                Item = pas::list_at<aItem::TItem>(GuaranteedDeathDropItems, I);
                Text = pas::concat_wide_reverse({pas::wide_int64_to_str(static_cast<std::int64_t>(static_cast<std::uint32_t>(Item->Id))), EC_Str::DecodeTextW(u"ImtreamrIodo"_w)});
                Item->LoadFromBlock(cpp_with_4->GetBlockByPath(Text));
            }
    ...
    void TShip::ResolveLoadedReferences(aGalaxy::TGalaxy* Galaxy) {
        std::int32_t I{};
        std::int32_t Count{};
        aItem::TItem* Item{};
        pas::list_add(Galaxy->LoadedShips, reinterpret_cast<void*>(this));
        TransitOriginStar = pas::checked_cast<aGalaxy::TStar*>(static_cast<pas::Object*>(Galaxy->IdToStar(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(TransitOriginStar)))));
        CurrentPlanet = pas::checked_cast<aPlanet::TPlanet*>(static_cast<pas::Object*>(Galaxy->IdToPlanet(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(CurrentPlanet)), true)));
        if (pas::class_cast_if<aPlayer::TPlayer*>(this) != nullptr && static_cast<aPlayer::TPlayer*>(this)->RuinsMode > 0) {
            DockedTo = pas::checked_cast<aPlayer::TPlayer*>(this)->RuinsProxy;
        } else {
            DockedTo = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(DockedTo)), true)));
        }
        HomePlanet = pas::checked_cast<aPlanet::TPlanet*>(static_cast<pas::Object*>(Galaxy->IdToPlanet(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(HomePlanet)), true)));
        if (GlobalsV::LoadedSaveVersion < 146 && HomePlanet != nullptr) {
            TechKnowledge = std::max<std::int32_t>(static_cast<std::int32_t>(TechKnowledge), static_cast<std::int32_t>(HomePlanet->InventionLevels[7]));
        }
        if (PickupTargets != nullptr) {
            Count = pas::list_count(PickupTargets);
            for (auto cpp_range = pas::for_to<std::int32_t>(0, Count - 1); cpp_range.next(I); ) {
                pas::list_put(PickupTargets, I, reinterpret_cast<void*>(pas::checked_cast<aItem::TItem*>(static_cast<pas::Object*>(Galaxy->IdToItem(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(pas::list_get(PickupTargets, I))), true)))));
            }
        }
        EnemyShip = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(EnemyShip)), false)));
        TruceShip = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(TruceShip)), false)));
        PartnerShip = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(PartnerShip)), false)));
        InterceptorSourceShip = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(InterceptorSourceShip)), false)));
        if (Order == soJump) {
            OrderTarget = pas::checked_cast<aGalaxy::TStar*>(static_cast<pas::Object*>(Galaxy->IdToStar(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)))));
        } else if (Order == soJumpHole) {
            OrderTarget = pas::checked_cast<aGalaxy::THole*>(static_cast<pas::Object*>(Galaxy->IdToHole(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)))));
        } else if (Order == soTeleport) {
            OrderTarget = pas::checked_cast<aGalaxy::TStar*>(static_cast<pas::Object*>(Galaxy->IdToStar(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)))));
        } else if (Order == soLand) {
            if ((static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)) & 0x80000000u) == 0x80000000u) {
                OrderTarget = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)) & 0x7fffffff, true)));
            } else {
                OrderTarget = pas::checked_cast<aPlanet::TPlanet*>(static_cast<pas::Object*>(Galaxy->IdToPlanet(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)), true)));
            }
        } else if (Order == soFollowShip) {
            OrderTarget = pas::checked_cast<TShip*>(static_cast<pas::Object*>(Galaxy->IdToShip(static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(OrderTarget)), true)));
        }
        Count = pas::list_count(Inventory);
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, Count - 1); cpp_range_2.next(I); ) {
            Item = pas::list_at<aItem::TItem>(Inventory, I);
        ...
        if (GlobalsV::LoadedSaveVersion < 85 && TypeId != aGalaxyStruct::stTranclucator) {
            RefreshCurrentStanding();
        }
        if (pas::in_set<92, 93>(GlobalsV::LoadedSaveVersion) && pas::class_cast_if<aNormalShip::TNormalShip*>(this) != nullptr && !(pas::class_cast_if<aPlayer::TPlayer*>(this) != nullptr)) {
            if (PilotRace == static_cast<std::uint8_t>(aGalaxyStruct::oiHuman) && pas::in_range(PortraitFaceId, 25, 32) && aPlayer::GetPlayer() != this && aPlayer::GetPlayer() != nullptr && pas::class_cast_if<aNormalShip::TNormalShip*>(this) != nullptr) {
                return;
            }
            if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr && OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiPirate)) {
                GetHull()->OwnerId = aConst::RaceToOwner(PilotRace);
            } else {
                GetHull()->OwnerId = OwnerId;
            }
            GetHull()->HullType = aConst::ShipToHullType(this);
            GetHull()->SpecialModuleIndex = 0;
            if (GetHull()->HullSeries != -1) {
                if (static_cast<std::uint8_t>(pas::contains(aConst::HullSeriesDefinitions[GetHull()->HullSeries].AllowedOwners, GetHull()->OwnerId) ^ 1) || static_cast<std::uint8_t>(pas::contains(aConst::HullSeriesDefinitions[GetHull()->HullSeries].AllowedShipTypes, GetHull()->HullType) ^ 1)) {
        ...
        if (DockedTo != nullptr && pas::class_cast_if<aRuins::TRuins*>(DockedTo) != nullptr || CurrentPlanet != nullptr && pas::in_set<0, 4, 7, 7>(CurrentPlanet->OwnerId) && reinterpret_cast<aPlanet::TPlanet*>(reinterpret_cast<std::uint8_t*>(CurrentPlanet) + 0)->GetRelationLevelToShip(this) != 0 || pas::class_cast_if<aRuins::TRuins*>(this) != nullptr) {
            if (aPlayer::GetPlayer() != this) {
                ...
                for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
                    ...
                    if (!CanUseEquipmentTech(Item)) {
                        pas::list_add(UseList, reinterpret_cast<void*>(Item));
                    }
                    if (!CanRepairEquipmentTech(Item)) {
    ...
    void TShip::NextDay() {
        ...
        try {
            ...
            if (Order == soFollowShip) {
                if (GetRelationLevelToShip(reinterpret_cast<TShip*>(OrderTarget)) > aGalaxyStruct::rlHostile) {
                    if (PartnerShip != OrderTarget) {
                        if (!(pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) || pas::checked_cast<aTranclucator::TTranclucator*>(this)->OwnerShip != OrderTarget) {
                            if (aPlayer::GetPlayer() != this && static_cast<std::uint8_t>(HasScriptControl() ^ 1)) {
                                OrderNone(false);
                            }
                        }
                    }
            ...
            if (InHyperspace || CurrentPlanet != nullptr) {
                TruceShip = nullptr;
            }
            if (!InNormalSpace()) {
            ...
            Stage = 2;
            if (aGalaxy::Galaxy->CurrentTurn > LastProcessedTurn || aGalaxy::Galaxy->StasisModEnabled == 1 && aPlayer::GetPlayer() == this) {
                if ((CurrentPlanet != nullptr || DockedTo != nullptr) && !(aPlayer::GetPlayer() == this && GlobalsV::CurrentScreenId == GlobalsV::screenPlanetQuest)) {
                    ++ConsecutiveDockedDays;
                } else {
                    ConsecutiveDockedDays = 0;
                }
                ++DaysSincePlayerSeen;
                ...
                Stage = 3;
                if (aPlayer::GetPlayer() != this && TypeId != aGalaxyStruct::stTranclucator) {
                    AutoEquipInventory();
                    AutoEquipArtefacts();
                    if (InNormalSpace()) {
                        DropCargoUntilNotOverloaded();
                    }
                    if (!(pas::class_cast_if<aRuins::TRuins*>(this) != nullptr) && DaysSincePlayerSeen > 100 && (aGalaxy::Galaxy->CurrentTurn + Id) % 50 == 0) {
                        ReloadWeaponAmmo();
                    }
                }
                if (PartnerShip != nullptr && PartnershipDaysRemaining > 0) {
                ...
                if (GetHull()->EnergyMax > GetHull()->Energy) {
                    GetHull()->Energy = std::min<std::int32_t>(GetHull()->EnergyMax, GetHull()->Energy + GetHullEnergyRegeneration());
                }
                Stage = 5;
                if (GetEngine() != nullptr && GetEngine()->OutputPercent < 100) {
                    RefreshDerivedStats(true);
                    if (GetEngine()->OutputPercent + 10 > 100) {
                        GetEngine()->OutputPercent = 100;
                    } else {
                        GetEngine()->OutputPercent += 10;
                    }
                }
                Stage = 6;
                ...
                if (pas::list_count(Artefacts) > 0) {
                    if (GetFuelTanks() != nullptr) {
                        if (GetFuelTanks()->Fuel < GetFuelTanks()->Capacity) {
                            GetFuelTanks()->Fuel += std::min<std::int32_t>(([&] {
                                std::int32_t cpp_left = aConst::FuelArtefactBase + aConst::FuelArtefactBoost * (CanBoostArtefact(aConst::t_ArtefactFuel, nullptr, false) & 127);
                                return cpp_left * CountActiveArtefacts(aConst::t_ArtefactFuel);
                            }()), GetFuelTanks()->Capacity - GetFuelTanks()->Fuel);
                        }
                    }
                    if (GetEngine() != nullptr && GetEngine()->OutputPercent < 100) {
                        GetEngine()->OutputPercent += std::min<std::int32_t>(([&] {
                            std::int32_t cpp_left_2 = aConst::EngineArtefactBase + aConst::EngineArtefactBoost * (CanBoostArtefact(aConst::t_ArtefactPower, GetEngine(), false) & 127);
                            return cpp_left_2 * CountActiveArtefacts(aConst::t_ArtefactPower);
                        }()), 100 - GetEngine()->OutputPercent);
                    }
                    for (auto cpp_range = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtefactNano)); cpp_range.next(I); ) {
                ...
                if (aPlayer::GetPlayer() == this) {
                    if (InNormalSpace()) {
                        ...
                        if (pas::in_set<1, 3, 5, 6>(Order)) {
                            {
                                pas::Extended cpp_right = aMyFunction::RemapClamped(CalculateMass(), aConst::WearMassMin, aConst::WearMassMax, 1.0, 1.0E+1);
                                double cpp_arg = aMyFunction::NextRandomUnitFloat(RandomState) * 0.5L * cpp_right * WearFactor;
                                aItem::TEquipment* engine = GetEngine();
                                TShip* self = this;
                                self->ApplyItemDegradation(engine, idkUse, cpp_arg);
                            }
                            {
                                double cpp_arg_2 = aMyFunction::NextRandomUnitFloat(RandomState) * 0.5L * WearFactor;
                                aItem::TEquipment* fuelTanks = GetFuelTanks();
                                TShip* self_2 = this;
                                self_2->ApplyItemDegradation(fuelTanks, idkUse, cpp_arg_2);
                            }
                            if (GetEngine() != nullptr) {
                                if (GetEngine()->BrokenFlag != 0) {
                                    if (Order == soJump) {
                                        std::int64_t cpp_right_2 = System::Round(aMyFunction::PointDistance(pas::checked_cast<aGalaxy::TStar*>(OrderTarget)->Position, CurrentStar->Position));
                                        if (GetFuelLimitedJumpRange() < cpp_right_2) {
                                            ([&] {
                                                const pas::WideString& formatText1 = ([&] {
                                                    auto name = pas::borrow(pas::checked_cast<aGalaxy::TStar*>(OrderTarget)->Name);
                        ...
                        if (GetRadar() != nullptr) {
                            double cpp_arg_3 = aMyFunction::NextRandomUnitFloat(RandomState) * 0.3L * WearFactor;
                            aItem::TEquipment* radar = GetRadar();
                            TShip* self_3 = this;
                            self_3->ApplyItemDegradation(radar, idkUse, cpp_arg_3);
                        }
                        if (GetScanner() != nullptr) {
                            double cpp_arg_4 = aMyFunction::NextRandomUnitFloat(RandomState) * 0.3L * WearFactor;
                            aItem::TEquipment* scanner = GetScanner();
                            TShip* self_4 = this;
                            self_4->ApplyItemDegradation(scanner, idkUse, cpp_arg_4);
                        }
                        if (GetDefGenerator() != nullptr) {
                            double cpp_arg_5 = aMyFunction::NextRandomUnitFloat(RandomState) * 0.3L * WearFactor;
                            aItem::TEquipment* defGenerator = GetDefGenerator();
                            TShip* self_5 = this;
                            self_5->ApplyItemDegradation(defGenerator, idkUse, cpp_arg_5);
                        }
                        {
                            double nextRandomFloatRange = aMyFunction::NextRandomFloatRange(0.1L * WearFactor, 0.3L * WearFactor, RandomState);
                            TShip* self_6 = this;
                    ...
                    Stage = 11;
                    {
                        double nextRandomFloatRange_2 = aMyFunction::NextRandomFloatRange(0.1, 0.3, RandomState);
                        TShip* self_7 = this;
                        self_7->ApplyArtefactUseDegradation(nextRandomFloatRange_2);
                    }
                    if (pas::in_set<1, 3>(TypeId) && CalculateMass() > aConst::WearMassMin && Order != soNone) {
                        pas::Extended cpp_right_3 = aMyFunction::RemapClamped(CalculateMass(), aConst::WearMassMin, aConst::WearMassMax, 0.0, 9.0);
                        double cpp_arg_6 = aMyFunction::NextRandomUnitFloat(RandomState) * 0.5L * cpp_right_3 * 0.5L;
                        aItem::TEquipment* engine_2 = GetEngine();
                        TShip* self_8 = this;
                        self_8->ApplyItemDegradation(engine_2, idkUse, cpp_arg_6);
                    }
                    ...
                    if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && static_cast<aKling::TKling*>(this)->ActiveProgramAppliedTurn > 0) {
                        Stage = 12;
                        Kling = pas::checked_cast<aKling::TKling*>(this);
                        {
            ...
            if (pas::Exception* E = pas::class_cast_if<pas::Exception*>(cpp_exception)) {
                ...
                pas::raise(pas::make_exception<pas::Exception>(static_cast<pas::AnsiString>(pas::concat_wide({u"Error in procedure TShip.NextDay ", GetFullName(u" "_wref.get()), u" label = ", pas::wide_int_to_str(Stage)}))));
            } else {
                throw;
            }
        }
    ...
    pas::WideString TShip::GetSpaceInfoText() {
        ...
        switch (Order) {
            case soLand: {
                if (pas::class_cast_if<aPlanet::TPlanet*>(OrderTarget) != nullptr) {
                    Result = pas::concat_wide({Result, u" ", ([&] {
                        ...
                        pas::WideString name = pas::checked_cast<aPlanet::TPlanet*>(OrderTarget)->Name;
            ...
            case soJump: {
                Result = pas::concat_wide({Result, u" ", ([&] {
                    ...
                    pas::WideString name_3 = pas::checked_cast<aGalaxy::TStar*>(OrderTarget)->Name;
            ...
            case soFollowShip: {
                if (pas::checked_cast<TShip*>(OrderTarget)->CurrentPlanet == nullptr) {
                    if (EnemyShip != OrderTarget) {
                        Result = pas::concat_wide({Result, u" ", ([&] {
                            pas::WideString name_4 = pas::checked_cast<TShip*>(OrderTarget)->GetName();
                        ...
                        Result = pas::concat_wide({Result, u" ", ([&] {
                            pas::WideString name_5 = pas::checked_cast<TShip*>(OrderTarget)->GetName();
        ...
        Result = pas::concat_wide({Result, u"\r\n", ([&] {
            pas::WideString intToStr = pas::wide_int_to_str(GetHull()->Weight);
        ...
        if (GetHull()->Weight > GetHull()->HullPoints) {
            Result = pas::concat_wide({Result, u" ", ([&] {
                pas::WideString int64ToStr = pas::wide_int64_to_str(System::Trunc(1.0E+2L - pas::real_divide(GetHull()->HullPoints, GetHull()->Weight * 0.01L)));
        ...
        Result = pas::concat_wide({Result, u"\r\n", ([&] {
            pas::WideString intToStr_2 = pas::wide_int_to_str(CalculateSpeed());
        ...
        Result = pas::concat_wide({Result, u"\r\n", ([&] {
            pas::WideString intToStr_3 = pas::wide_int_to_str(GetDefensePercent() & 0x0000007f);
        ...
        if (aPlayer::GetPlayer() != this) {
            Result = pas::concat_wide({Result, u"\r\n", ([&] {
                pas::WideString lowerCaseWideString = EC_Str::LowerCaseWideString(GetRelationLevelTextToShip(aPlayer::GetPlayer()));
            ...
            if (aPlayer::GetPlayer()->CountActiveArtefacts(aConst::t_ArtefactAnalyzer) > 0) {
                return pas::concat_wide({Result, u"\r\n", ([&] {
                    pas::WideString intToStr_4 = pas::wide_int_to_str(aPlayer::GetPlayer()->GetWinChancePercent(this) & 0x0000007f);
    ...
    pas::WideString TShip::LookupVisibleTalkText(const pas::WideString& Path, TShip* OtherShip) {
        pas::WideString Result{};
        if (aPlayer::GetPlayer() != nullptr && aPlayer::GetPlayer()->CurrentStar == CurrentStar) {
    ...
    double TShip::CalculateAttackStrength() {
        ...
        double Result = 1.0E-7;
        {
            ...
            if (1 <= cpp_last) {
                for (I = 1; I <= cpp_last; ++I) {
                    if (IsEquipmentUsable(Weapons[I])) {
    ...
    double TShip::GetRepairStrengthFactor() {
        if (GetHullIntegrityPercent() > 50 && IsEquipmentUsable(GetRepairRobot())) {
    ...
    pas::WideString TShip::GetCaptainPortraitResourceBase() {
        pas::WideString Result{};
        ...
        if (aPlayer::GetPlayer() == this) {
            ...
            return pas::concat_wide({u"Bm.Captain.2", aConst::OwnerInfo[aConst::RaceToOwner(PilotRace) & 0x0000007f].InternalName, pas::wide_int_to_str(PortraitFaceId)});
        }
        if (HasScriptStateText() && GR_Main::CacheDataRoot->FileExistsByPath(pas::concat_wide({u"Bm.Captain.2", reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText, pas::wide_int_to_str(PortraitFaceId), u"i"})) && GR_Main::CacheDataRoot->FileExistsByPath(pas::concat_wide({u"Bm.Captain.2", reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText, pas::wide_int_to_str(PortraitFaceId), u"a"}))) {
            return pas::concat_wide({u"Bm.Captain.2", reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText, pas::wide_int_to_str(PortraitFaceId)});
        }
        if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) {
            if (PortraitFaceId >= 0 && GR_Main::CacheDataRoot->FileExistsByPath(static_cast<pas::WideString>(pas::concat_ansi({"Bm.Captain.2Tranclucator", SysUtils::IntToStr(PortraitFaceId), "i"}))) && GR_Main::CacheDataRoot->FileExistsByPath(static_cast<pas::WideString>(pas::concat_ansi({"Bm.Captain.2Tranclucator", SysUtils::IntToStr(PortraitFaceId), "a"})))) {
                return static_cast<pas::WideString>(pas::concat_ansi({"Bm.Captain.2Tranclucator", SysUtils::IntToStr(PortraitFaceId)}));
            }
            return u"Bm.Captain.2Tranclucator"_w;
        }
        if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
            if (pas::checked_cast<aKling::TKling*>(this)->KlingType == aGalaxyStruct::ktBoss) {
                if (pas::checked_cast<aKling::TKling*>(this)->DominatorSeries == aGalaxyStruct::dsBlazer) {
                    return u"Bm.Captain.2BlazerB"_w;
                } else if (pas::checked_cast<aKling::TKling*>(this)->DominatorSeries == aGalaxyStruct::dsKeller) {
                    return u"Bm.Captain.2KellerB"_w;
                } else if (pas::checked_cast<aKling::TKling*>(this)->DominatorSeries == aGalaxyStruct::dsTerron) {
                    return u"Bm.Captain.2TerronB"_w;
                } else {
                    return Result;
                }
            } else if (pas::checked_cast<aKling::TKling*>(this)->DominatorSeries == aGalaxyStruct::dsBlazer) {
        ...
        Owner = aConst::RaceToOwner(PilotRace);
        if (PortraitFaceId < 0) {
            if (TypeId == aGalaxyStruct::stRanger) {
                Role = u"R"_w;
            } else if (TypeId == aGalaxyStruct::stWarrior) {
                Role = u"W"_w;
            } else if (TypeId == aGalaxyStruct::stPirate) {
                Role = u"P"_w;
            } else if (TypeId == aGalaxyStruct::stTransport) {
                if (pas::checked_cast<aTransport::TTransport*>(this)->TransportType == aTransport::ttTransport) {
                    Role = u"T"_w;
                } else if (pas::checked_cast<aTransport::TTransport*>(this)->TransportType == aTransport::ttLiner) {
                    Role = u"L"_w;
                } else if (pas::checked_cast<aTransport::TTransport*>(this)->TransportType == aTransport::ttDiplomat) {
                    Role = u"D"_w;
                } else {
                    Role = u"L"_w;
                }
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(0, Limit - 1); cpp_range.next(I); ) {
                if (EC_Str::FindTextOffsetW(Styles->GetParamValue(I), Role, 0) >= 0) {
                    ...
                    if (aPlayer::GetPlayer() == nullptr || aPlayer::GetPlayer()->PortraitFaceId != Faces[CandidateCount] || aPlayer::GetPlayer()->PilotRace != PilotRace) {
                        ++CandidateCount;
                    }
                }
            }
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range_2.next(I); ) {
                ...
                for (auto cpp_range_3 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Ships) - 1); cpp_range_3.next(ShipIndex); ) {
                    ...
                    if (Ship->PilotRace != PilotRace || Ship->TypeId == aGalaxyStruct::stWarrior) {
                        continue;
                    }
        ...
        return pas::concat_wide({u"Bm.Captain.2", aConst::OwnerInfo[Owner].InternalName, pas::wide_int_to_str(PortraitFaceId)});
    }
    ...
    void TShip::RefreshGraphic() {
        std::uint8_t Dominator = !(pas::class_cast_if<aPlayer::TPlayer*>(this) != nullptr) && aGalaxy::Galaxy->GraphDominatorSurfacesEnabled || ChameleonActive;
        if (ScriptChameleon) {
            GraphDominator = Dominator;
            return;
        }
        if (Graphic == nullptr || Dominator != GraphDominator) {
            if (Dominator) {
    ...
    void TShip::TryRelocateUnseenShip() {
        ...
        if (aPlayer::GetPlayer()->ShouldKeepShipForQuests(this)) {
            return;
        }
        if (aPlayer::GetPlayer() == PartnerShip) {
            return;
        }
        if (ScriptShip != nullptr) {
            return;
        ...
        if (aMyFunction::NextRandomUnitFloat(RandomState) < 0.002L) {
            return;
        }
        if (DaysSincePlayerSeen < 20) {
            return;
        }
        if (([&] {
        ...
        }()) && pas::class_cast_if<aRanger::TRanger*>(this) != nullptr && static_cast<aRanger::TRanger*>(this)->PlaceInRating > 10) {
            return;
        }
        if (aPlayer::GetPlayer() == this) {
            return;
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range.next(I); ) {
            ...
            if (Candidate != CurrentStar && Candidate->Status.ControlFaction == aGalaxyStruct::sfCoalition && Candidate->Status.CustomFaction == u"" && Candidate->Status.Battle == 0 && aPlayer::GetPlayer()->CurrentStar != Candidate && Candidate->ShipTypeCounts[aGalaxyStruct::stRanger] < 3) {
                Distance = aMyFunction::PointDistanceSquared(CurrentStar->Position, Candidate->Position);
                if (Distance < BestDistance && (Destination == nullptr || aMyFunction::NextRandomIntRange(0, 1, RandomState) == 0)) {
                    BestDistance = Distance;
        ...
        if (Destination != nullptr) {
            ...
            if (Planet != nullptr) {
                ...
                GetHull()->HullPoints = GetHull()->Weight / 2;
                pas::list_add(aGalaxy::Galaxy->ShipsInTransit, reinterpret_cast<void*>(this));
                if (aGalaxy::Galaxy->AverageRangerCapital / 2 > Wealth) {
                    SetMoney(Money + (aGalaxy::Galaxy->AverageRangerCapital - Wealth) / 2);
                    CalculateWealth();
                }
            }
        }
    ...
    TShip* TShip::FindNearestDockableStation(TStationStandingMask StandingMask) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
            ...
            if (pas::in_range(Ship->TypeId, static_cast<std::int32_t>(aGalaxyStruct::rstRangerCenter), static_cast<std::int32_t>(aGalaxyStruct::rstCustomStation)) && (StandingMask == pas::constant_set<TStationStandingMask>({}) || pas::contains(StandingMask, Ship->CurrentStanding)) && Ship->CanDock(this)) {
    ...
    std::uint8_t TShip::TryMirrorPartnerTravelOrders() {
        ...
        if (PartnerShip->CurrentStar == CurrentStar) {
            if (PartnerShip->InNormalSpace()) {
                ...
                if (pas::class_cast_if<aPlanet::TPlanet*>(PartnerShip->OrderTarget) != nullptr && CanQueueReachablePlanet(pas::checked_cast<aPlanet::TPlanet*>(PartnerShip->OrderTarget)) && PartnerShip->EstimateOrderTravelTurns() < 5 && (static_cast<std::uint8_t>(OrderAbsolute ^ 1) || OrderTarget == PartnerShip)) {
                    OrderLanding(PartnerShip->OrderTarget, CanRefuel());
                    return true;
                }
                if (pas::class_cast_if<TShip*>(PartnerShip->OrderTarget) != nullptr && PartnerShip->Order == soLand && PartnerShip->EstimateOrderTravelTurns() < 5 && (static_cast<std::uint8_t>(OrderAbsolute ^ 1) || OrderTarget == PartnerShip) && reinterpret_cast<TShip*>(PartnerShip->OrderTarget)->CanDock(this)) {
                ...
                if (pas::class_cast_if<aGalaxy::TStar*>(PartnerShip->OrderTarget) != nullptr && CanRefuel()) {
                    if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr) {
                        ...
                        pas::checked_cast<aRanger::TRanger*>(this)->SelectNearestReachableDestination();
                    }
                    return false;
                }
                ...
                if (CanQueueReachablePlanet(PartnerShip->CurrentPlanet)) {
                    OrderLanding(PartnerShip->CurrentPlanet, true);
                    return true;
                }
            } else if (PartnerShip->DockedTo != nullptr && pas::class_cast_if<aRuins::TRuins*>(PartnerShip->DockedTo) != nullptr && pas::checked_cast<aRuins::TRuins*>(PartnerShip->DockedTo)->CanDock(this)) {
        ...
        double Distance = aMyFunction::PointDistance(Position, Pursuer->Position);
        {
            ...
            if (1 <= cpp_last) {
                for (I = 1; I <= cpp_last; ++I) {
                    ...
                    if (Pursuer->IsEquipmentUsable(Weapon) && static_cast<long double>(GetWeaponRange(Weapon)) > Distance) {
                        return false;
                    }
                }
            }
    ...
    std::uint8_t TShip::HasLandablePlanetInStar(aGalaxy::TStar* Star) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Star->Planets) - 1); cpp_range.next(I); ) {
            ...
            if (Planet->OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiUninhabited) && CanQueueReachablePlanet(Planet)) {
                return true;
            }
        }
        return false;
    ...
    aGalaxy::TStar* TShip::FindNextStarTowardDestination(aGalaxy::TStar* Destination, std::uint8_t RequireFuelMargin) {
        ...
        static const pas::Set<0, 255> CoalitionShips = pas::constant_set<pas::Set<0, 255>>({{1, 5}});
        std::int32_t I{};
        std::int32_t J{};
        std::int32_t K{};
    ...
    void TShip::NotifyCompanionDeath() {
        pas::WideString Text{};
        if (pas::class_cast_if<aRanger::TRanger*>(this) != nullptr) {
            Text = aConst::PickLocalizedTextVariant(u"GalaxyNews.DeadShip.Partner"_wref.get(), Seed + aGalaxy::Galaxy->CurrentTurn / 11);
        } else if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr) {
            Text = aConst::PickLocalizedTextVariant(u"GalaxyNews.DeadShip.Pirate"_wref.get(), Seed + aGalaxy::Galaxy->CurrentTurn / 11);
        } else if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) {
            Text = aConst::PickLocalizedTextVariant(u"GalaxyNews.DeadShip.Tranclucator"_wref.get(), Seed + aGalaxy::Galaxy->CurrentTurn / 11);
        } else {
            return;
        }
        if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) {
    ...
    std::int32_t TShip::ApplyDamage(pas::Object* Source, std::int32_t Damage, float HitRange, std::uint32_t& DamageColor, aGalaxyStruct::TDamageFlagSet DamageFlags) {
        std::int32_t Result{};
        static const pas::Set<0, 255> ScannerDamageFlags = pas::constant_set<pas::Set<0, 255>>({{13, 19}});
        ...
        DamageColor = 0u;
        if (TypeId == aGalaxyStruct::stKling && Attacker != nullptr && Attacker->TypeId == aGalaxyStruct::stKling && ([&] {
            aGalaxyStruct::TDominatorSeries cpp_left = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
            return cpp_left == pas::checked_cast<aKling::TKling*>(Attacker)->DominatorSeries;
        }()) && static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(Attacker)->IsProgramActive(aGalaxyStruct::prgInsanity) ^ 1) && HasIndependentScriptFaction() == Attacker->HasIndependentScriptFaction() && (static_cast<std::uint8_t>(HasIndependentScriptFaction() ^ 1) || reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText == reinterpret_cast<aScript::TScriptShip*>(Attacker->ScriptShip)->StateText)) {
            return 0;
        }
        if (HitRange != -1.0L && (pas::in_set<0, 4, 7, 7>(OwnerId) || TypeId == aGalaxyStruct::stTranclucator) && Attacker != nullptr && GetRelationLevelToShip(Attacker) > aGalaxyStruct::rlHostile) {
            return 0;
        }
        if (HitRange != -1.0L && TypeId == aGalaxyStruct::stKling && aPlayer::GetPlayer() == Attacker && pas::checked_cast<aKling::TKling*>(this)->IsPlayerCamouflageEffective(Attacker)) {
            return 0;
        }
        if (HitRange != -1.0L && Attacker != nullptr && Attacker->TypeId == aGalaxyStruct::stKling && aPlayer::GetPlayer() == this && pas::checked_cast<aKling::TKling*>(Attacker)->IsPlayerCamouflageEffective(this)) {
            return 0;
        }
        if (Attacker != nullptr && this != Attacker && (!(pas::class_cast_if<aMissile::TMissile*>(Source) != nullptr) || HitRange == -1.0L || GetRelationLevelToShip(Attacker) <= aGalaxyStruct::rlHostile)) {
            if (aPlayer::GetPlayer() == this && Attacker->ScriptShip != nullptr) {
            ...
            if (ScriptShip != nullptr) {
                if (aPlayer::GetPlayer() == Attacker || aPlayer::GetPlayer() == Attacker->PartnerShip || pas::class_cast_if<aTranclucator::TTranclucator*>(Attacker) != nullptr && static_cast<aTranclucator::TTranclucator*>(Attacker)->OwnerShip == aPlayer::GetPlayer()) {
                    reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->Hit = true;
                }
            }
        }
        ...
        if (ScannerEffects) {
            ...
            if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkBonusToDamaged))) != 0) {
                DamageValue = (1.0L + (1.0L - pas::real_divide(GetHull()->HullPoints, GetHull()->Weight)) * 0.33L) * DamageValue;
            }
        }
        if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkEnergy))) != 0) {
            for (auto cpp_range = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtEnergyDef)); cpp_range.next(J); ) {
                pas::Extended cpp_right = 0.3L * ((CanBoostArtefact(aConst::t_ArtEnergyDef, nullptr, false) & 127 & 0x0000007f) + 1);
                if (aMyFunction::NextRandomUnitFloat(RandomState) < cpp_right) {
                    DamageValue = 0.0f;
                    break;
                }
            }
        ...
        if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkMissile))) != 0) {
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtMissileDef)); cpp_range_2.next(J); ) {
                pas::Extended cpp_right_2 = (CanBoostArtefact(aConst::t_ArtMissileDef, nullptr, false) & 127 & 0x0000007f) + 1;
                DamageValue = pas::real_divide(DamageValue, 1.0L + aMyFunction::NextRandomFloatRange(0.1, 0.4, RandomState) * cpp_right_2);
            }
        }
        if (Attacker != nullptr && aPlayer::GetPlayer() == Attacker) {
            if (Attacker->IsHealthEffectActive(10) || Attacker->IsHealthEffectActive(7) || Attacker->IsHealthEffectActive(8)) {
                DamageValue = static_cast<long double>(DamageValue) * aMyFunction::NextRandomUnitFloat(RandomState);
            }
            if (TypeId == aGalaxyStruct::stKling) {
                pas::checked_cast<aKling::TKling*>(this)->DetectAttackingPlayer(Attacker);
            }
        }
        if (DamageValue < 1.0L) {
        ...
        if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && DamageValue > 0.0L && static_cast<aKling::TKling*>(this)->DominatorSeries == aGalaxyStruct::dsKeller && static_cast<aKling::TKling*>(this)->HasNearbyBertorAura()) {
            if (CurrentStar->RecordingTurnFilm && static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(this)->AuraEffectShownThisTurn ^ 1)) {
                ...
                Globals::PrimaryFilm->SetWeaponEndpoints(CurrentStar->CurrentStepIndex, Effect, FilmObject, FilmObject);
                Globals::PrimaryFilm->SetWeaponHit(CurrentStar->CurrentStepIndex, Effect, 0, 0, false, true);
                Globals::PrimaryFilm->AttachObject(CurrentStar->CurrentStepIndex, Effect);
                pas::checked_cast<aKling::TKling*>(this)->AuraEffectShownThisTurn = true;
            }
            DamageValue = DamageValue * 0.8L;
        }
        DamageValue = static_cast<long double>(DamageValue) * GetHull()->GetFragilityFactor(DamageFlags);
        float StatusStrength = GetCombatStatusStrength(cseBWBuff);
        ...
        if (TypeId == aGalaxyStruct::stKling) {
            if (pas::checked_cast<aKling::TKling*>(this)->KlingType == aGalaxyStruct::ktBoss) {
                if (aPlayer::GetPlayer()->CurrentStar != CurrentStar || static_cast<std::uint8_t>(aPlayer::GetPlayer()->InNormalSpace() ^ 1)) {
                    ...
                    if (System::Round(DamageValue) >= GetHull()->HullPoints) {
                        DamageValue = GetHull()->HullPoints - 1;
                    }
                }
                if (System::Round(DamageValue) >= GetHull()->HullPoints && aKling::KellerShip == this) {
                    DamageValue = GetHull()->HullPoints - 1;
                }
            }
        ...
        if (aPlayer::GetPlayer() == this && (aGalaxy::Galaxy->GodModEnabled == 1 || aGalaxy::Galaxy->SpecialSimulationMode != 0)) {
            AdjustedDamage = 0;
        }
        if (Order == soTeleport && OrderTarget != nullptr && OrderTarget != CurrentStar) {
            AdjustedDamage = 0;
        }
        if (GetHull()->ImpulseShieldsEnabled && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkUndefendable))) == 0 && ([&] {
            pas::Extended cpp_left_7 = aMyFunction::NextRandomIntRange(0, 100, RandomState);
            return cpp_left_7 > GetDefenseDamageFactor() * 1.0E+2L;
        }())) {
        ...
        if (Attacker != nullptr) {
            AdjustedDamage = Attacker->ScriptItemsAct(aConst::satOnDealingDamage, this, nullptr, AdjustedDamage);
        }
        AdjustedDamage = ScriptItemsAct(DamageScriptActionTypes[static_cast<std::int32_t>(aConst::ClassifyWeaponDamageFlags(pas::load_unaligned<std::uint32_t>(&DamageFlags))) & 0x0000007f], Source, nullptr, AdjustedDamage);
        if (AdjustedDamage <= 0) {
            Result = 0;
            DamageColor = 0u;
            return Result;
        ...
        if (GetHull()->HullPoints - AdjustedDamage <= 0 && aKling::KellerShip != this && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkNonLethal))) == 0) {
            if (Attacker != nullptr) {
                Attacker->ScriptItemsAct(aConst::satOnDealingFatalDamage, this, nullptr, 0);
                if (pas::class_cast_if<aNormalShip::TNormalShip*>(Attacker) != nullptr) {
                    pas::checked_cast<aNormalShip::TNormalShip*>(Attacker)->ProcessShipKill(this);
                }
                if (Attacker->TypeId == aGalaxyStruct::stRanger) {
                    pas::checked_cast<aRanger::TRanger*>(Attacker)->ApplyAttackReputationChanges(this, 1.0);
                }
                if (Attacker->PartnerShip != nullptr && Attacker->PartnerShip->TypeId == aGalaxyStruct::stRanger) {
                    pas::checked_cast<aRanger::TRanger*>(Attacker->PartnerShip)->ApplyAttackReputationChanges(this, 0.5);
                }
                if (pas::class_cast_if<aTranclucator::TTranclucator*>(Attacker) != nullptr && reinterpret_cast<aTranclucator::TTranclucator*>(Attacker)->OwnerShip != nullptr && reinterpret_cast<aTranclucator::TTranclucator*>(Attacker)->OwnerShip->TypeId == aGalaxyStruct::stRanger) {
                    pas::checked_cast<aRanger::TRanger*>(pas::checked_cast<aTranclucator::TTranclucator*>(Attacker)->OwnerShip)->ApplyAttackReputationChanges(this, 1.0);
                    if (pas::checked_cast<aTranclucator::TTranclucator*>(Attacker)->OwnerShip == aPlayer::GetPlayer()) {
                        ...
                        Event->AddTextData(GetFullName(u" "_wref.get()));
                        Event->AddTextData(TypeNameOverrideKey);
                        if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
                            Event->AddData(static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(this)->KlingType));
                        } else if (pas::class_cast_if<aTransport::TTransport*>(this) != nullptr) {
                            Event->AddData(static_cast<std::uint8_t>(pas::checked_cast<aTransport::TTransport*>(this)->TransportType));
                        } else if (pas::class_cast_if<aWarrior::TWarrior*>(this) != nullptr) {
                            Event->AddData(pas::checked_cast<aWarrior::TWarrior*>(this)->WarriorType);
                        } else if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr) {
                            Event->AddData(pas::checked_cast<aPirate::TPirate*>(this)->PirateType);
                        } else {
                            Event->AddData(0);
                        }
                    }
            ...
            if (Source == nullptr) {
                ScriptItemsAct(aConst::satOnDeath, nullptr, nullptr, 0);
            } else if (pas::class_cast_if<aMissile::TMissile*>(Source) != nullptr) {
                ScriptItemsAct(aConst::satOnDeath, reinterpret_cast<aMissile::TMissile*>(Source)->OwnerShip, Source, 0);
            } else if (pas::class_cast_if<TShip*>(Source) != nullptr) {
                ScriptItemsAct(aConst::satOnDeath, Source, nullptr, 0);
            } else {
                ScriptItemsAct(aConst::satOnDeath, nullptr, nullptr, 0);
            }
            ...
            aPlayer::GetPlayer()->ProcessShipDestructionQuests(this);
            if (aMyFunction::NextRandomUnitFloat(RandomState) < 0.5L && static_cast<std::uint8_t>(HasScriptStateText() ^ 1)) {
                Cistern = pas::construct_call<aItem::TCistern>(aItem::TEquipment_Create);
                I = aMyFunction::NextRandomIntRange(10, System::Round(aMyFunction::RemapClamped(GetHull()->Weight, 2.0E+2, 2.0E+3, 1.0E+1, 1.5E+2)), RandomState);
                {
                    std::uint8_t ownerId = OwnerId;
                    std::int32_t nextRandomIntRange = aMyFunction::NextRandomIntRange(1, I, RandomState);
                ...
                if (OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiDominator)) {
                    Cistern->DominatorSeries = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
                }
                pas::list_add(Inventory, reinterpret_cast<void*>(Cistern));
                DropCarriedItemAsMovingLoot(Cistern);
            }
            if (pas::class_cast_if<aRuins::TRuins*>(this) != nullptr) {
                JettisonCargoGoodsTowardTargetValue(aGalaxy::Galaxy->ComputeScaledHugeMoney(2));
            } else if (CurrentStar->Items == nullptr || pas::list_count(CurrentStar->Items) < 20 || OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiDominator) || HasIndependentScriptFaction()) {
                DropRoll = aMyFunction::NextRandomUnitFloat(RandomState);
                if (DropRoll < 0.1L && (!(pas::class_cast_if<aKling::TKling*>(this) != nullptr) || Attacker == nullptr || !(pas::class_cast_if<aKling::TKling*>(Attacker) != nullptr) || DropRoll < pas::real_divide(0.1L, aConst::GalaxyDifficultyTuning[aGalaxy::Galaxy->DifficultyLevels[7]].QuestTimeAndExperienceFactor))) {
                    ...
                    if (Attacker != nullptr && Attacker->CountActiveArtefacts(aConst::t_ArtefactMiniExpl) > 0) {
                        if (aMyFunction::NextRandomUnitFloat(RandomState) > 0.6L) {
                            ...
                            return aMyFunction::NextRandomUnitFloat(RandomState) > cpp_right_3;
                        }()) || ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkMoreDrop))) != 0 && aMyFunction::NextRandomUnitFloat(RandomState) > 0.9L) {
                            DropRandomValuableItemsOnDestruction(1);
                        }
                    }
                    if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkMoreDrop))) != 0 && aMyFunction::NextRandomUnitFloat(RandomState) > 0.6L) {
                        ++DropCount;
                    }
                    DropRandomCheapItemsOnDestruction(DropCount);
                }
                if (OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiPirate) && TypeId == aGalaxyStruct::stPirate && (RandomState + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn)) % 31 == 0) {
                    Module = pas::construct_call<aItem::TMicroModule>(aItem::TEquipment_Create);
                    MinimumPriority = System::Round(aMyFunction::RemapClamped(WealthInBestRanger, 0.5, 2.0, 1.0E+1, 0.0));
                    ...
                    MinimumPriority += aMyFunction::SeededRandomIntRange(-10, 10, Id + System::Trunc(static_cast<std::int32_t>(aGalaxy::Galaxy->GenerationSeed)));
                    MinimumPriority = std::max<std::int32_t>(1, std::min<std::int32_t>(MinimumPriority, 100));
                    MaximumPriority = 100;
                    Module->Init(aGalaxy::TGalaxy::SelectMicroModule(MinimumPriority, MaximumPriority, Id + System::Trunc(static_cast<std::int32_t>(aGalaxy::Galaxy->GenerationSeed)), this));
                    Module->DominatorSeries = aGalaxyStruct::dsBlazer;
                    Module->OwnerId = static_cast<std::uint8_t>(aGalaxyStruct::oiPirate);
                    pas::list_add(Inventory, reinterpret_cast<void*>(Module));
                    DropCarriedItemAsMovingLoot(Module);
                }
                if (OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiDominator)) {
                    if ((RandomState + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn)) % 2 == 0 || pas::checked_cast<aKling::TKling*>(this)->KlingType == aGalaxyStruct::ktBoss) {
                        Nodes = pas::construct_call<aItem::TProtoplasm>(aItem::TEquipment_Create);
                        Nodes->Init_2(NodeReserve + 1, 1);
                        Nodes->DominatorSeries = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
                        pas::list_add(Inventory, reinterpret_cast<void*>(Nodes));
                        DropCarriedItemAsMovingLoot(Nodes);
                    } else {
                        Debris = pas::construct_call<aItem::TUselessItem>(aItem::TUselessItem_Create);
                        Debris->Init(u"Remains"_w, pas::checked_cast<aKling::TKling*>(this)->DominatorSeries, Seed + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn / 10), false);
                        pas::list_add(Inventory, reinterpret_cast<void*>(Debris));
                        DropCarriedItemAsMovingLoot(Debris);
                    }
                    if ((RandomState + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn)) % 47 == 0) {
                        Module = pas::construct_call<aItem::TMicroModule>(aItem::TEquipment_Create);
                        MinimumPriority = System::Round(aMyFunction::RemapClamped(WealthInBestRanger, 0.5, 2.0, 1.0E+1, 0.0));
                        MinimumPriority += System::Round(aMyFunction::RemapClamped(aPlayer::GetPlayer()->PlaceInRating, 1.0, pas::list_count(aGalaxy::Galaxy->Rangers), 0.0, 1.0E+1));
                        ...
                        MaximumPriority = 100;
                        Module->Init(aGalaxy::TGalaxy::SelectMicroModule(MinimumPriority, MaximumPriority, Id + System::Trunc(static_cast<std::int32_t>(aGalaxy::Galaxy->GenerationSeed)), this));
                        Module->DominatorSeries = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
                        Module->OwnerId = static_cast<std::uint8_t>(aGalaxyStruct::oiDominator);
                        pas::list_add(Inventory, reinterpret_cast<void*>(Module));
                        DropCarriedItemAsMovingLoot(Module);
                    }
                } else if (pas::in_range(TypeId, aGalaxyStruct::stRanger, aGalaxyStruct::stPirate)) {
                    JettisonCargoGoodsTowardTargetValue(aGalaxy::Galaxy->ComputeScaledAverageMoney(OwnerId));
                }
            }
            ...
            GetHull()->HullPoints -= AdjustedDamage;
            if (GetHull()->HullPoints != ExpectedHullPoints - AdjustedDamage) {
                GR_Main::CCInterface->SetTamperDetected(true);
            }
            if ((aKling::KellerShip == this || (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkNonLethal))) != 0) && GetHull()->HullPoints < 1) {
                GetHull()->HullPoints = 1;
            }
            ExpectedHullPoints = GetHull()->HullPoints;
            if (AdjustedDamage == 1 && aPlayer::GetPlayer() == Attacker && Attacker != nullptr) {
                ++PlayerScratchHitsReceived;
                static_cast<void>(aPlayer::GetPlayer()->AchievementStats), Achievements::TAchievementStats::CheckScratchDamageAchievement(PlayerScratchHitsReceived);
            }
            EngineLoss = aMyFunction::NextRandomIntRange(0, 2, RandomState);
            if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkDecelerate))) != 0) {
                EngineLoss += aMyFunction::NextRandomIntRange(5, 15, RandomState);
            }
            ...
            if (EngineLoss > 0 && GetEngine() != nullptr) {
                if (GetEngine()->OutputPercent > EngineLoss) {
                    GetEngine()->OutputPercent -= EngineLoss;
                } else {
                    GetEngine()->OutputPercent = 0;
                }
            }
            if (TypeId != aGalaxyStruct::stTranclucator) {
                if (OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiDominator)) {
                    Wear = aMyFunction::RemapClamped(AdjustedDamage, 1.0, GetHull()->Weight * 0.1L, 0.05, 0.15);
                    if (pas::class_cast_if<aRanger::TRanger*>(this) != nullptr) {
                        if (aPlayer::GetPlayer() == this) {
                            Wear = Wear;
                        } else {
                        ...
                        Wear = Wear * 0.5L;
                        if (pas::checked_cast<aWarrior::TWarrior*>(this)->WarriorType == aWarrior::wtFlagship) {
                            Wear = Wear * 0.6L;
                        }
                    }
                    if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkReduceEngine))) != 0 && GetEngine() != nullptr && GetEngine()->BrokenFlag == 0) {
                        ApplyCombatItemDegradation(0.5L * Wear);
                        ApplyItemDegradation(GetEngine(), idkBattle, 15.0L * Wear);
                    } else {
                    ...
                    if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkDestruct))) != 0) {
                        Wear = aMyFunction::NextRandomFloatRange(5.0, 15.0, RandomState);
                ...
                } else if (aKling::TerronShip != this) {
                    if ((RandomState + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn)) % 4 == 0 && pas::list_count(CurrentStar->Items) < 25) {
                        Nodes = pas::construct_call<aItem::TProtoplasm>(aItem::TEquipment_Create);
                        Nodes->Init_2(aMyFunction::NextRandomIntRange(NodeReserve / 8, NodeReserve / 4, RandomState) + 1, 1);
                        Nodes->DominatorSeries = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
                        pas::list_add(Inventory, reinterpret_cast<void*>(Nodes));
                        DropCarriedItemAsMovingLoot(Nodes);
                    }
                    if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkDropCargo))) != 0 && aMyFunction::NextRandomUnitFloat(RandomState) > 0.95L) {
                        Nodes = pas::construct_call<aItem::TProtoplasm>(aItem::TEquipment_Create);
                        Nodes->Init_2(aMyFunction::NextRandomIntRange(NodeReserve / 4, NodeReserve / 2, RandomState) + 1, 1);
                        Nodes->DominatorSeries = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
                        pas::list_add(Inventory, reinterpret_cast<void*>(Nodes));
                        DropCarriedItemAsMovingLoot(Nodes);
                    }
            ...
            if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkDropCargo))) != 0 && aMyFunction::NextRandomUnitFloat(RandomState) > 0.95L) {
                JettisonCargoGoodsTowardTargetValue(aGalaxy::Galaxy->ComputeScaledMiniMoney(2));
            }
            if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkBlockWeapon))) != 0) {
                AddCombatStatusStrength(cseWeaponBlock, 0.1f, Attacker);
            }
            if (ScannerEffects && (pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkDroidBlock))) != 0) {
                AddCombatStatusStrength(cseDroidBlock, 0.1f, Attacker);
            }
            if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkShock))) != 0) {
                if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
                    AddCombatStatusStrength(cseShock, AdjustedDamage * 0.12L, Attacker);
                } else {
                    AddCombatStatusStrength(cseShock, AdjustedDamage * 0.2L, Attacker);
                }
            }
            if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkAcid))) != 0 && static_cast<long double>(GetHull()->Armor) > GetCombatStatusStrength(cseAcid)) {
                if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
                    AddCombatStatusStrength(cseAcid, 0.66f, Attacker);
                } else {
                    AddCombatStatusStrength(cseAcid, 1.0f, Attacker);
                }
            }
            if ((pas::load_unaligned<std::uint32_t>(&DamageFlags) & pas::shl(1, static_cast<std::int32_t>(aGalaxyStruct::dkMagnetic))) != 0) {
                if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
                    AddCombatStatusStrength(cseMagnetic, 0.07L + std::max<std::int32_t>(0, Damage + GetArmor()) * 0.075L, Attacker);
                } else {
                    AddCombatStatusStrength(cseMagnetic, 0.1L + std::max<std::int32_t>(0, Damage + GetArmor()) * 0.05L, Attacker);
                }
            }
        }
        if (aPlayer::GetPlayer() == this) {
        ...
        if (aPlayer::GetPlayer() == this && aMyFunction::RandomIntRange(0, 100) == 0) {
            SysUtilsImports::Sleep(1u);
        }
        if (GetHull()->HullPoints != ExpectedHullPoints && static_cast<std::uint8_t>(GR_Main::CCInterface->GetTamperDetected() ^ 1)) {
            GR_Main::CCInterface->SetTamperDetected(true);
        }
        if (IsHullDestroyed()) {
    ...
    std::int32_t TShip::ApplyWeaponHit(TShip* Source, aItem::TWeapon* Weapon, float HitRange, std::uint32_t& DamageColor, pas::Var<std::uint32_t> DamageFlags, float DamageScale, std::int32_t FixedDamage) {
        static const pas::Set<0, 255> EnergyDamageFlags = pas::constant_set<pas::Set<0, 255>>({{0}});
        std::int32_t I{};
        ...
        if (FixedDamage > 0 && static_cast<std::uint8_t>(pas::contains(Flags, aGalaxyStruct::dkUndefendable) ^ 1)) {
        ...
        } else {
            {
                ...
                SkillDifference = cpp_left - (GetEffectiveSkillLevel(psManeuverability, false) & 0x0000007f);
            }
            ...
            RolledDamage = aMyFunction::NextRandomIntRange(std::max<std::int32_t>(MinDamage, RolledDamage - Spread), std::min<std::int32_t>(MaxDamage, RolledDamage + Spread), RandomState);
            ...
            if (pas::class_cast_if<aKling::TKling*>(Source) != nullptr && static_cast<aKling::TKling*>(Source)->DominatorSeries == aGalaxyStruct::dsBlazer) {
                if (pas::checked_cast<aKling::TKling*>(Source)->HasNearbyBertorAura()) {
                    if (CurrentStar->RecordingTurnFilm && static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(Source)->AuraEffectShownThisTurn ^ 1)) {
                        ...
                        Globals::PrimaryFilm->SetWeaponEndpoints(CurrentStar->CurrentStepIndex, Film, Source->FilmObject, Source->FilmObject);
                        Globals::PrimaryFilm->SetWeaponHit(CurrentStar->CurrentStepIndex, Film, 0, 0, false, true);
                        Globals::PrimaryFilm->AttachObject(CurrentStar->CurrentStepIndex, Film);
                        pas::checked_cast<aKling::TKling*>(Source)->AuraEffectShownThisTurn = true;
                    }
                    RolledDamage = System::Round(RolledDamage * 1.25L);
                }
            }
            RolledDamage = Source->ScriptItemsAct(aConst::satOnWeaponShot, this, Weapon, RolledDamage);
            if (pas::contains(Flags, aGalaxyStruct::dkUndefendable)) {
                AdjustedDamage = RolledDamage;
            } else if (!GetHull()->ImpulseShieldsEnabled) {
                pas::Extended cpp_left_2 = static_cast<long double>(GetDefenseDamageFactor()) * RolledDamage;
                AdjustedDamage = cpp_left_2 - GetArmor();
            } else {
                pas::Extended cpp_left_3 = (GetDefenseDamageFactor() * 0.5L + 0.5L) * RolledDamage;
                AdjustedDamage = cpp_left_3 - GetArmor();
            }
        }
        ...
        ScriptFlags = Source->ScriptItemsAct(aConst::satOnWeaponShot2, this, Weapon, ScriptFlags);
        ScriptFlags = ScriptItemsAct(aConst::satOnGettingWeaponHit, Source, Weapon, ScriptFlags);
        ...
        std::int32_t Result = ApplyDamage(Source, Damage, HitRange, DamageColor, Flags);
        if (GetHull()->HullPoints < 1 && aPlayer::GetPlayer() == Source) {
            if ((Weapon->GetWeaponInfo()->DamageFlags & 2) != 0) {
    ...
    std::int32_t TShip::ApplyMissileHit(pas::Object* Missile, std::uint32_t& DamageColor, pas::Var<std::uint32_t> DamageFlags) {
        std::int32_t SkillDifference{};
        float AdjustedDamage{};
        aGalaxyEvent::TGalaxyEvent* Event{};
        aGalaxyStruct::TDamageFlagSet Flags{};
        ...
        if (Shot->OwnerShip != nullptr) {
            ScriptFlags = Shot->OwnerShip->ScriptItemsAct(aConst::satOnMissileShot2, this, Missile, ScriptFlags);
        }
        ScriptFlags = ScriptItemsAct(aConst::satOnGettingMissileHit, Shot->OwnerShip, Missile, ScriptFlags);
        Flags = pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&ScriptFlags);
        pas::store_unaligned<std::uint32_t>(DamageFlags.address, pas::load_unaligned<std::uint32_t>(&Flags));
        if (Shot->OwnerShip == nullptr) {
            SkillDifference = 0 - (GetEffectiveSkillLevel(psManeuverability, false) & 0x0000007f);
        } else {
        ...
        if (pas::contains(Flags, aGalaxyStruct::dkUndefendable)) {
            ...
            if (!GetHull()->ImpulseShieldsEnabled) {
                AdjustedDamage = static_cast<long double>(GetDefenseDamageFactor()) * RolledDamage;
            } else {
                AdjustedDamage = (GetDefenseDamageFactor() * 0.5L + 0.5L) * RolledDamage;
            }
            AdjustedDamage = static_cast<long double>(AdjustedDamage) - GetArmor();
        }
        if (Shot->Target == this) {
            HitRange = -1.0f;
        } else {
        ...
        std::int32_t Damage = System::Round(AdjustedDamage);
        std::int32_t Result = ApplyDamage(Shot, Damage, HitRange, DamageColor, pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(DamageFlags.address));
        if (GetHull()->HullPoints < 1 && Shot->OwnerShip != nullptr && aPlayer::GetPlayer() == Shot->OwnerShip) {
    ...
    void TShip::FireWeaponAtMissile(aItem::TWeapon* Weapon, pas::Object* Target, std::uint8_t RecordFilm) {
        aGalaxy::TStar* Star{};
        ...
        if (pas::class_cast_if<aMissile::TMissile*>(Target) != nullptr) {
            ...
            Hit = Shot->CanBeHit(this, Weapon);
            Hit = ScriptItemsAct(aConst::satOnWeaponShot, Shot, Weapon, static_cast<std::uint8_t>(Hit)) != 0;
            ...
            if (RecordFilm) {
                ...
                Film = Globals::PrimaryFilm->AddObject(0u, Effect, 0, 0);
                Globals::PrimaryFilm->SetWeaponEndpoints(StepIndex, Film, FilmObject, Shot->FilmObject);
            ...
            if ((Info->DamageFlags & 1) != 0 && Info->ShotType == aGalaxyStruct::wstSplash) {
                ...
                while (I < pas::list_count(Star->Missiles)) {
                    ...
                    OtherHit = Other->CanBeHit(this, Weapon);
                    OtherHit = ScriptItemsAct(aConst::satOnWeaponShot, Other, Weapon, static_cast<std::uint8_t>(OtherHit)) != 0;
                    if (OtherHit) {
                        if (RecordFilm) {
                            Effect = pas::construct_call<SE_Weapon::TWeaponSE>(SE_Weapon::TWeaponSE_Create, u"Weapon.Asteroid"_wref.get(), ClassesImports::Point(0, 0), 0, -1);
                            Film = Globals::PrimaryFilm->AddObject(0u, Effect, 0, 0);
                ...
                while (I < pas::list_count(Star->Missiles)) {
                    ...
                    {
                        pas::Extended cpp_right = pas::sqr(GetWeaponRange(Weapon)) * 1.3L;
                        if (aMyFunction::PointDistanceSquared(Position, Other->Position) > cpp_right) {
                            continue;
                        }
                    }
                    if (TypeId == aGalaxyStruct::stKling && Other->OwnerShip != nullptr && Other->OwnerShip->TypeId == aGalaxyStruct::stKling && ([&] {
                        aGalaxyStruct::TDominatorSeries cpp_left = pas::checked_cast<aKling::TKling*>(this)->DominatorSeries;
    ...
    void TShip::FireWeaponAtShip(aItem::TWeapon* Weapon, TShip* Target, std::uint8_t RecordFilm) {
        std::uint32_t Color{};
        aGalaxyStruct::TDamageFlagSet Flags{};
        std::int32_t Count{};
        ...
        if (aPlayer::GetPlayer() == Target || aPlayer::GetPlayer() == this) {
            Star->PlayerCombatOccurred = true;
        }
        if (RecordFilm & (aPlayer::GetPlayer() == Target)) {
            Globals::PrimaryFilm->AddCameraEvent(StepIndex, aPlayer::GetPlayer()->Position, Position, 1);
        }
        std::int32_t PrimaryDamage = 0;
        std::int32_t DrainedDamage = 0;
        Info = Weapon->GetWeaponInfo();
        if (Info->ShotType == aGalaxyStruct::wstNormal) {
            Damage = Target->ApplyWeaponHit(this, Weapon, -1.0f, Color, pas::Var<std::uint32_t>(&Flags), 1.0f, 0);
            ...
            if (RecordFilm & (aPlayer::GetPlayer() == this) && Star->PlayerFilmPath != nullptr) {
                float cpp_arg = pas::real_divide(static_cast<long double>(Target->Position.X) + Position.X, 2.0L);
                float cpp_arg_2 = pas::real_divide(static_cast<long double>(Target->Position.Y) + Position.Y, 2.0L);
                Star->PlayerFilmPath->AppendWaypoint(EC_Struct::MakePointF(cpp_arg, cpp_arg_2), StepIndex + 25);
            }
            RadiusSquared = pas::sqr(GetWeaponRange(Weapon)) * 1.3L;
            Damage = Target->ApplyWeaponHit(this, Weapon, -1.0f, Color, pas::Var<std::uint32_t>(&Flags), 1.0f, 0);
            if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                DrainedDamage += Damage;
            }
            ...
            pas::list_add(Ships, reinterpret_cast<void*>(this));
            ...
            if (!Target->IsHullDestroyed()) {
                ...
                for (auto cpp_range = pas::for_to<std::int32_t>(2, Weapon->GetShotCount()); cpp_range.next(ShotIndex); ) {
                    ...
                    for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, Count - 1); cpp_range_2.next(I); ) {
                        ...
                        if (Ship->InNormalSpace() && static_cast<std::uint8_t>(Ship->IsHullDestroyed() ^ 1) && pas::list_indexof(Ships, reinterpret_cast<void*>(Ship)) < 0 && GetRelationLevelToShip(Ship) <= aGalaxyStruct::rlHostile) {
                            if (!(pas::class_cast_if<aKling::TKling*>(this) != nullptr) || static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(this)->IsPlayerCamouflageEffective(Ship) ^ 1)) {
                                if (!(pas::class_cast_if<aKling::TKling*>(Ship) != nullptr) || static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(Ship)->IsPlayerCamouflageEffective(this) ^ 1)) {
                    ...
                    if (Nearest == nullptr) {
                        ...
                        Damage = Nearest->ApplyWeaponHit(this, Weapon, -1.0f, Color, pas::Var<std::uint32_t>(&Flags), DamageScale, 0);
                    ...
                    Damage = Nearest->ApplyWeaponHit(this, Weapon, 0.0f, Color, pas::Var<std::uint32_t>(&Flags), DamageScale, 0);
                    if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                        DrainedDamage += Damage;
                    }
            ...
            if (RecordFilm & (aPlayer::GetPlayer() == this) && Star->PlayerFilmPath != nullptr) {
                float cpp_arg_5 = pas::real_divide(static_cast<long double>(Target->Position.X) + Position.X, 2.0L);
                float cpp_arg_6 = pas::real_divide(static_cast<long double>(Target->Position.Y) + Position.Y, 2.0L);
            ...
            for (auto cpp_range_4 = pas::for_to<std::int32_t>(0, Count - 1); cpp_range_4.next(I); ) {
                Ship = pas::list_at<TShip>(Star->Ships, I);
                if (Ship != this && Ship != Target && Ship->InNormalSpace() && static_cast<std::uint8_t>(Ship->IsHullDestroyed() ^ 1)) {
                    ...
                    if (pas::sqr(static_cast<pas::Extended>(Info->SecondaryDamageRadius)) >= DistanceSquared) {
                        Damage = Ship->ApplyWeaponHit(this, Weapon, System::Sqrt(DistanceSquared), Color, pas::Var<std::uint32_t>(&Flags), 1.0f, 0);
                        if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                            DrainedDamage += Damage;
                        }
                        if (RecordFilm) {
            ...
            if (Target->IsHullDestroyed()) {
                ...
                Colors = pas::make_object<pas::List>();
                Films = pas::make_object<pas::List>();
                Damage = aShip::ApplyChainExplosion(Target, this, Weapon, RecordFilm, Color, Flags, Count, Star, DistanceSquared, Info, Damage, DrainedDamage, Effect, Film, StepIndex, Ships, Damages, Colors, Films, GraphKey);
                if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                    DrainedDamage += Damage;
                }
                if (RecordFilm) {
                    aShip::FinishChainExplosionFilm(StepIndex, Ships, Damages, Colors, Films);
                }
                pas::list_clear(Ships);
                pas::free(Ships);
                Ships = nullptr;
            ...
            if (RecordFilm) {
                ...
                if (GraphKey != u"") {
                    Effect = pas::construct_call<SE_Weapon::TWeaponSE>(SE_Weapon::TWeaponSE_Create, GraphKey, ClassesImports::Point(0, 0), Weapon->GetShotPalette(), -1);
                    Film = Globals::PrimaryFilm->AddObject(0u, Effect, 0, 0);
                    Globals::PrimaryFilm->SetWeaponEndpoints(StepIndex, Film, FilmObject, FilmObject);
                    Globals::PrimaryFilm->SetWeaponHit(StepIndex, Film, 0, 0, false, true);
                    Globals::PrimaryFilm->AttachObject(StepIndex, Film);
                }
            }
            ...
            for (auto cpp_range_5 = pas::for_to<std::int32_t>(0, Count - 1); cpp_range_5.next(I); ) {
                Ship = pas::list_at<TShip>(Star->Ships, I);
                if (Ship != this && Ship->InNormalSpace() && static_cast<std::uint8_t>(Ship->IsHullDestroyed() ^ 1)) {
                    if (aMyFunction::PointDistanceSquared(Ship->Position, Position) <= RadiusSquared) {
                        if (Target == Ship) {
                            Damage = Ship->ApplyWeaponHit(this, Weapon, -1.0f, Color, pas::Var<std::uint32_t>(&Flags), 1.0f, 0);
                        } else {
                            Damage = Ship->ApplyWeaponHit(this, Weapon, aMyFunction::PointDistance(Position, Ship->Position), Color, pas::Var<std::uint32_t>(&Flags), 1.0f, 0);
                        }
                        if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                            DrainedDamage += Damage;
                        }
                        if (RecordFilm) {
                            Effect = pas::construct_call<SE_Weapon::TWeaponSE>(SE_Weapon::TWeaponSE_Create, Info->SecondarySE, ClassesImports::Point(0, 0), Weapon->GetShotPalette(), -1);
                            Film = Globals::PrimaryFilm->AddObject(0u, Effect, 0, 0);
                            Globals::PrimaryFilm->SetWeaponEndpoints(StepIndex, Film, FilmObject, Ship->FilmObject);
        ...
        if (RecordFilm && DrainedDamage > 0) {
            ...
            Globals::PrimaryFilm->SetWeaponEndpoints(StepIndex, Film, FilmObject, FilmObject);
            if (aPlayer::GetPlayer() == this) {
                DisplayColor = aConst::OwnerToFilmColor(aConst::RaceToOwner(PilotRace));
            } else if (HasNamedScriptFaction()) {
                DisplayColor = aConst::CustomFactionToFilmColor(reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText);
            } else {
                DisplayColor = aConst::OwnerToFilmColor(OwnerId);
            }
        ...
        if (PrimaryDamage != 0 && Target->CountActiveArtefacts(aConst::t_ArtefactDef) > 0) {
            ...
            if (Reflect) {
                Damage = ApplyWeaponHit(Target, Weapon, -1.0f, Color, pas::Var<std::uint32_t>(&Flags), 1.0f, pas::abs(PrimaryDamage));
                if (RecordFilm) {
                    Effect = pas::construct_call<SE_Weapon::TWeaponSE>(SE_Weapon::TWeaponSE_Create, u"Weapon.NoGraph"_wref.get(), ClassesImports::Point(0, 0), 0, -1);
                    Film = Globals::PrimaryFilm->AddObject(0u, Effect, 0, 0);
                    Globals::PrimaryFilm->SetWeaponEndpoints(StepIndex, Film, Target->FilmObject, FilmObject);
                    Globals::PrimaryFilm->SetWeaponHit(StepIndex, Film, Color, Damage, IsHullDestroyed(), true);
                    Globals::PrimaryFilm->AttachObject(StepIndex, Film);
                }
            }
        }
    ...
    std::int32_t ApplyChainExplosion(TShip* ExplodingShip, TShip* Self, aItem::TWeapon*& Weapon, std::uint8_t& RecordFilm, std::uint32_t& Color, aGalaxyStruct::TDamageFlagSet& Flags, std::int32_t& Count, aGalaxy::TStar*& Star, float& DistanceSquared, aConst::PWeaponInfo& Info, std::int32_t& Damage, std::int32_t& DrainedDamage, SE_Space::TObjectSE*& Effect, aEFilm::TEFilmObj*& Film, std::int32_t& StepIndex, pas::List*& Ships, pas::List*& Damages, pas::List*& Colors, pas::List*& Films, pas::WideString& GraphKey) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, Count - 1); cpp_range.next(I); ) {
            ...
            if (Ship != ExplodingShip && Self != Ship && Ship->InNormalSpace() && static_cast<std::uint8_t>(Ship->IsHullDestroyed() ^ 1)) {
                ...
                if (pas::sqr(static_cast<pas::Extended>(Info->SecondaryDamageRadius)) >= DistanceSquared && Self->GetRelationLevelToShip(Ship) <= aGalaxyStruct::rlHostile) {
                        ...
                        if (!(pas::class_cast_if<aKling::TKling*>(Ship) != nullptr) || static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(Ship)->IsPlayerCamouflageEffective(Self) ^ 1)) {
                            {
                                ...
                                Damage = Ship->ApplyWeaponHit(Self, Weapon, sqrt, Color, pas::Var<std::uint32_t>(&Flags), 1.0f, round);
                            }
                            if (pas::contains(Flags, aGalaxyStruct::dkDrain) && Damage > 0) {
                                DrainedDamage += Damage;
                            }
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(0, Count - 1); cpp_range.next(I); ) {
                ...
                if (Ship->InNormalSpace() && static_cast<std::uint8_t>(Ship->IsHullDestroyed() ^ 1) && (aPlayer::GetPlayer() != Ship || aGalaxy::Galaxy->GodModEnabled != 1 && aGalaxy::Galaxy->SpecialSimulationMode == 0)) {
                    ...
                    if (static_cast<long double>(aConst::ItemExplosionRadiusSquared) >= DistanceSquared) {
                        Damage = Ship->ApplyExplosionDamage(this, Target, ActionResult, nullptr);
    ...
    std::int32_t TShip::ApplyInterceptorDamage(std::uint32_t& DamageColor) {
        static const pas::Set<0, 255> InterceptorDamageFlags = pas::constant_set<pas::Set<0, 255>>({{0}});
        std::int32_t Damage{};
        aGalaxyEvent::TGalaxyEvent* Event{};
        ...
        std::int32_t Result = ApplyDamage(SourceShip, Damage, -1.0f, DamageColor, static_cast<aGalaxyStruct::TDamageFlagSet>(InterceptorDamageFlags));
        if (aPlayer::GetPlayer() == this && GetHull()->HullPoints < 1) {
            Globals::ScoreScreen->RecordPlayerResult(false);
            Event = aGalaxyEvent::AddGalaxyEvent(u"PlayerDeath"_w, nullptr);
    ...
    std::int32_t TShip::ApplyShockStatusDamage(std::uint32_t& DamageColor) {
        static const pas::Set<0, 255> ShockDamageFlags = pas::constant_set<pas::Set<0, 255>>({{0}, {12}});
        return ApplyDamage(nullptr, System::Round(GetCombatStatusStrength(cseShock)), -1.0f, DamageColor, static_cast<aGalaxyStruct::TDamageFlagSet>(ShockDamageFlags));
    }
    ...
    std::int32_t TShip::ApplyAsteroidImpactDamage(aAsteroid::TAsteroid* Asteroid, std::uint32_t& DamageColor) {
        ...
        if (ScriptShip != nullptr && static_cast<std::uint8_t>(HasScriptStateText() ^ 1)) {
            ...
            if (IsEquipmentUsable(GetDefGenerator())) {
    ...
    std::int32_t TShip::ApplyExplosionDamage(TShip* SourceShip, pas::Object* ExplodingObject, std::int32_t ExtraDamage, pas::Object* Missile) {
        TShip* Ship{};
        float DistanceSquared{};
        aGalaxyEvent::TGalaxyEvent* Event{};
        ...
        if (pas::class_cast_if<aItem::TItem*>(ExplodingObject) != nullptr) {
            ...
            DistanceSquared = aMyFunction::PointDistanceSquared(Position, Item->Position);
            if (static_cast<long double>(aConst::ItemExplosionRadiusSquared) < DistanceSquared) {
                return Result;
            }
            ...
            if (Missile != nullptr) {
                Damage = ScriptItemsAct(aConst::satOnTakingDamage, Item, Missile, Damage);
            } else {
                Damage = ScriptItemsAct(aConst::satOnTakingDamage, Item, SourceShip, Damage);
            }
        } else if (pas::class_cast_if<TShip*>(ExplodingObject) != nullptr) {
            Ship = reinterpret_cast<TShip*>(ExplodingObject);
            if (aMyFunction::PointDistanceSquared(Position, Ship->Position) > 2.25E+4L) {
                return Result;
            }
            Damage = Ship->GetHull()->Weight / 3;
            Damage = Ship->ScriptItemsAct(aConst::satOnDealingKamikazeDamage, this, nullptr, Damage);
            Damage = ScriptItemsAct(aConst::satOnTakingDamage, Ship, nullptr, Damage);
        }
        if (Damage > 0) {
            Result = Damage;
            if (SourceShip != nullptr && this != SourceShip && Item != nullptr) {
                if (Missile == nullptr || reinterpret_cast<aMissile::TMissile*>(Missile)->Target == Item) {
                    if (aPlayer::GetPlayer() == this && SourceShip->ScriptShip != nullptr) {
                        reinterpret_cast<aScript::TScriptShip*>(SourceShip->ScriptShip)->HitPlayer = true;
                    }
                    if (aPlayer::GetPlayer() == SourceShip && ScriptShip != nullptr) {
                        reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->Hit = true;
                    }
                }
                if (Item->ItemType == aConst::t_ArtefactBomb) {
                    ...
                    ChangeRelationToRanger(SourceShip, System::Round(aMyFunction::RemapClamped(Damage, 1.0, GetHull()->HullPoints, -1.0, -8.0E+1)));
                }
            }
            if (GetHull()->HullPoints <= Damage && pas::class_cast_if<aKling::TKling*>(this) != nullptr && static_cast<aKling::TKling*>(this)->KlingType == aGalaxyStruct::ktBoss && (aKling::KellerShip == this || aPlayer::GetPlayer() != nullptr && aPlayer::GetPlayer()->CurrentStar != CurrentStar)) {
                GetHull()->HullPoints = Damage + 1;
            }
            if (GetHull()->HullPoints <= Damage && aPlayer::GetPlayer() != nullptr) {
                ScriptItemsAct(aConst::satOnDeath, SourceShip, ExplodingObject, 0);
                aPlayer::GetPlayer()->ProcessShipDestructionQuests(this);
                if (SourceShip != nullptr) {
                    if (pas::class_cast_if<aNormalShip::TNormalShip*>(SourceShip) != nullptr) {
                        reinterpret_cast<aNormalShip::TNormalShip*>(SourceShip)->ProcessShipKill(this);
                    }
                    if (pas::class_cast_if<aRanger::TRanger*>(SourceShip) != nullptr) {
                        reinterpret_cast<aRanger::TRanger*>(SourceShip)->ApplyAttackReputationChanges(this, 1.0);
                    }
                }
                if (aPlayer::GetPlayer() == PartnerShip || pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && static_cast<aTranclucator::TTranclucator*>(this)->OwnerShip == aPlayer::GetPlayer()) {
                    NotifyCompanionDeath();
            ...
            if (TypeId == aGalaxyStruct::stKling && SourceShip != nullptr) {
                pas::checked_cast<aKling::TKling*>(this)->DetectAttackingPlayer(SourceShip);
            }
            GetHull()->HullPoints = std::max<std::int32_t>(GetHull()->HullPoints - Damage, 0);
            RefreshDerivedStats(true);
            if (IsHullDestroyed()) {
                CurrentStar->ClearShipReferences(this);
            }
    ...
    std::int32_t TShip::ApplyStarHeatDamage() {
        ...
        if (pas::sqr(static_cast<pas::Extended>(CurrentStar->DamageRadius)) > DistanceSquared) {
            ...
            if (GetHull()->HullPoints <= 0 && aPlayer::GetPlayer() != nullptr) {
                ...
                aPlayer::GetPlayer()->ProcessShipDestructionQuests(this);
                if (GetRelationLevelToShip(aPlayer::GetPlayer()) <= aGalaxyStruct::rlHostile) {
                    if (aPlayer::GetPlayer() == OrderTarget || aPlayer::GetPlayer() == EnemyShip && pas::in_set<0, 0, 2, 3>(Order) || aPlayer::GetPlayer()->IsAttackingShip(this)) {
    ...
    pas::WideString TShip::GetWeaponDamageSummary() {
        pas::WideString Result{};
        ...
        if (WeaponCount != 0) {
            ...
            MinimumDamage *= GetAttackMultiplier();
            MaximumDamage *= GetAttackMultiplier();
            ...
            return pas::concat_wide({static_cast<pas::WideString>(pas::concat_ansi({"(", SysUtils::IntToStr(GetEffectiveSkillLevel(psAccuracy, false) & 0x0000007f), ") "})), Result});
        }
        return Result;
    }
    ...
    pas::WideString TShip::GetRepairPointsSummary() {
        pas::WideString Result{};
        Result = u"0"_w;
        if (GetRepairRobot() != nullptr) {
            return pas::wide_int_to_str(CalculateRepairPoints(GetRepairRobot()));
        }
        return Result;
    }
    ...
    std::int32_t TShip::GetWeaponActionRange(aItem::TWeapon* Weapon) {
        return GetWeaponRange(Weapon);
    }

    std::int32_t TShip::GetWeaponSlotRange(std::int32_t SlotIndex) {
        return GetWeaponRange(Weapons[SlotIndex]);
    }
        ...
        if (pas::in_range(static_cast<std::uint8_t>(Weapon->ItemType), static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon))) {
            ...
            if (pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstMissile), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) && static_cast<std::uint8_t>(aGalaxy::Galaxy->AreOldMissileBonusesEnabled() ^ 1)) {
                if (Weapon->MicroModuleIndex > 0) {
                    Result -= aConst::MicroModuleTemplates[Weapon->MicroModuleIndex - 1].StatBonuses[BonusKind];
                    Bonus += aConst::MicroModuleTemplates[Weapon->MicroModuleIndex - 1].StatBonuses[BonusKind];
                }
            ...
            if (Weapon->ExtraSpecials != nullptr) {
                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Weapon->ExtraSpecials) - 1); cpp_range.next(I); ) {
                    ...
                    Bonus += aConst::MicroModuleTemplates[Extra->ModuleIndexPlusOne - 1].StatBonuses[BonusKind] * Extra->Count;
                }
            }
    ...
    std::int32_t TShip::GetMaxWeaponRange() {
        ...
        std::int32_t Result = 0;
        {
            ...
            if (1 <= cpp_last) {
                for (I = 1; I <= cpp_last; ++I) {
                    if (IsEquipmentUsable(Weapons[I]) && GetWeaponSlotRange(I) > Result) {
    ...
    double TShip::ChanceToWin(TShip* Target) {
        double Result{};
        std::int32_t Index{};
        double Damage{};
        double OwnDamage{};
        ...
        try {
            ...
            Armor = Target->GetArmor();
            {
                ...
                if (1 <= cpp_last) {
                    for (Index = 1; Index <= cpp_last; ++Index) {
                        if (IsEquipmentUsable(Weapons[Index])) {
                            Damage = static_cast<long double>(GetWeaponMaxDamage(Weapons[Index])) * DefenseFactor - Armor;
                            if (Damage > 0.0L) {
            ...
            Armor = GetArmor();
            {
                ...
                if (1 <= cpp_last_2) {
                    for (Index = 1; Index <= cpp_last_2; ++Index) {
                        if (Target->IsEquipmentUsable(Target->Weapons[Index])) {
            ...
            {
                pas::Extended cpp_left = pas::real_divide(static_cast<long double>(GetHull()->HullPoints) * OwnDamage, pas::real_max<float>(0.01f, GetHull()->GetFragilityFactor(pas::constant_set<aGalaxyStruct::TDamageFlagSet>({}))));
            ...
            if (pas::Exception* E = pas::class_cast_if<pas::Exception*>(cpp_exception)) {
                GR_Main::AppendLogLineThreadSafe(pas::concat_ansi({static_cast<pas::AnsiString>(pas::class_name(pas::class_type(E))), " ", E->message}));
                GR_Main::AppendLogLineThreadSafe(static_cast<pas::AnsiString>(pas::concat_wide({u"Error in procedure TShip.ChanceToWin ", GetFullName(u" "_wref.get())})));
    ...
    std::uint8_t TShip::GetWinChancePercent(TShip* Target) {
        double Value = ChanceToWin(Target);
        if (Value >= 1.0L) {
        ...
        if (CurrentStar != nullptr && (Item->ItemType == aConst::t_ArtefactBomb || pas::class_cast_if<aItem::TCistern*>(Item) != nullptr)) {
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                ...
                if (!Ship->IsOutsideStarSpace()) {
                    ...
                    if (static_cast<long double>(aConst::ItemExplosionRadiusSquared) >= Distance && GetRelationLevelToShip(Ship) > aGalaxyStruct::rlHostile) {
                        return false;
                    }
                }
            }
    ...
    aGalaxyStruct::TRelationLevel TShip::GetRelationLevelToShip(TShip* Ship) {
        std::int32_t cpp_case = RelationToShip(Ship) & 0x0000007f;
    ...
    pas::WideString TShip::GetRelationLevelTextToShip(TShip* Ship) {
        pas::WideString Result{};
        aGalaxyStruct::TRelationLevel Level = GetRelationLevelToShip(Ship);
        Result = aConst::RelationInfo[Level].DisplayName;
        if (Level != aGalaxyStruct::rlHostile && pas::class_cast_if<aRuins::TRuins*>(this) != nullptr && pas::class_cast_if<aRanger::TRanger*>(Ship) != nullptr && RangerRelations != nullptr && pas::list_count(RangerRelations) > 0) {
            Level = aConst::RelationValueToLevel(static_cast<std::uint8_t>(reinterpret_cast<std::uintptr_t>(pas::list_get(RangerRelations, pas::list_indexof(aGalaxy::Galaxy->Rangers, reinterpret_cast<void*>(Ship))))));
    ...
    std::uint8_t TShip::TryExtortShip(TShip* Target) {
        std::int32_t Amount{};
        pas::WideString Response{};
        std::uint8_t Result = false;
        if (pas::in_range(Target->TypeId, aGalaxyStruct::stRanger, aGalaxyStruct::stPirate) && Target != TruceShip && Target->InNormalSpace() && (CurrentStanding != aGalaxyStruct::ssPirateMilitary || Target->CurrentStanding != aGalaxyStruct::ssCoalitionMilitary) && (Target->CurrentStanding != aGalaxyStruct::ssPirateMilitary || CurrentStanding != aGalaxyStruct::ssCoalitionMilitary) && CanContactShip(Target)) {
            if (TypeId == aGalaxyStruct::stRanger) {
                pas::checked_cast<aRanger::TRanger*>(this)->AddPirateCareerActivity(2);
            }
            ...
            if (GetCargoHook() != nullptr && CargoFreeSpace > 20 && aMyFunction::NextRandomUnitFloat(RandomState) > 0.4L && Target->HasCargoGoods()) {
                if (Target->BuildCargoExtortionResponse(this, Response)) {
                    Result = true;
                }
                if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && (Result || aMyFunction::NextRandomUnitFloat(RandomState) > 0.8L) && (aPlayer::GetPlayer() != Target || aRanger::PlayerAutomaticControl)) {
                    NotifyCargoDemand(Target, Response);
                }
            } else {
                Amount = System::Round(Wealth * 0.033333333333333333335L);
                if (Target->BuildMoneyExtortionResponse(this, Response, Amount)) {
                    Result = true;
                }
                if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && (Result || aMyFunction::NextRandomUnitFloat(RandomState) > 0.8L) && (aPlayer::GetPlayer() != Target || aRanger::PlayerAutomaticControl)) {
    ...
    void TShip::TruceWithShip(TShip* Ship) {
        std::int32_t I{};
        aItem::TWeapon* Weapon{};
        aMissile::TMissile* Missile{};
        TShip* Other{};
        if (aPlayer::GetPlayer() == Ship && aRanger::PendingPlayerFollowTarget == this || aPlayer::GetPlayer() == this && aRanger::PendingPlayerFollowTarget == Ship) {
            aRanger::PendingPlayerFollowTarget = nullptr;
        }
        if (EnemyShip == Ship) {
            EnemyShip = nullptr;
        ...
        TruceShip = Ship;
        if (aPlayer::GetPlayer() == Ship) {
            aPlayer::GetPlayer()->TruceShip = this;
        }
        {
            const std::int32_t cpp_last = static_cast<std::int32_t>(WeaponCount);
            if (1 <= cpp_last) {
        ...
        }
        {
            ...
            if (1 <= cpp_last_2) {
                for (I = 1; I <= cpp_last_2; ++I) {
                    ...
                    if (Weapon->Target == this) {
                        Weapon->Target = nullptr;
                    }
                }
            }
        }
        if (GetHull()->InterceptorTarget == Ship) {
            GetHull()->InterceptorTarget = nullptr;
        }
        if (Ship->InterceptorSourceShip == this) {
            Ship->ClearIncomingInterceptors();
        }
        if (Ship->GetHull()->InterceptorTarget == this) {
            Ship->GetHull()->InterceptorTarget = nullptr;
        }
        if (InterceptorSourceShip == Ship) {
            ClearIncomingInterceptors();
        }
        if (TypeId == aGalaxyStruct::stRanger && Ship->RelationToRanger(this) < 10) {
            Ship->ChangeRelationToRanger(this, 10);
        }
        if (Ship->TypeId == aGalaxyStruct::stRanger) {
            ChangeRelationToRanger(Ship, 30);
        }
        if (Order == soFollowShip && pas::checked_cast<TShip*>(OrderTarget) == Ship) {
            OrderNone(false);
            if (aPlayer::GetPlayer() != this) {
                NextDay();
            }
        ...
        if (Ship->Order == soFollowShip && pas::checked_cast<TShip*>(Ship->OrderTarget) == this) {
            Ship->OrderNone(false);
            if (aPlayer::GetPlayer() != Ship) {
                Ship->NextDay();
            }
        ...
        Ship->ClearCombatStatusSourceReferences(this);
        if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
            ...
            if (Other->PartnerShip == this) {
                Other->TruceWithShip(Ship);
            }
            if (Other->PartnerShip == Ship) {
                Other->TruceWithShip(this);
            }
            if (pas::class_cast_if<aTranclucator::TTranclucator*>(Other) != nullptr) {
                if (pas::checked_cast<aTranclucator::TTranclucator*>(Other)->OwnerShip == this) {
                    Other->TruceWithShip(Ship);
                }
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Missiles) - 1); cpp_range_2.next(I); ) {
            ...
            if (Missile->OwnerShip == this && Missile->Target == Ship) {
                Missile->Target = nullptr;
            }
            if (Missile->OwnerShip == Ship && Missile->Target == this) {
                Missile->Target = nullptr;
            }
        }
    }

    void TShip::RequestAlliesAttackShip(TShip* Target) {
        std::int32_t I{};
        ...
        if (Target->InNormalSpace()) {
            ...
            }()), 250000)) >= aMyFunction::PointDistanceSquared(Position, Target->Position)) {
                if (TypeId == aGalaxyStruct::stRanger) {
                    pas::checked_cast<aRanger::TRanger*>(this)->AddWarriorCareerActivity(1);
                }
                Requests = 0;
                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Ships) - 1); cpp_range.next(I); ) {
                    Other = pas::list_at<TShip>(CurrentStar->Ships, I);
                    if (Other != this && Other != Target && Other->InNormalSpace() && static_cast<std::uint8_t>(pas::contains(aConst::NonNegotiatingShipTypes, Other->TypeId) ^ 1) && Other->OrderTarget != Target && (Other->EnemyShip == nullptr || Other->EnemyShip->CurrentStar != CurrentStar) && Other->GetRelationLevelToShip(this) >= aGalaxyStruct::rlGood && (aPlayer::GetPlayer() != Other || GetRelationLevelToShip(Other) >= aGalaxyStruct::rlGood) && (Other->GetRelationLevelToShip(Target) <= aGalaxyStruct::rlNormal || pas::class_cast_if<aPirate::TPirate*>(Other) != nullptr && pas::class_cast_if<aPirate::TPirate*>(this) != nullptr) && static_cast<std::uint8_t>(pas::in_set<1, 2>(Other->TargetingRestriction) ^ 1) && static_cast<std::uint8_t>(pas::in_set<1, 2>(Target->TargetingRestriction) ^ 1) && CanContactShip(Other) && Other->PartnerShip != Target && Target->PartnerShip != Other && aMyFunction::NextRandomUnitFloat(RandomState) <= 0.9L && (!(pas::class_cast_if<aWarrior::TWarrior*>(Other) != nullptr) || pas::checked_cast<aWarrior::TWarrior*>(Other)->WarriorType != aWarrior::wtFlagship) && (aPlayer::GetPlayer() != Other || CurrentStar->Status.ControlFaction != aGalaxyStruct::sfCoalition || CurrentStar->Status.CustomFaction != u"" || TypeId != aGalaxyStruct::stWarrior || aPlayer::GetPlayer()->OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiPirate) || !(pas::class_cast_if<aNormalShip::TNormalShip*>(Target) != nullptr) || static_cast<std::uint8_t>(pas::in_set<2, 2, 4, 4>(Target->TypeId) ^ 1) && (Target->TypeId != aGalaxyStruct::stRanger || Target->GetDominantCareer() == aGalaxyStruct::rcPirate)) && (aPlayer::GetPlayer() != Other || CurrentStar->Status.ControlFaction != aGalaxyStruct::sfPirates || CurrentStar->Status.CustomFaction != u"" || TypeId != aGalaxyStruct::stPirate || OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiPirate) || aPlayer::GetPlayer()->OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiPirate) || !(pas::class_cast_if<aNormalShip::TNormalShip*>(Target) != nullptr) || Target->OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiPirate))) {
                        ...
                        if ((Other->BuildAttackRequestResponse(this, Response, Target) || aMyFunction::NextRandomUnitFloat(RandomState) > 0.8L) && aPlayer::GetPlayer()->CurrentStar == CurrentStar && (aPlayer::GetPlayer() != Other || aRanger::PlayerAutomaticControl)) {
    ...
    std::uint8_t TShip::IsInPrison() {
        if (aPlayer::GetPlayer() == this) {
            return aPlayer::GetPlayer()->InPrison;
        }
        switch (TypeId) {
            case aGalaxyStruct::stRanger: {
                if (static_cast<std::uint32_t>(pas::checked_cast<aRanger::TRanger*>(this)->PrisonTermRemaining) > 0) {
                    return true;
                }
                break;
            }
            case aGalaxyStruct::stPirate: {
                if (pas::checked_cast<aPirate::TPirate*>(this)->PrisonTermRemaining > 0) {
                    return true;
                }
                break;
            }
    ...
    std::uint8_t TShip::CanUseEquipmentTech(aItem::TEquipment* Item) {
        std::int32_t Level{};
        if (aGalaxy::Galaxy == nullptr || aGalaxy::Galaxy->IsEquipmentKnowledgeUnrestricted()) {
            return true;
        } else if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip != nullptr) {
            return reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->CanUseEquipmentTech(Item);
        } else if (pas::class_cast_if<aKling::TKling*>(this) != nullptr || Item->OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiDominator)) {
            return true;
        } else {
            ...
            Level -= 4;
            return TechKnowledge >= Level;
        }
    }

    std::uint8_t TShip::CanRepairEquipmentTech(aItem::TEquipment* Item) {
        std::int32_t Level{};
        if (aGalaxy::Galaxy == nullptr || aGalaxy::Galaxy->IsEquipmentKnowledgeUnrestricted()) {
            return true;
        } else if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip != nullptr) {
            return reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->CanRepairEquipmentTech(Item);
        } else if (pas::class_cast_if<aItem::THull*>(Item) != nullptr || pas::class_cast_if<aKling::TKling*>(this) != nullptr || Item->OwnerId != static_cast<std::uint8_t>(aGalaxyStruct::oiDominator)) {
            return true;
        } else {
            Level = Item->GetLevel();
            if (pas::class_cast_if<aItem::TWeapon*>(Item) != nullptr) {
    ...
    std::uint8_t TShip::IsEquipmentUsable(aItem::TEquipment* Item) {
        return Item != nullptr && (static_cast<std::uint8_t>(pas::in_set<43, 68, 73, 73>(static_cast<std::uint8_t>(Item->ItemType)) ^ 1) || Item->BrokenFlag == 0) && CanUseEquipmentTech(Item);
    }
    ...
    std::int32_t TShip::CalculateMass() {
        ...
        if (GetHull()->MicroModuleIndex != 0) {
            Bonus += aConst::MicroModuleTemplates[GetHull()->MicroModuleIndex - 1].StatBonuses[aConst::bonMass];
        }
    ...
    std::int32_t TShip::CalculateEquippedMass(aItem::TEquipment* ItemForModule) {
        ...
        if (ItemForModule != nullptr && ItemForModule->SpecialModuleIndex != 0) {
            Bonus += aConst::MicroModuleTemplates[ItemForModule->SpecialModuleIndex - 1].StatBonuses[aConst::bonMass];
        }
        Mass = Mass * (1.0L + pas::real_divide(Bonus, 1.0E+2L));
        if (pas::list_count(Artefacts) > 0) {
    ...
    std::int32_t TShip::GetArmor() {
        ...
        std::int32_t Result = CalculateHullArmor(GetHull()) - cpp_right;
    ...
    std::int32_t TShip::GetFuelLimitedJumpRange() {
        if (GetFuelTanks() == nullptr || static_cast<std::uint8_t>(CanUseEquipmentTech(GetFuelTanks()) ^ 1) || GetEngine() == nullptr || static_cast<std::uint8_t>(CanUseEquipmentTech(GetEngine()) ^ 1)) {
            return 0;
        }
    ...
    std::int32_t TShip::GetJumpRange() {
        ...
        if (GetEngine()->BrokenFlag != 0) {
            return System::Round(CalculateEngineJumpRange(GetEngine()) * 0.6L);
        }
        return CalculateEngineJumpRange(GetEngine());
    }

    std::uint8_t TShip::IsMicroModuleRaciallyRestricted(std::int32_t ModuleIndex) {
        ...
        if (aConst::MicroModuleTemplates[ModuleIndex].RacialRestriction) {
            if (ScriptShip != nullptr && reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText != u"") {
                Position = pas::pos(reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText, aConst::MicroModuleTemplates[ModuleIndex].AllowedCustomHullFactions);
                if (Position > 1) {
                    NameLength = reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText.length();
                ...
                if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && pas::load_unaligned<aGalaxy::TDominatorSeriesMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedDominatorSeriesMask) != static_cast<aGalaxy::TDominatorSeriesMask>(AllSeries)) {
                    return true;
                }
                if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) {
                    return true;
            ...
            if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && pas::contains(pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask), aGalaxyStruct::oiDominator) && pas::contains(pas::load_unaligned<aGalaxy::TDominatorSeriesMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedDominatorSeriesMask), static_cast<std::uint8_t>(static_cast<aKling::TKling*>(this)->DominatorSeries))) {
                return Result;
            }
            if ((pas::class_cast_if<aNormalShip::TNormalShip*>(this) != nullptr || pas::class_cast_if<aRuins::TRuins*>(this) != nullptr) && (pas::contains(pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask), aConst::RaceToOwner(PilotRace)) || OwnerId == static_cast<std::uint8_t>(aGalaxyStruct::oiPirate) && pas::contains(pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask), aGalaxyStruct::oiPirate))) {
                return Result;
            }
            if (!(pas::class_cast_if<aKling::TKling*>(this) != nullptr) && pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask) * static_cast<aGalaxyStruct::TOwnerMask>(PlanetOwners) == static_cast<aGalaxyStruct::TOwnerMask>(NoOwners) && pas::contains(pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask), OwnerId)) {
                return Result;
            }
            if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && pas::contains(pas::load_unaligned<aGalaxyStruct::TOwnerMask>(&aConst::MicroModuleTemplates[ModuleIndex].AllowedHullOwnerMask), aGalaxyStruct::oiUninhabited)) {
                return Result;
            }
            return true;
        }
    ...
    std::int32_t TShip::GetEquipmentStatBonus(std::uint8_t BonusKind, aItem::TEquipment* Item) {
        if (Item->SpecialModuleIndex != 0 && IsMicroModuleRaciallyRestricted(Item->SpecialModuleIndex - 1)) {
            return 0;
        }
    ...
    std::int32_t TShip::GetTotalStatBonus(std::uint8_t BonusKind) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
            Item = pas::list_at<aItem::TEquipment>(Inventory, I);
            if ((I == 0 || Item->EquippedFlag != 0) && IsEquipmentUsable(Item)) {
                Result += GetEquipmentStatBonus(BonusKind, Item);
            }
        }
    ...
    std::int32_t TShip::GetScannerPower() {
        std::int32_t Result = 0;
        if (IsEquipmentUsable(GetScanner())) {
            return CalculateScannerPower(GetScanner()) + 12 * (IsHealthEffectActive(21) & 127 & 0x0000007f);
        }
        return Result;
    }
    ...
    void TShip::ApplyRepairDroidHealing() {
        std::int32_t Repair{};
        if (!IsEquipmentUsable(GetRepairRobot())) {
            return;
        }
                ...
                if (TypeId != aGalaxyStruct::stKling) {
                    if (CountActiveArtefacts(aConst::t_ArtefactDroid) > 0) {
                        if (CanBoostArtefact(aConst::t_ArtefactDroid, nullptr, false)) {
                            ...
                            TShip* self = this;
                            self->ApplyItemDegradation(repairRobot, idkUse, cpp_arg);
                        } else {
                            ...
                            TShip* self_2 = this;
                            self_2->ApplyItemDegradation(repairRobot_2, idkUse, cpp_arg_2);
                        }
                    } else {
                        double cpp_arg_3 = aMyFunction::NextRandomUnitFloat(RandomState) * 2.0L;
                        aItem::TEquipment* repairRobot_3 = GetRepairRobot();
                        TShip* self_3 = this;
                        self_3->ApplyItemDegradation(repairRobot_3, idkUse, cpp_arg_3);
                    }
                }
    ...
    double TShip::GetDefenseDamageFactor() {
        ...
        return CalculateDefGeneratorFactor(GetDefGenerator());
    }
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
            ...
            if (Item->EquippedFlag != 0) {
                if (pas::in_range(static_cast<std::uint8_t>(Item->ItemType), static_cast<std::int32_t>(aConst::t_Hull), static_cast<std::int32_t>(aConst::t_DefGenerator))) {
                    ...
                    ++WeaponCount;
                    Weapons[WeaponCount] = pas::checked_cast<aItem::TWeapon*>(Item);
                    if (IsEquipmentUsable(Weapons[WeaponCount])) {
                        ++UsableWeaponCount;
                    }
                }
            ...
            case 1: {
                Result = 999999;
                {
                    ...
                    if (1 <= cpp_last) {
                        for (I = 1; I <= cpp_last; ++I) {
                            Weapon = Weapons[I];
                            if (IsEquipmentUsable(Weapon) && GetWeaponRange(Weapon) < Result) {
    ...
    void TShip::MergeItemIntoPlayerStorage(aItem::TItem* Item, pas::Object* Location, std::int32_t Slot) {
        ...
        if (pas::class_cast_if<aItem::TCountableItem*>(Item) != nullptr) {
            ...
            if (Index >= 0) {
                ...
                reinterpret_cast<aItem::TCountableItem*>(Entry->Item)->Merge(Item);
                if (Slot >= 0) {
                        ...
                        for (I = cpp_first; I >= 0; --I) {
                            ...
                            if (pas::class_cast_if<aItem::TArtefact*>(Entry->Item) != nullptr) {
                                ...
                                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Inventory) - 1); cpp_range.next(J); ) {
                                    ...
                                    if (pas::checked_cast<aItem::TCountableItem*>(Entry->Item)->CanMerge(Item)) {
                                        Stack = pas::checked_cast<aItem::TCountableItem*>(Item);
                                        break;
                                    }
                                }
                                if (Stack != nullptr) {
                                    Stack->Merge(pas::checked_cast<aItem::TCountableItem*>(Entry->Item));
                                    pas::free(Entry->Item);
                                } else {
    ...
    void TShip::AutoEquipArtefacts() {
        static const pas::Set<0, 255> EnergyFlags = pas::constant_set<pas::Set<0, 255>>({{0}});
        static const pas::Set<0, 255> SplinterFlags = pas::constant_set<pas::Set<0, 255>>({{1}});
        std::int32_t I{};
        aItem::TEquipment* Item{};
        ...
        std::int32_t RemainingSlots = GetSlotCountForItemType(aConst::t_Artefact);
        if (RemainingSlots <= 0) {
            return;
        }
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range_2.next(I); ) {
        ...
        while (RemainingSlots > 0 && I > 0) {
            aShip::EquipType(aConst::t_ArtefactAntigrav, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactSpeed, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactHull, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtMissileDef, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtEnergyDef, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtPDTurret, this, RemainingSlots);
            if (HasEnergy) {
                aShip::EquipType(aConst::t_ArtEnergyPulse, this, RemainingSlots);
            }
            if (HasSplinter) {
                aShip::EquipType(aConst::t_ArtSplinter, this, RemainingSlots);
            }
            if (HasMissiles) {
                aShip::EquipType(aConst::t_ArtFastRacks, this, RemainingSlots);
            }
            if (GetDefGenerator() != nullptr) {
                aShip::EquipType(aConst::t_ArtefactDef, this, RemainingSlots);
            }
            if (GetRepairRobot() != nullptr) {
                aShip::EquipType(aConst::t_ArtefactDroid, this, RemainingSlots);
            }
            if (HasSplinter) {
                aShip::EquipType(aConst::t_ArtDecelerate, this, RemainingSlots);
            }
            if (HasEnergy & (GetDefGenerator() != nullptr)) {
                aShip::EquipType(aConst::t_ArtDefToEnergy, this, RemainingSlots);
            }
            if (GetCargoHook() != nullptr && static_cast<std::uint8_t>(pas::in_set<0, 0, 2, 2, 4, 4>(TypeId) ^ 1)) {
                aShip::EquipType(aConst::t_ArtefactHook, this, RemainingSlots);
            }
            if (!pas::in_set<0, 0, 2, 2, 4, 4>(TypeId)) {
                aShip::EquipType(aConst::t_ArtefactMiniExpl, this, RemainingSlots);
            }
            aShip::EquipType(aConst::t_ArtefactNano, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactPower, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactRadar, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactScaner, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtefactFuel, this, RemainingSlots);
            aShip::EquipType(aConst::t_ArtBio, this, RemainingSlots);
            if (GetSlotCount(aConst::sskWeapon) < 5) {
                aShip::EquipType(aConst::t_ArtDefToArms1, this, RemainingSlots);
            }
            if (GetSlotCount(aConst::sskDefGenerator) > 0 && GetSlotCount(aConst::sskWeapon) < 4) {
                aShip::EquipType(aConst::t_ArtDefToArms2, this, RemainingSlots);
            }
            if (GetSlotCount(aConst::sskWeapon) > WeaponCount) {
                aShip::EquipType(aConst::t_ArtWeaponToSpeed, this, RemainingSlots);
            }
            if (GetSlotCount(aConst::sskAfterburner) > 0 && pas::in_set<1, 1, 3, 4>(TypeId)) {
                aShip::EquipType(aConst::t_ArtForsage, this, RemainingSlots);
            }
            if (GetEngine() != nullptr && GetEngine()->JumpRange * 1.2L < aConst::HyperJumpArtefactRange) {
                aShip::EquipType(aConst::t_ArtGiperJump, this, RemainingSlots);
            }
            --I;
        }
        RefreshDerivedStats(true);
    ...
    float TShip::CalculateItemEffectiveness(aItem::TItem* Item) {
        ...
        if (pas::class_cast_if<aItem::TEquipment*>(Item) != nullptr && static_cast<std::uint8_t>(CanUseEquipmentTech(reinterpret_cast<aItem::TEquipment*>(Item)) ^ 1)) {
            return Result;
        }
        ...
        if (pas::in_range(static_cast<std::uint8_t>(Equipment->ItemType), static_cast<std::int32_t>(aConst::t_Hull), static_cast<std::int32_t>(aConst::t_DefGenerator))) {
            switch (Equipment->ItemType) {
                case aConst::t_Hull: {
                    ...
                    Result = EvaluateStatBonus(aConst::bonHull, CalculateHullArmor(CandidateHull));
                ...
                case aConst::t_Engine: {
                    pas::Extended cpp_left_19 = EvaluateStatBonus(aConst::bonSpeed, CalculateEngineSpeed(reinterpret_cast<aItem::TEngine*>(Equipment), InNormalSpace() || static_cast<std::uint8_t>(CanRepairEquipmentTech(Equipment) ^ 1)));
                    Result = cpp_left_19 + EvaluateStatBonus(aConst::bonJump, CalculateEngineJumpRange(reinterpret_cast<aItem::TEngine*>(Equipment)));
                    break;
                }
                case aConst::t_Radar: {
                    Result = EvaluateStatBonus(aConst::bonRadar, CalculateRadarRange(reinterpret_cast<aItem::TRadar*>(Equipment)));
                    break;
                }
                case aConst::t_Scaner: {
                    if (GetRadar() != nullptr) {
                        Result = EvaluateStatBonus(aConst::bonScan, CalculateScannerPower(reinterpret_cast<aItem::TScaner*>(Equipment)));
                    }
                    break;
                }
                case aConst::t_RepairRobot: {
                    Result = EvaluateStatBonus(aConst::bonDroid, CalculateRepairPoints(reinterpret_cast<aItem::TRepairRobot*>(Equipment)));
                    break;
                }
                case aConst::t_CargoHook: {
                    Result = EvaluateStatBonus(aConst::bonHook, CalculateCargoHookPower(reinterpret_cast<aItem::TCargoHook*>(Equipment)));
                    break;
                }
                case aConst::t_DefGenerator: {
                    Result = EvaluateStatBonus(aConst::bonDef, System::Round(1.0E+2L - CalculateDefGeneratorFactor(reinterpret_cast<aItem::TDefGenerator*>(Equipment)) * 1.0E+2L));
                    break;
                }
            }
        ...
        Result = static_cast<long double>(Result) + GetEquipmentEvaluationSynergyBonus(Equipment);
        aGalaxy::Galaxy->CustomRules.ChaoticRandom = SavedChaoticRandom;
        if (Equipment->ScriptItem != nullptr && reinterpret_cast<aScript::TScriptItem*>(Equipment->ScriptItem)->Name != u"" || Equipment->NoDropFlag > 0) {
            ...
            Result = 0.0f;
        } else if (Equipment->BrokenFlag != 0 && static_cast<std::uint8_t>(CanRepairEquipmentTech(Equipment) ^ 1)) {
            Result = 0.0f;
        } else if (!CanUseEquipmentTech(Equipment)) {
            Result = 0.0f;
        }
        if (TemporarilyUnequipped) {
            EquipItem(Equipment);
    ...
    std::int32_t TShip::GetWeaponRange(aItem::TWeapon* Weapon) {
        std::int32_t Range{};
        std::int32_t I{};
        ...
        Range = Weapon->Range + GetTotalStatBonus(aConst::bonWRadius);
        if (Weapon->ExtraSpecials != nullptr) {
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Weapon->ExtraSpecials) - 1); cpp_range.next(I); ) {
                Extra = pas::list_at<aItem::TExtraSpecial>(Weapon->ExtraSpecials, I);
                Range += aConst::MicroModuleTemplates[Extra->ModuleIndexPlusOne - 1].StatBonuses[aConst::bonWRadius] * Extra->Count;
            }
        }
        if (pas::in_range(Weapon->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket))) {
            ...
            if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && static_cast<aKling::TKling*>(this)->KlingType == 0) {
                ...
                Range = std::max<std::int32_t>(Range, TemplateRange);
            } else if (aGalaxy::Galaxy->AreMaxRangeMissilesEnabled() || aPlayer::GetPlayer() == this) {
                aShip::ClampMissileWeaponRange(this, TemplateRange, Range);
            } else {
                Range = std::min<std::int32_t>(GetRadarRange(), Range);
            }
        }
        return std::max<std::int32_t>(100, Range);
    }

    std::int32_t TShip::CalculateHullArmor(aItem::THull* Hull) {
        if (Hull->ItemType != aConst::t_Hull) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonHull);
        if (GetHull() != Hull) {
            Bonus -= GetEquipmentStatBonus(aConst::bonHull, GetHull());
            Bonus += GetEquipmentStatBonus(aConst::bonHull, Hull);
        }
        std::int32_t Value = Hull->Armor + Bonus;
        {
            std::int32_t cpp_right = aConst::HullArtefactArmor + aConst::HullArtefactBoostArmor * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtefactHull, Hull, false));
            Value += CountActiveArtefacts(aConst::t_ArtefactHull) * cpp_right;
        }
    ...
    std::int32_t TShip::CalculateEngineSpeed(aItem::TEngine* Engine, std::uint8_t ApplyBrokenPenalty) {
        std::int32_t I{};
        if (Engine->ItemType != aConst::t_Engine) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonSpeed);
        if (Engine->EquippedFlag == 0 && Engine->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonSpeed, Engine);
        }
        ...
        {
            std::int32_t cpp_right = aConst::WeaponToSpeedArtefactBonus + aConst::WeaponToSpeedArtefactBoost * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtWeaponToSpeed, Engine, false));
            Value += CountActiveArtefacts(aConst::t_ArtWeaponToSpeed) * cpp_right;
        }
        Value = std::max<std::int32_t>(std::min<std::int32_t>(200, Engine->Speed), Value + Bonus);
        for (auto cpp_range = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtefactSpeed)); cpp_range.next(I); ) {
            Value = System::Round(Value * (aConst::SpeedArtefactFactor + static_cast<long double>(aConst::SpeedArtefactBoostFactor) * static_cast<std::int8_t>(CanBoostArtefact(aConst::t_ArtefactSpeed, nullptr, false))));
        }
        return std::max<std::int32_t>(0, Value);
    }

    std::int32_t TShip::CalculateEngineJumpRange(aItem::TEngine* Engine) {
        if (Engine->ItemType != aConst::t_Engine) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonJump);
        if (Engine->EquippedFlag == 0 && Engine->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonJump, Engine);
        }
        std::int32_t Value = Engine->JumpRange;
        if (!CanUseEquipmentTech(Engine)) {
            Value = Value / 2;
        }
        if (CountActiveArtefacts(aConst::t_ArtGiperJump) > 0) {
            Value = std::max<std::int64_t>(static_cast<std::int64_t>(Value), ([&] {
                pas::Extended cpp_right = aConst::HyperJumpArtefactRange + aConst::HyperJumpArtefactBoostRange * (CanBoostArtefact(aConst::t_ArtGiperJump, Engine, false) & 127 & 0x0000007f);
                return System::Round(System::Sqrt(CountActiveArtefacts(aConst::t_ArtGiperJump)) * cpp_right);
            }()));
        }
        return std::max<std::int32_t>(0, Value + Bonus);
    }

    std::int32_t TShip::CalculateRadarRange(aItem::TRadar* Radar) {
        if (Radar->ItemType != aConst::t_Radar) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonRadar);
        if (Radar->EquippedFlag == 0 && Radar->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonRadar, Radar);
        }
        std::int32_t Value = Radar->Range;
        {
            std::int32_t cpp_right = aConst::RadarArtefactRange + aConst::RadarArtefactBoostRange * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtefactRadar, Radar, false));
            Value += CountActiveArtefacts(aConst::t_ArtefactRadar) * cpp_right;
        }
        return std::max<std::int32_t>(0, Value + Bonus);
    }

    std::int32_t TShip::CalculateScannerPower(aItem::TScaner* Scanner) {
        if (Scanner->ItemType != aConst::t_Scaner) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonScan);
        if (Scanner->EquippedFlag == 0 && Scanner->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonScan, Scanner);
        }
        std::int32_t Value = Scanner->ScanPower;
        {
            std::int32_t cpp_right = aConst::ScannerArtefactPower + aConst::ScannerArtefactBoostPower * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtefactScaner, Scanner, false));
            Value += CountActiveArtefacts(aConst::t_ArtefactScaner) * cpp_right;
        }
        return std::max<std::int32_t>(0, Value + Bonus);
    }

    std::int32_t TShip::CalculateRepairPoints(aItem::TRepairRobot* RepairRobot) {
        if (RepairRobot->ItemType != aConst::t_RepairRobot) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonDroid);
        if (RepairRobot->EquippedFlag == 0 && RepairRobot->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonDroid, RepairRobot);
        }
        std::int32_t Value = RepairRobot->RepairPoints;
        {
            std::int32_t cpp_right = aConst::DroidArtefactRepair + aConst::DroidArtefactBoostRepair * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtefactDroid, RepairRobot, false));
            Value += CountActiveArtefacts(aConst::t_ArtefactDroid) * cpp_right;
        }
        return std::max<std::int32_t>(0, Value + Bonus);
    }

    std::int32_t TShip::CalculateCargoHookPower(aItem::TCargoHook* CargoHook) {
        if (CargoHook->ItemType != aConst::t_CargoHook) {
            return 0;
        }
        std::int32_t Bonus = GetTotalStatBonus(aConst::bonHook);
        if (CargoHook->EquippedFlag == 0 && CargoHook->SpecialModuleIndex != 0) {
            Bonus += GetEquipmentStatBonus(aConst::bonHook, CargoHook);
        }
        std::int32_t Value = CargoHook->PickupPower;
        {
            std::int32_t cpp_right = aConst::CargoHookArtefactPower + aConst::CargoHookArtefactBoostPower * static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtefactHook, CargoHook, false));
            Value += CountActiveArtefacts(aConst::t_ArtefactHook) * cpp_right;
        }
        return std::max<std::int32_t>(0, Value + Bonus);
    }

    float TShip::CalculateDefGeneratorFactor(aItem::TDefGenerator* DefGenerator) {
        std::int32_t I{};
        if (DefGenerator->ItemType != aConst::t_DefGenerator) {
            return 0.0f;
        }
        std::int32_t Percent = GetTotalStatBonus(aConst::bonDef);
        if (DefGenerator->EquippedFlag == 0 && DefGenerator->SpecialModuleIndex != 0) {
            Percent += GetEquipmentStatBonus(aConst::bonDef, DefGenerator);
        }
        ...
        {
            pas::Extended cpp_right = aConst::DefenseArtefactBonus + static_cast<long double>(aConst::DefenseArtefactBoost) * static_cast<std::int8_t>(CanBoostArtefact(aConst::t_ArtefactDef, DefGenerator, false));
            Factor = Factor - CountActiveArtefacts(aConst::t_ArtefactDef) * cpp_right;
        }
        for (auto cpp_range = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtDefToEnergy)); cpp_range.next(I); ) {
            Factor = pas::real_min<pas::Extended>(1.0L, Factor + (1.0L - Factor) * (aConst::DefenseToEnergyPenalty + static_cast<long double>(aConst::DefenseToEnergyBoostPenalty) * static_cast<std::int8_t>(CanBoostArtefact(aConst::t_ArtDefToEnergy, DefGenerator, false))));
        }
        if (CountActiveArtefacts(aConst::t_ArtDefToArms1) > 0 && static_cast<std::uint8_t>(CanBoostArtefact(aConst::t_ArtDefToArms1, DefGenerator, false) ^ 1)) {
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtDefToArms1)); cpp_range_2.next(I); ) {
    ...
    float TShip::GetEquipmentEvaluationSynergyBonus(aItem::TEquipment* Item) {
        float Positive{};
        float Nonpositive{};
        float DamageBonus{};
        ...
        if (Item->SpecialModuleIndex != 0) {
            ...
            if (!IsMicroModuleRaciallyRestricted(ModuleIndex)) {
                Positive = 0.0f;
                Nonpositive = 0.0f;
                ...
                if (Item->ExtraSpecials == nullptr || pas::list_count(Item->ExtraSpecials) == 0) {
                    ...
                    if (ItemType != aConst::t_Hull) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonHull, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonHull * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (ItemType != aConst::t_Engine) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSpeed, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSpeed * sizeof(std::int32_t)))), Positive, Nonpositive);
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonJump, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonJump * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (GetRadar() != nullptr && ItemType != aConst::t_Radar) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonRadar, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonRadar * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (GetScanner() != nullptr && ItemType != aConst::t_Scaner) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonScan, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonScan * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (GetRepairRobot() != nullptr && ItemType != aConst::t_RepairRobot) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonDroid, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonDroid * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (GetCargoHook() != nullptr && ItemType != aConst::t_CargoHook) {
                        aShip::AccumulateEquipmentBonus(([&] {
                            pas::Extended cpp_left = ([&] {
                                pas::Extended cpp_left_2 = EvaluateStatBonus(aConst::bonHook, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonHook * sizeof(std::int32_t))));
                                return cpp_left_2 + CalculateCargoHookPower(GetCargoHook());
                            }());
                            return cpp_left - EvaluateStatBonus(aConst::bonHook, CalculateCargoHookPower(GetCargoHook()));
                        }()), Positive, Nonpositive);
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonHookRadius, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonHookRadius * sizeof(std::int32_t)))), Positive, Nonpositive);
                    }
                    if (GetDefGenerator() != nullptr && ItemType != aConst::t_DefGenerator) {
                        aShip::AccumulateEquipmentBonus(([&] {
                            pas::Extended cpp_left_3 = EvaluateStatBonus(aConst::bonDef, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonDef * sizeof(std::int32_t))) + static_cast<std::int32_t>(System::Round(1.0E+2L - CalculateDefGeneratorFactor(GetDefGenerator()) * 1.0E+2L)));
                            return cpp_left_3 - EvaluateStatBonus(aConst::bonDef, System::Round(1.0E+2L - CalculateDefGeneratorFactor(GetDefGenerator()) * 1.0E+2L));
                        }()), Positive, Nonpositive);
                    }
                    aShip::AccumulateEquipmentBonus(([&] {
                        pas::Extended cpp_left_4 = EvaluateStatBonus(aConst::bonWRadius, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonWRadius * sizeof(std::int32_t))));
                        return cpp_left_4 * (CountEquippedWeapons() & 0x0000007f);
                    }()), Positive, Nonpositive);
                    if (!pas::in_range(static_cast<std::uint8_t>(ItemType), static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon))) {
                        for (auto cpp_range = pas::for_to<std::int32_t>(1, CountEquippedWeapons() & 0x0000007f); cpp_range.next(I); ) {
                            DamageBonus = 0.0f;
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWEnergy, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonWEnergy * sizeof(std::int32_t))));
                            }
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWSplinter, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonWSplinter * sizeof(std::int32_t))));
                            }
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkMissile)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWMissile, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonWMissile * sizeof(std::int32_t))));
                            }
                            if (DamageBonus > 0.001L || DamageBonus < -0.001L) {
                                DamageBonus = EvaluateWeaponDamage(Weapons[I], false, DamageBonus);
                            }
                            aShip::AccumulateEquipmentBonus(DamageBonus, Positive, Nonpositive);
                        }
                    }
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill1, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill1 * sizeof(std::int32_t)))), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill2, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill2 * sizeof(std::int32_t)))), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill3, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill3 * sizeof(std::int32_t)))), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill4, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill4 * sizeof(std::int32_t)))), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill5, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill5 * sizeof(std::int32_t)))), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill6, pas::load_unaligned<std::int32_t>(pas::byte_offset(&Module->StatBonuses, aConst::bonSkill6 * sizeof(std::int32_t)))), Positive, Nonpositive);
                } else {
                    if (ItemType != aConst::t_Hull) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonHull, Item->GetStatBonus(aConst::bonHull)), Positive, Nonpositive);
                    }
                    if (ItemType != aConst::t_Engine) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSpeed, Item->GetStatBonus(aConst::bonSpeed)), Positive, Nonpositive);
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonJump, Item->GetStatBonus(aConst::bonJump)), Positive, Nonpositive);
                    }
                    if (GetRadar() != nullptr && ItemType != aConst::t_Radar) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonRadar, Item->GetStatBonus(aConst::bonRadar)), Positive, Nonpositive);
                    }
                    if (GetScanner() != nullptr && ItemType != aConst::t_Scaner) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonScan, Item->GetStatBonus(aConst::bonScan)), Positive, Nonpositive);
                    }
                    if (GetRepairRobot() != nullptr && ItemType != aConst::t_RepairRobot) {
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonDroid, Item->GetStatBonus(aConst::bonDroid)), Positive, Nonpositive);
                    }
                    if (GetCargoHook() != nullptr && ItemType != aConst::t_CargoHook) {
                        {
                            pas::Extended cpp_left_6 = EvaluateStatBonus(aConst::bonHook, Item->GetStatBonus(aConst::bonHook));
                            pas::Extended cpp_left_5 = cpp_left_6 + CalculateCargoHookPower(GetCargoHook());
                            aShip::AccumulateEquipmentBonus(cpp_left_5 - EvaluateStatBonus(aConst::bonHook, CalculateCargoHookPower(GetCargoHook())), Positive, Nonpositive);
                        }
                        aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonHookRadius, Item->GetStatBonus(aConst::bonHookRadius)), Positive, Nonpositive);
                    }
                    if (GetDefGenerator() != nullptr && ItemType != aConst::t_DefGenerator) {
                        std::int32_t cpp_right = System::Round(1.0E+2L - CalculateDefGeneratorFactor(GetDefGenerator()) * 1.0E+2L);
                        pas::Extended cpp_left_7 = EvaluateStatBonus(aConst::bonDef, Item->GetStatBonus(aConst::bonDef) + cpp_right);
                        aShip::AccumulateEquipmentBonus(cpp_left_7 - EvaluateStatBonus(aConst::bonDef, System::Round(1.0E+2L - CalculateDefGeneratorFactor(GetDefGenerator()) * 1.0E+2L)), Positive, Nonpositive);
                    }
                    {
                        pas::Extended cpp_left_8 = EvaluateStatBonus(aConst::bonWRadius, Item->GetStatBonus(aConst::bonWRadius));
                        aShip::AccumulateEquipmentBonus(cpp_left_8 * (CountEquippedWeapons() & 0x0000007f), Positive, Nonpositive);
                    }
                    if (!pas::in_range(static_cast<std::uint8_t>(ItemType), static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon))) {
                        for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, CountEquippedWeapons() & 0x0000007f); cpp_range_2.next(I); ) {
                            DamageBonus = 0.0f;
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWEnergy, Item->GetStatBonus(aConst::bonWEnergy));
                            }
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWSplinter, Item->GetStatBonus(aConst::bonWSplinter));
                            }
                            if (pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[I]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkMissile)) {
                                DamageBonus = static_cast<long double>(DamageBonus) + EvaluateStatBonus(aConst::bonWMissile, Item->GetStatBonus(aConst::bonWMissile));
                            }
                            if (DamageBonus > 0.001L || DamageBonus < -0.001L) {
                                DamageBonus = EvaluateWeaponDamage(Weapons[I], false, DamageBonus);
                            }
                    ...
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill1, Item->GetStatBonus(aConst::bonSkill1)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill2, Item->GetStatBonus(aConst::bonSkill2)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill3, Item->GetStatBonus(aConst::bonSkill3)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill4, Item->GetStatBonus(aConst::bonSkill4)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill5, Item->GetStatBonus(aConst::bonSkill5)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(EvaluateStatBonus(aConst::bonSkill6, Item->GetStatBonus(aConst::bonSkill6)), Positive, Nonpositive);
                    aShip::AccumulateEquipmentBonus(Item->GetStatBonus(aConst::bonAIValue), Positive, Nonpositive);
                }
                {
                    pas::Extended cpp_left_9 = EvaluateStatBonus(aConst::bonMass, CalculateEquippedMass(Item));
                    aShip::AccumulateEquipmentBonus(cpp_left_9 - EvaluateStatBonus(aConst::bonMass, CalculateEquippedMass(nullptr)), Positive, Nonpositive);
                }
                if (Item->MicroModuleIndex != 0) {
                    ...
                    Nonpositive = static_cast<long double>(Nonpositive) + System::Round(static_cast<long double>(aConst::MicroModuleTemplates[Item->MicroModuleIndex - 1].StatBonuses[aConst::bonExtraAkrinPenalty]) * Nonpositive * 1.0E-4L);
                }
            ...
            for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
                ...
                if (pas::class_cast_if<aItem::TMicroModule*>(Module) != nullptr) {
                    ...
                    for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Inventory) - 1); cpp_range_2.next(J); ) {
                        ...
                        if (pas::checked_cast<aItem::TMicroModule*>(Module)->CanInstallOn(Candidate) && (Candidate->EquippedFlag != 0 || Candidate->ItemType == aConst::t_Hull) && aShip::FitsMicroModuleCapacity(this, Candidate, Module)) {
    ...
    std::uint8_t FitsMicroModuleCapacity(TShip* Self, aItem::TEquipment*& Candidate, aItem::TEquipment*& Module) {
        if (Candidate->ItemType != aConst::t_Hull) {
            return Candidate->Weight * (aConst::MicroModuleTemplates[pas::checked_cast<aItem::TMicroModule*>(Module)->MicroModuleIndex - 1].SizePercent - 100) * 0.01L <= Self->CargoFreeSpace + 1;
        }
        return -Candidate->Weight * (aConst::MicroModuleTemplates[pas::checked_cast<aItem::TMicroModule*>(Module)->MicroModuleIndex - 1].SizePercent - 100) * 0.01L <= Self->CargoFreeSpace + 1;
    }
    ...
    void TShip::DropCargoUntilNotOverloaded() {
        std::int32_t Count{};
        aItem::TEquipment* Item{};
        aItem::TArtefact* Artefact{};
        std::uint8_t Good{};
        ...
        std::uint8_t CanDrop = InNormalSpace() && static_cast<std::uint8_t>(pas::in_range(TypeId, static_cast<std::int32_t>(aGalaxyStruct::rstRangerCenter), static_cast<std::int32_t>(aGalaxyStruct::rstCustomStation)) ^ 1);
        if (CargoFreeSpace < 0 && NoDrop && CanDrop) {
            if (ScriptShip == nullptr) {
                GR_Main::AppendLogLineThreadSafe(static_cast<pas::AnsiString>(pas::concat_wide({u"Warning! Ship ", GetFullName(u" "_wref.get()), u" is overloaded but cant drop anything with NoDrop flag"})));
            } else {
                GR_Main::AppendLogLineThreadSafe(static_cast<pas::AnsiString>(pas::concat_wide({u"Warning! Ship ", GetFullName(u" "_wref.get()), u" (", reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->Script->ScriptFileName, u" ", reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->State->Name, u") is overloaded but cant drop anything with NoDrop flag"})));
            }
        } else {
            while (CargoFreeSpace < 0) {
                if (NoDrop) {
                    Item = nullptr;
                } else {
                    Item = reinterpret_cast<aItem::TEquipment*>(SelectLeastValuableInventoryItem());
                }
                if (Item != nullptr) {
                ...
                if (SelectCheapestCargoGood() < 255) {
                    Good = SelectCheapestCargoGood();
                    Count = std::min<std::int32_t>(pas::abs(CargoFreeSpace), CargoGoods[Good].Count);
                ...
                if (Artefact != nullptr && ArtefactValue <= ItemValue) {
                    ...
                    if (CanDrop && pas::class_cast_if<aItem::TCountableItem*>(Item) != nullptr && Item->Weight + CargoFreeSpace > 0) {
                        Item = pas::checked_cast<aItem::TCountableItem*>(Item)->Split(-CargoFreeSpace);
                        pas::list_add(Inventory, reinterpret_cast<void*>(Item));
                        DropCarriedItemAsMovingLoot(Item);
                    } else if (CanDrop) {
                        DropCarriedItemAsMovingLoot(Item);
                ...
                } else if (Item != nullptr && static_cast<std::uint8_t>(IsEssentialInventoryItem(Item) ^ 1)) {
                    if (CanDrop) {
                        DropCarriedItemAsMovingLoot(Item);
                    } else {
                        LiquidateInventoryItem(Item);
                    ...
                    for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
                        Candidate = pas::list_at<aItem::TItem>(Inventory, I);
                        if (Candidate->NoDropFlag > 0) {
                            continue;
                        }
                        if (pas::in_range(Candidate->ItemType, static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon)) && Candidate->Weight > GetHull()->Weight * 0.2L) {
                            Item = reinterpret_cast<aItem::TEquipment*>(Candidate);
                            break;
                        }
                        if (Candidate->ItemType == aConst::t_CargoHook && Candidate->Weight > GetHull()->Weight * 0.2L) {
                            Item = reinterpret_cast<aItem::TEquipment*>(Candidate);
                            break;
                        }
                    }
                    if (Item != nullptr && (pas::in_range(Item->ItemType, static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon)) || Item->ItemType == aConst::t_CargoHook)) {
                        ...
                        if (!(pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) && ConsecutiveDockedDays > 100) {
                            DestroyQueued = true;
                        }
                        break;
                    }
    ...
    void TShip::DropRandomCheapItemsOnDestruction(std::int32_t Count) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, Count); cpp_range.next(I); ) {
            if (pas::list_count(Inventory) > 1) {
                Item = aShip::SelectCheapItem(this, Index);
                if (Item != nullptr) {
                    DropCarriedItemAsMovingLoot(Item);
                }
            }
    ...
    aItem::TItem* SelectCheapItem(TShip* Self, std::int32_t& Index) {
        ...
        double Value{};
        aItem::TItem* Result = nullptr;
        Index = aMyFunction::NextRandomIntRange(1, pas::list_count(Self->Inventory) - 1, Self->RandomState);
        for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Self->Inventory) - 1); cpp_range.next(Attempt); ) {
            aMyFunction::IncrementWrapped(Index, 1, pas::list_count(Self->Inventory) - 1);
            Candidate = pas::list_at<aItem::TItem>(Self->Inventory, Index);
            ...
            if (pas::class_cast_if<aKling::TKling*>(Self) != nullptr) {
                Value = static_cast<long double>(KlingCheapDropValueFactors[pas::checked_cast<aKling::TKling*>(Self)->KlingType]) * Value;
            }
            if (Candidate->Cost > 1000 && ([&] {
                std::int32_t cpp_left = aMyFunction::NextRandomIntRange(1, 100, Self->RandomState);
                return cpp_left > System::Round(1.0E+2L * System::Exp(2.0L - 2.0L * Value));
            }())) {
                continue;
            }
            if (Result == nullptr || ([&] {
                pas::Extended cpp_left_2 = aMyFunction::NextRandomFloatRange(0.5, 1.0, Self->RandomState);
    ...
    void TShip::DropRandomValuableItemsOnDestruction(std::int32_t Count) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, Count); cpp_range.next(I); ) {
            if (pas::list_count(Inventory) > 1) {
                Item = aShip::SelectValuableItem(this, Index);
                if (Item != nullptr) {
                    DropCarriedItemAsMovingLoot(Item);
                }
            }
    ...
    aItem::TItem* SelectValuableItem(TShip* Self, std::int32_t& Index) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Self->Inventory) - 1); cpp_range.next(Attempt); ) {
            ...
            if (pas::class_cast_if<aKling::TKling*>(Self) != nullptr) {
                Value = static_cast<long double>(KlingValuableDropValueFactors[pas::checked_cast<aKling::TKling*>(Self)->KlingType]) * Value;
            }
            if (Candidate->Cost > 1000 && ([&] {
                std::int32_t cpp_left = aMyFunction::NextRandomIntRange(1, 100, Self->RandomState);
    ...
    std::uint8_t TShip::CanDropTreasureMap() {
        std::uint8_t Result = false;
        if (pas::class_cast_if<aPirate::TPirate*>(this) != nullptr && static_cast<aPirate::TPirate*>(this)->PirateType == 0 && aPlayer::GetPlayer() != PartnerShip && ScriptShip == nullptr) {
            if (aMyFunction::SeededRandomIntRange(1, 100, Seed) <= 15) {
                return true;
            }
        }
        return Result;
    ...
    std::uint8_t TShip::TryDropTreasureMap() {
        aItem::TItem* Item{};
        aPlanet::TPlanet* Planet{};
        std::uint8_t Result = false;
        if (CanDropTreasureMap() && static_cast<std::uint8_t>(NoDrop ^ 1)) {
            Planet = SelectTreasureMapPlanet();
            if (Planet != nullptr) {
                Item = pas::construct_call<aItem::TTreasureMap>(aItem::TEquipment_Create);
                pas::checked_cast<aItem::TTreasureMap*>(Item)->Init(Planet, this);
                pas::list_add(Inventory, reinterpret_cast<void*>(Item));
                DropCarriedItemAsMovingLoot(Item);
                return true;
            }
    ...
    void TShip::DropGoodsIntoSpace(std::uint8_t Good, std::int32_t Count) {
        aItem::TGoods* Goods = pas::construct_call<aItem::TGoods>(aItem::TItem_Create);
        Goods->Init(static_cast<aConst::TItemType>(Good), Count);
        Goods->Cost = System::Round(static_cast<long double>(GetAverageCargoCost(Good)) * Count);
    ...
    void TShip::QueueMovingItemDrop(aItem::TItem* Item, std::uint8_t UseFlag) {
        ...
        while (Retry) {
            Angle = aMyFunction::HeadingDegreesToRadians(aMyFunction::NextRandomIntRange(0, 360, RandomState));
            Distance = aMyFunction::NextRandomIntRange(100, 200, RandomState);
            Drop->Destination.X = System::Sin(Angle) * Distance + Item->Position.X;
            Drop->Destination.Y = Item->Position.Y - System::Cos(Angle) * Distance;
            Retry = false;
            Count = pas::list_count(CurrentStar->Items);
            for (auto cpp_range = pas::for_to<std::int32_t>(0, Count - 2); cpp_range.next(I); ) {
                OtherItem = pas::list_at<aItem::TItem>(CurrentStar->Items, I);
        ...
        std::int32_t ActionResult = ScriptItemsAct(aConst::satOnDropItem, Item, nullptr, 0);
        if (pas::class_cast_if<aItem::TEquipmentWithActCode*>(Item) != nullptr) {
            ActionResult = aScript::RunItemConfigActionCode(Item, aConst::satOnDropItem, this, Item, nullptr, ActionResult);
        }
        if (Item->ScriptItem != nullptr) {
            ActionResult = reinterpret_cast<aScript::TScriptItem*>(Item->ScriptItem)->RunActionCode(aConst::satOnDropItem, this, Item, nullptr, ActionResult);
        }
        if (ActionResult == 0) {
            pas::list_add(CurrentStar->MovingDropItems, static_cast<void*>(Drop));
            if (!(pas::class_cast_if<aKling::TKling*>(this) != nullptr && pas::class_cast_if<aItem::TProtoplasm*>(Item) != nullptr) && aPlayer::GetPlayer() != this) {
    ...
    void TShip::OptimizeInventory() {
        ...
        while (([&] {
            ...
            return cpp_left > aShip::GetRetainedCapacity(this);
        }())) {
            CostPerWeight = 0.0;
            Best = nullptr;
            for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(Index); ) {
        ...
        while (pas::list_count(Artefacts) > 0 && ([&] {
            ...
            return cpp_left_2 > aShip::GetRetainedCapacity(this);
        }())) {
            CostPerWeight = 0.0;
            Artefact = nullptr;
    ...
    std::int32_t GetRetainedCapacity(TShip* Self) {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Self->Artefacts) - 1); cpp_range_2.next(I); ) {
            Entry = pas::list_at<aItem::TEquipment>(Self->Artefacts, I);
            if (Entry->EquippedFlag != 0 || Entry->NoDropFlag > 0 || Entry->ScriptItem != nullptr && reinterpret_cast<aScript::TScriptItem*>(Entry->ScriptItem)->Name != u"") {
                Result -= Entry->Weight;
            }
        }
    ...
    void TShip::LiquidateInventoryItem(aItem::TItem* Item) {
        if (Item->ItemType == aConst::t_Protoplasm && aPlayer::GetPlayer() != this && (pas::class_cast_if<aRanger::TRanger*>(this) != nullptr && DockedTo != nullptr && DockedTo->TypeId == static_cast<std::uint8_t>(aGalaxyStruct::rstRangerCenter) || DaysSincePlayerSeen > 100 || pas::class_cast_if<aWarrior::TWarrior*>(this) != nullptr && static_cast<aWarrior::TWarrior*>(this)->WarriorType == aWarrior::wtFlagship)) {
            DepositCarriedNodes();
            pas::checked_cast<aNormalShip::TNormalShip*>(this)->TrainSkillsAutomatically();
        } else {
            SetMoney(Money + Item->CalculateResaleValue(GetEffectiveSkillLevel(psTrading, false)));
            pas::list_delete(Inventory, pas::list_indexof(Inventory, reinterpret_cast<void*>(Item)));
            if (pas::class_cast_if<aItem::TWeapon*>(Item) != nullptr && CurrentStar != nullptr) {
                CurrentStar->ClearCombatEventWeaponReferences(Item);
            }
            pas::free(Item);
            RefreshDerivedStats(true);
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
            ...
            if (Item->BrokenFlag == 0 && pas::contains(WearableTypes, static_cast<std::uint8_t>(Item->ItemType))) {
                if (GetDefGenerator() == Item) {
                    ApplyItemDegradation(Item, idkBattle, BaseDurabilityDamage * 1.3L);
                } else if (GetRepairRobot() == Item) {
                    ApplyItemDegradation(Item, idkBattle, BaseDurabilityDamage * 1.25L);
                    ...
                    ApplyItemDegradation(Item, idkBattle, BaseDurabilityDamage * 1.15L);
                } else {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range_2.next(I); ) {
            ...
            if (Artefact->BrokenFlag == 0 && pas::contains(WearableTypes, static_cast<std::uint8_t>(Artefact->ItemType))) {
                ...
                if (cpp_case >= 1 && cpp_case <= 6) {
                    ...
                    ApplyItemDegradation(Artefact, idkBattle, BaseDurabilityDamage * 0.3L);
                } else if (cpp_case == 8) {
                    ApplyItemDegradation(Artefact, idkBattle, BaseDurabilityDamage * 0.6L);
                } else if (cpp_case == 9) {
                    ApplyItemDegradation(Artefact, idkBattle, BaseDurabilityDamage * 0.9L);
                } else if (cpp_case == 10) {
                    ApplyItemDegradation(Artefact, idkBattle, BaseDurabilityDamage * 1.2L);
                }
            }
        }
    }
    ...
    void TShip::ApplyArtefactUseDegradation(double BaseDurabilityDamage) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range.next(I); ) {
            Item = pas::list_at<aItem::TEquipment>(Artefacts, I);
            if (!pas::in_set<8, 8, 10, 22, 26, 34, 39, 41>(Item->ItemType)) {
                Item->Repair();
            }
    ...
    void TShip::ApplyAfterburnerItemDegradation() {
        ...
        aItem::TEquipment* Item{};
        {
            ...
            aItem::TEquipment* engine = GetEngine();
            ApplyItemDegradation(engine, idkAfterburner, afterburnerWear);
        }
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range.next(I); ) {
            Item = pas::list_at<aItem::TEquipment>(Artefacts, I);
            if (Item->ItemType == aConst::t_ArtForsage && Item->EquippedFlag != 0) {
                ApplyItemDegradation(Item, idkAfterburner, 3.0);
                break;
            }
        }
    }

    std::uint8_t TShip::ApplyItemDegradation(aItem::TEquipment* Item, TItemDegradationKind Kind, double DurabilityDamage) {
        std::int32_t AfterScript{};
        std::int32_t AttackCount{};
        std::uint8_t Result = false;
        if (aPlayer::GetPlayer() == this && (aGalaxy::Galaxy->TechnicModEnabled == 1 || aGalaxy::Galaxy->SpecialSimulationMode != 0)) {
            return Result;
        }
        if (Item == nullptr) {
            return Result;
        }
        if (pas::class_cast_if<aItem::TArtefact*>(Item) != nullptr && static_cast<std::uint8_t>(pas::in_set<8, 8, 10, 22, 26, 34, 39, 41>(Item->ItemType) ^ 1)) {
            return Result;
        }
        ...
        if (CanUseEquipmentTech(Item) && GetEffectiveSkillLevel(psTechnical, false) > 0) {
            DurabilityDamage = pas::real_divide(DurabilityDamage, 1.0L + (GetEffectiveSkillLevel(psTechnical, false) & 0x0000007f) * 0.2L);
        }
        ...
        switch (Kind) {
            case idkBattle: {
                AfterScript = ScriptItemsAct(aConst::satOnReduceEqBattle, Item, nullptr, BeforeScript);
                break;
            }
            case idkUse: AfterScript = ScriptItemsAct(aConst::satOnReduceEqUse, Item, nullptr, BeforeScript); break;
            case idkForce: AfterScript = ScriptItemsAct(aConst::satOnReduceEqForce, Item, nullptr, BeforeScript); break;
            case idkAfterburner: {
                AfterScript = ScriptItemsAct(aConst::satOnReduceEqForsage, Item, nullptr, BeforeScript);
                break;
            }
        ...
        if (Item->ConditionPercent < 0.0L && Item->BrokenFlag == 0) {
            if (pas::class_cast_if<aItem::TWeapon*>(Item) != nullptr) {
                if (TypeId == aGalaxyStruct::stWarrior && UsableWeaponCount < 3) {
            ...
            Result = true;
            Item->BrokenFlag = 1;
            if (aPlayer::GetPlayer() == this && Item->EquippedFlag != 0) {
                switch (Kind) {
                    case idkBattle: {
                        ([&] {
            ...
            if (aPlayer::GetPlayer() == this && pas::class_cast_if<aItem::TSatellite*>(Item) != nullptr && static_cast<aItem::TSatellite*>(Item)->TargetPlanet != nullptr) {
                ([&] {
    ...
    void TShip::ImproveRandomEquipment(std::uint8_t ResolveOverload) {
        ...
        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Inventory) - 1); cpp_range_2.next(Index); ) {
            Item = pas::list_at<aItem::TEquipment>(Inventory, Index);
            if (aMyFunction::NextRandomUnitFloat(RandomState) < 0.3L && IsEquipmentUsable(Item) && (Item->EquippedFlag != 0 || Item->ItemType == aConst::t_Hull) && Item->MicroModuleIndex == 0 && BestCost < Item->Cost) {
                BestCost = Item->Cost;
                Best = Item;
            }
        }
    ...
    void TShip::GenerateAndApplyMicroModule(aItem::TEquipment* Item, std::uint8_t ResolveOverload) {
        std::int32_t ModuleIndex{};
        ...
        do {
            ...
            if (aItem::CanInstallMicroModule(ModuleIndex, Item) && aShip::FitsGeneratedModuleCapacity(this, Item, ModuleIndex) && aShip::AcceptDuplicate(this, ModuleIndex)) {
    ...
    void TShip::GenerateExtraWeapon() {
        ...
        aConst::PWeaponInfo Info = aGalaxy::Galaxy->SelectWeaponInfo(RandomState, pas::constant_set<aGalaxyStruct::TWeaponAvailabilityMask>({{0}}), MaximumLevel, MinimumLevel);
        aItem::TWeapon* Weapon = ([&] {
            std::uint8_t ownerId = OwnerId;
            std::int32_t nextRandomIntRange = ([&] {
                ...
                return aMyFunction::NextRandomIntRange(round_2, round, RandomState);
            }());
            std::int32_t nextRandomIntRange_2 = aMyFunction::NextRandomIntRange(MinimumLevel, MaximumLevel, RandomState);
            return aItem::CreateGeneratedWeapon(Info, nextRandomIntRange, nextRandomIntRange_2, ownerId);
        }());
        pas::list_add(Inventory, reinterpret_cast<void*>(Weapon));
        if (aMyFunction::NextRandomUnitFloat(RandomState) < 0.5L) {
            if (aGalaxy::Galaxy->TechLevel > 6 && aPlayer::GetPlayer() != nullptr && aPlayer::GetPlayer()->StrengthInBestRanger > 0.6L && (StrengthInBestRanger < 0.5L || aMyFunction::NextRandomUnitFloat(RandomState) < 0.4L)) {
    ...
    std::uint8_t TShip::IsEssentialInventoryItem(aItem::TItem* Item) {
        ...
        if (Item->ItemType == aConst::t_CargoHook && pas::in_set<1, 1, 3, 3>(TypeId) && GetSlotCount(aConst::sskCargoHook) > 0) {
            return Result;
        }
        return false;
    }
    ...
    std::uint8_t TShip::IsOptionalUtilityEquipment(aItem::TItem* Item) {
        ...
        if (Item != nullptr) {
            if (pas::in_range(static_cast<std::uint8_t>(Item->ItemType), static_cast<std::int32_t>(aConst::t_Radar), static_cast<std::int32_t>(aConst::t_Scaner))) {
                return true;
            } else if (Item->ItemType == aConst::t_CargoHook && static_cast<std::uint8_t>(pas::in_set<1, 1, 3, 3>(TypeId) ^ 1)) {
                return true;
            } else {
                return Result;
            }
    ...
    std::uint8_t TShip::NeedsEssentialEquipment() {
        std::uint8_t Result = true;
        if (Speed > 0 && WeaponCount > 0 && (static_cast<std::uint8_t>(pas::in_set<1, 1, 3, 3>(TypeId) ^ 1) || GetCargoHook() != nullptr || GetSlotCount(aConst::sskCargoHook) <= 0)) {
            return false;
        }
        return Result;
    }
    ...
    void TShip::BuyEquipmentAtLocation(std::uint8_t ForceGeneratedOffers) {
        ...
        std::int32_t OriginalMoney = Money;
        if (CurrentPlanet != nullptr) {
            if (fEquipmentShop::TemporaryShopSlots != nullptr && fEquipmentShop::TemporaryShopPlanet == CurrentPlanet) {
                fEquipmentShop::RestoreTemporaryShopStock();
                RestoreStock = true;
            }
            if (aGalaxy::Galaxy->IsAIShoppingEnabled() && static_cast<std::uint8_t>(ForceGeneratedOffers ^ 1) && (!(pas::class_cast_if<aWarrior::TWarrior*>(this) != nullptr) || pas::checked_cast<aWarrior::TWarrior*>(this)->WarriorType != aWarrior::wtFlagship)) {
                Offers = CurrentPlanet->EquipmentShop;
            } else {
                Offers = CurrentPlanet->BuildEquipmentOfferBatch(this, ForceGeneratedOffers);
                GeneratedBatch = true;
            }
        } else if (DockedTo != nullptr && pas::class_cast_if<aRuins::TRuins*>(DockedTo) != nullptr) {
            if (fEquipmentShop::TemporaryShopSlots != nullptr && fEquipmentShop::TemporaryShopStation == DockedTo) {
                fEquipmentShop::RestoreTemporaryShopStock();
                RestoreStock = true;
            }
            if (aGalaxy::Galaxy->IsAIShoppingEnabled() && static_cast<std::uint8_t>(ForceGeneratedOffers ^ 1) && (!(pas::class_cast_if<aWarrior::TWarrior*>(this) != nullptr) || pas::checked_cast<aWarrior::TWarrior*>(this)->WarriorType != aWarrior::wtFlagship)) {
                Offers = reinterpret_cast<aRuins::TRuins*>(DockedTo)->EquipmentShop;
            } else {
                Offers = reinterpret_cast<aRuins::TRuins*>(DockedTo)->GenerateEquipmentOfferBatch(this, ForceGeneratedOffers);
                GeneratedBatch = true;
            }
        } else if (pas::class_cast_if<aRuins::TRuins*>(this) != nullptr && aGalaxy::Galaxy->IsStationShopUpdateEnabled()) {
            if (fEquipmentShop::TemporaryShopSlots != nullptr && fEquipmentShop::TemporaryShopStation == this) {
                fEquipmentShop::RestoreTemporaryShopStock();
                RestoreStock = true;
            }
            Offers = reinterpret_cast<aRuins::TRuins*>(this)->EquipmentShop;
            UseMoney = false;
        } else {
            if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip != nullptr && CurrentStar == nullptr && CurrentPlanet == nullptr && DockedTo == nullptr) {
                CurrentStar = reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->CurrentStar;
                CurrentPlanet = reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->CurrentPlanet;
                DockedTo = reinterpret_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->DockedTo;
                if (CurrentPlanet != nullptr || DockedTo != nullptr) {
        ...
        std::int32_t I = pas::list_count(Inventory);
        if (UseMoney) {
            while (I > 0) {
                --I;
                OldItem = pas::list_at<aItem::TEquipment>(Inventory, I);
                if (OldItem->ItemType != aConst::t_Hull && OldItem->EquippedFlag != 0 && static_cast<std::uint8_t>(IsEssentialInventoryItem(OldItem) ^ 1) && EvaluateItem(OldItem, 2) <= 0.0L && (OldItem->ScriptItem == nullptr || reinterpret_cast<aScript::TScriptItem*>(OldItem->ScriptItem)->Name == u"") && OldItem->NoDropFlag <= 0) {
                    if (pas::class_cast_if<aItem::TWeapon*>(OldItem) != nullptr) {
                        aShip::TemporarilyUnequipWeapon(reinterpret_cast<aItem::TWeapon*>(OldItem), this, ReplacementWeapon, SavedWeapons, SavedTarget);
                    } else {
                        UnequipSlot(static_cast<std::uint8_t>(OldItem->ItemType), 0);
                    }
                    LiquidateInventoryItem(OldItem);
                    I = pas::list_count(Inventory);
                }
            }
        }
        ReplacementWeapon = aShip::SelectReplacedWeapon(this);
        std::int32_t FreeWeight = CargoFreeSpace - std::max<std::int32_t>(0, GetDesiredCargoFreeSpace() - GetCargoGoodsWeight());
        float BestGain = 0.0f;
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Offers) - 1); cpp_range.next(I); ) {
            ...
            if (pas::class_cast_if<aItem::TWeapon*>(OfferItem) != nullptr) {
                ...
                OldItem = pas::load_unaligned<aItem::TEquipment*>(pas::byte_offset(&reinterpret_cast<PShipEquipmentCacheView>(this)->Slots, (static_cast<std::uint8_t>(OfferItem->ItemType) - 42) * sizeof(aItem::TEquipment*)));
            }
            if (pas::in_range(OfferItem->ItemType, static_cast<std::int32_t>(aConst::t_FuelTanks), static_cast<std::int32_t>(aConst::t_Engine)) && OldItem == nullptr && (static_cast<std::uint8_t>(UseMoney ^ 1) || Money >= OfferItem->Cost)) {
                BestItem = OfferItem;
                break;
            }
            OldScore = 0.0f;
            ...
            if (OldItem != nullptr) {
                ...
                if (pas::class_cast_if<aItem::TWeapon*>(OldItem) != nullptr) {
                    aShip::TemporarilyUnequipWeapon(nullptr, this, ReplacementWeapon, SavedWeapons, SavedTarget);
                ...
                if (TypeId == aGalaxyStruct::stRanger) {
                    pas::Extended cpp_left = AdjustItemEvaluation(OfferItem, 2, NewEffectiveness) * 1.2L;
                    if (cpp_left > AdjustItemEvaluation(OldItem, 2, OldEffectiveness)) {
                        if (Money < OfferItem->Cost) {
                            EquipmentPriceSensitivity = 0.99L * EquipmentPriceSensitivity + 0.01L;
                        } else {
                            EquipmentPriceSensitivity = 0.99L * EquipmentPriceSensitivity;
                        }
                    }
                }
                if (UseMoney && OldItem->CalculateResaleValue(GetEffectiveSkillLevel(psTrading, false)) + Money - OfferItem->Cost < 0) {
                    if (!(pas::class_cast_if<aItem::THull*>(OldItem) != nullptr)) {
                        if (pas::class_cast_if<aItem::TWeapon*>(OldItem) != nullptr) {
                            aShip::RestoreWeapon(this, ReplacementWeapon, SavedWeapons, SavedTarget);
                        } else {
                            EquipItem(OldItem);
                        }
                    }
                ...
                if (FreeWeight - OfferItem->Weight < 0 || UseMoney && Money - OfferItem->Cost < 0) {
                    continue;
                }
                NewEffectiveness = CalculateItemEffectiveness(OfferItem);
            }
        ...
        if (BestItem != nullptr) {
            if (pas::class_cast_if<aItem::TWeapon*>(BestItem) != nullptr) {
                ...
                OldItem = pas::load_unaligned<aItem::TEquipment*>(pas::byte_offset(&reinterpret_cast<PShipEquipmentCacheView>(this)->Slots, (static_cast<std::uint8_t>(BestItem->ItemType) - 42) * sizeof(aItem::TEquipment*)));
            }
            ...
            if (OldItem != nullptr) {
                ...
                if (UseMoney) {
                    SetMoney(OldItem->CalculateResaleValue(GetEffectiveSkillLevel(psTrading, false)) + Money);
                }
                pas::list_delete(Inventory, pas::list_indexof(Inventory, reinterpret_cast<void*>(OldItem)));
                pas::free(OldItem);
            }
            if (UseMoney) {
            ...
            if (pas::class_cast_if<aItem::THull*>(BestItem) != nullptr) {
                pas::list_insert(Inventory, 0, reinterpret_cast<void*>(BestItem));
            } else {
                pas::list_add(Inventory, reinterpret_cast<void*>(BestItem));
            }
            EquipItem(BestItem);
            if (pas::class_cast_if<aItem::THull*>(BestItem) != nullptr) {
                RefreshGraphicSize();
    ...
    aItem::THull* TShip::CreateAndEquipHull(std::uint16_t Capacity, std::uint8_t Level, std::uint8_t Owner, std::int32_t Series, std::uint8_t PirateBuilt) {
        aItem::THull* Item = pas::construct_call<aItem::THull>(aItem::TEquipment_Create);
        std::uint8_t Kind = GetDefaultHullType();
    ...
    std::uint8_t TShip::ScanForCollectableItems() {
        ...
        aItem::TItem* Item{};
        std::uint8_t Result = false;
        if (IsEquipmentUsable(GetCargoHook())) {
    ...
    void TShip::QueueItemsWithinPickupRange() {
        std::int32_t I{};
        aItem::TItem* Item{};
        if (IsEquipmentUsable(GetCargoHook()) && Speed > 0) {
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range.next(I); ) {
                Item = pas::list_at<aItem::TItem>(CurrentStar->Items, I);
                if (IsItemInPickupRange(Item) && static_cast<std::uint8_t>(IsRecentlyDroppedItem(Item) ^ 1) && AcceptPickupItem(Item) && ShouldPickUpItem(Item)) {
    ...
    std::uint8_t TShip::TryCollectBestFloatingItem(std::int32_t MaximumTravelTurns) {
        ...
        std::uint8_t Result = false;
        if (IsEquipmentUsable(GetCargoHook()) && Speed >= 1) {
            Found = false;
            BestItem = nullptr;
            BestDistance = 1.0E+4;
            for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range.next(I); ) {
    ...
    std::uint8_t TShip::IsItemInPickupRange(aItem::TItem* Item) {
        return aItem::CanCargoHookHandleItem(Item, this) && static_cast<long double>(aMyFunction::PointDistanceSquared(Position, Item->Position)) <= GetCargoHookRangeSquared();
    }
    ...
    std::uint8_t TShip::ShouldPickUpItem(aItem::TItem* Item) {
        ...
        Result = false;
        if (pas::in_range(Item->ItemType, static_cast<std::int32_t>(aConst::t_Hull), static_cast<std::int32_t>(aConst::t_DefGenerator)) && IsEquipmentUsable(pas::checked_cast<aItem::TEquipment*>(Item))) {
            for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range.next(I); ) {
        ...
        } else if (pas::in_range(Item->ItemType, static_cast<std::int32_t>(aConst::t_Weapon1), static_cast<std::int32_t>(aConst::t_CustomWeapon)) && IsEquipmentUsable(pas::checked_cast<aItem::TEquipment*>(Item))) {
            EquippedWeight = 0;
            EquippedValue = 0.0f;
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(1, pas::list_count(Inventory) - 1); cpp_range_2.next(I); ) {
                Other = pas::list_at<aItem::TEquipment>(Inventory, I);
    ...
    void TShip::TogglePickupTargets(std::uint8_t IgnoreRange) {
        ...
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range.next(I); ) {
            Item = pas::list_at<aItem::TItem>(CurrentStar->Items, I);
            if (aShip::AcceptPickupTarget(Item, this, IgnoreRange) && static_cast<std::uint8_t>(HasPickupTarget(Item) ^ 1)) {
                AddPickupTarget(Item, false);
                Added = true;
            }
        }
        if (!Added) {
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range_2.next(I); ) {
                Item = pas::list_at<aItem::TItem>(CurrentStar->Items, I);
                if (aShip::AcceptPickupTarget(Item, this, IgnoreRange)) {
    ...
    std::uint8_t AcceptPickupTarget(aItem::TItem* Item, TShip* Self, std::uint8_t& IgnoreRange) {
        std::uint8_t Result = false;
        if (IgnoreRange) {
            if (aItem::CanCargoHookHandleItem(Item, Self)) {
                return true;
            }
            return Result;
        } else if (Self->IsItemInPickupRange(Item)) {
            return true;
        } else {
            return Result;
        }
        ...
        if (Order == soNone) {
            if (!InHyperspace) {
                if (RecordFilm && static_cast<std::uint8_t>(IsOnPlanet() ^ 1) && static_cast<std::uint8_t>(IsDockedToShip() ^ 1)) {
                    aShip::InitializeFilm(255, this, StartStepIndex);
                    if (aPlayer::GetPlayer() == this) {
                ...
                if (OrderStateData <= 0) {
                    ...
                    if (RecordFilm) {
                        if (pas::class_cast_if<SE_Ruins::TRuinsSE*>(FilmObject->SceneObject) != nullptr && reinterpret_cast<SE_Ruins::TRuinsSE*>(FilmObject->SceneObject)->HasTransitionImages) {
                            ...
                            FilmAlpha = -255.0f;
                            FilmAlphaStep = 3.825f;
                            aShip::InitializeFilm(0, this, StartStepIndex);
                        }
                        if (aPlayer::GetPlayer() == this) {
    ...
    void InitializeFilm(std::uint8_t Alpha, TShip* Self, std::int32_t& StartStepIndex) {
        Globals::PrimaryFilm->SetObjectPosition(StartStepIndex, Self->FilmObject, Self->Position);
        Globals::PrimaryFilm->SetObjectAngle(StartStepIndex, Self->FilmObject, aMyFunction::HeadingDegreesToByte(Self->MovementDirection));
        Globals::PrimaryFilm->SetObjectAlpha(StartStepIndex, Self->FilmObject, Alpha);
        Globals::PrimaryFilm->AttachObject(StartStepIndex, Self->FilmObject);
        if (Self->AuxiliaryFilmObject != nullptr) {
            Globals::PrimaryFilm->SetObjectPosition(StartStepIndex, Self->AuxiliaryFilmObject, Self->Position);
            Globals::PrimaryFilm->SetObjectAngle(StartStepIndex, Self->AuxiliaryFilmObject, aMyFunction::HeadingDegreesToByte(Self->MovementDirection));
            Globals::PrimaryFilm->SetObjectAlpha(StartStepIndex, Self->AuxiliaryFilmObject, Self->AuxiliaryFilmObject->SceneObject->GetAlpha());
            Globals::PrimaryFilm->AttachObject(StartStepIndex, Self->AuxiliaryFilmObject);
        }
    }
    ...
    std::uint8_t TShip::ProcessMovementStep(std::int32_t StepIndex, std::uint8_t RecordFilm) {
        ...
        if (Order == soNone) {
            if (FilmObject != nullptr) {
                aShip::AdvancePath(this, StepIndex, RecordFilm, Node, Angle);
            }
        } else if (Order == soMove) {
    ...
    void AdvancePath(TShip* Self, std::int32_t& StepIndex, std::uint8_t& RecordFilm, aPath::PSPathNode& Node, std::uint8_t& Angle) {
        if (Self->MovementPath != nullptr && Self->MovementPath->ActiveHead != nullptr) {
            Node = Self->MovementPath->ActiveHead;
            if (RecordFilm) {
                Angle = aMyFunction::HeadingDegreesToByte(Node->Heading);
                if (aMyFunction::HeadingDegreesToByte(Self->MovementDirection) != Angle) {
                    Globals::PrimaryFilm->SetObjectAngle(StepIndex, Self->FilmObject, Angle);
                }
            }
            Self->Position = Node->Position;
            Self->MovementDirection = Node->Heading;
            Self->MovementPath->RemoveNode(Self->MovementPath->ActiveHead);
        }
        if (RecordFilm) {
            Globals::PrimaryFilm->SetObjectPosition(StepIndex, Self->FilmObject, Self->Position);
            if (Self->AuxiliaryFilmObject != nullptr) {
                Globals::PrimaryFilm->SetObjectPosition(StepIndex, Self->AuxiliaryFilmObject, Self->Position);
            }
            if (aPlayer::GetPlayer() == Self) {
                Globals::PrimaryFilm->SetRadarCenter(StepIndex, Self->Position);
            }
            if (Self->FilmAlphaStep != 0.0L) {
                Self->FilmAlpha = static_cast<long double>(Self->FilmAlpha) + Self->FilmAlphaStep;
                if (Self->FilmAlpha <= 0.0L) {
                    Globals::PrimaryFilm->SetObjectAlpha(StepIndex, Self->FilmObject, 0);
                } else if (Self->FilmAlpha >= 255.0L) {
                    Globals::PrimaryFilm->SetObjectAlpha(StepIndex, Self->FilmObject, 255);
                } else {
                    Globals::PrimaryFilm->SetObjectAlpha(StepIndex, Self->FilmObject, System::Round(Self->FilmAlpha));
                }
            }
        }
    }
            ...
            if (CurrentStar->Id != 71 && CurrentStar->Id != 72) {
                for (auto cpp_range = pas::for_to<std::int32_t>(1, pas::list_count(aGalaxy::Galaxy->Stars) - 1); cpp_range.next(I); ) {
                    ...
                    if (Star->Status.ControlFaction == aGalaxyStruct::sfDominators && Star->Status.Battle == 0 && Star->Status.CustomFaction == u"" && (Star->Status.DominatorSeries == aGalaxyStruct::dsKeller || aMyFunction::NextRandomUnitFloat(RandomState) >= 0.8L && PowerFactor >= 2.0L)) {
                        ...
                        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Star->Ships) - 1); cpp_range_2.next(J); ) {
                            Ship = pas::list_at<TShip>(Star->Ships, J);
                            if (Ship->InNormalSpace() && pas::class_cast_if<aKling::TKling*>(Ship) != nullptr && static_cast<std::uint8_t>(Ship->HasIndependentScriptFaction() ^ 1) && pas::in_set<0, 1>(Ship->Order) && pas::in_range(static_cast<aKling::TKling*>(Ship)->KlingType, static_cast<std::int32_t>(aGalaxyStruct::ktEquentor), static_cast<std::int32_t>(aGalaxyStruct::ktShtip))) {
                                ++Available;
                            }
                        }
                        if (Available <= 1) {
    ...
    std::uint8_t TShip::HasMatchingArtefactOrCustomItem(aItem::TItem* Item) {
        ...
        if (Item->ItemType == aConst::t_UselessItem) {
            ...
            for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range_2.next(I); ) {
                ...
                if (Item->ItemType == Other->ItemType && (static_cast<std::uint8_t>(pas::in_set<8, 9>(Other->ItemType) ^ 1) || Other->ConfigBlockName == reinterpret_cast<aItem::TEquipment*>(Item)->ConfigBlockName)) {
                    return true;
                }
            }
        }
    ...
    std::int32_t TShip::CountActiveArtefacts(std::uint8_t ArtefactType) {
        ...
        if (pas::in_set<14, 15, 20, 20, 39, 39>(ArtefactType) && ArtefactType != static_cast<std::uint8_t>(aConst::t_ArtArtefactor)) {
            AllActive = CountActiveArtefacts(aConst::t_ArtArtefactor) > 0 && (static_cast<std::uint8_t>(pas::in_set<1, 1, 3, 3>(aCalc::TurnCalculationPhase) ^ 1) || CountActiveArtefacts(aConst::t_ArtArtefactor) > 1);
        }
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(Artefacts) - 1); cpp_range.next(I); ) {
            Item = pas::list_at<aItem::TArtefact>(Artefacts, I);
    ...
    std::uint8_t TShip::CanBoostArtefact(std::uint8_t ArtefactType, aItem::TEquipment* Item, std::uint8_t IgnoreArtefactAvailability) {
        ...
        if (Item == nullptr) {
            switch (ArtefactType) {
                case aConst::t_ArtefactHull: Result = aShip::IsArtefactBoostEquipment(GetHull()); break;
                case aConst::t_ArtefactFuel: Result = aShip::IsArtefactBoostEquipment(GetFuelTanks()); break;
                case aConst::t_ArtefactSpeed: Result = aShip::IsArtefactBoostEquipment(GetEngine()); break;
                case aConst::t_ArtefactPower: {
                    Result = aShip::IsArtefactBoostEquipment(GetEngine()) || aShip::IsArtefactBoostEquipment(GetHull());
                    break;
                }
                case aConst::t_ArtefactRadar: Result = aShip::IsArtefactBoostEquipment(GetRadar()); break;
                case aConst::t_ArtefactScaner: Result = aShip::IsArtefactBoostEquipment(GetScanner()); break;
                case aConst::t_ArtefactDroid: Result = aShip::IsArtefactBoostEquipment(GetRepairRobot()); break;
                case aConst::t_ArtefactHook: Result = aShip::IsArtefactBoostEquipment(GetCargoHook()); break;
                case aConst::t_ArtefactDef: Result = aShip::IsArtefactBoostEquipment(GetDefGenerator()); break;
                case aConst::t_ArtefactAntigrav: Result = aShip::IsArtefactBoostEquipment(GetHull()); break;
                case aConst::t_ArtDefToEnergy: Result = aShip::IsArtefactBoostEquipment(GetDefGenerator()); break;
                case aConst::t_ArtGiperJump: Result = aShip::IsArtefactBoostEquipment(GetEngine()); break;
                case aConst::t_ArtDefToArms1: Result = aShip::IsArtefactBoostEquipment(GetDefGenerator()); break;
                case aConst::t_ArtForsage: Result = aShip::IsArtefactBoostEquipment(GetEngine()); break;
                case aConst::t_ArtWeaponToSpeed: {
                    Result = aShip::IsArtefactBoostEquipment(GetEngine()) || aShip::IsArtefactBoostEquipment(GetHull());
                    break;
                }
                case aConst::t_ArtEnergyDef: Result = aShip::IsArtefactBoostEquipment(GetHull()); break;
                case aConst::t_ArtMissileDef: Result = aShip::IsArtefactBoostEquipment(GetHull()); break;
                case aConst::t_ArtefactMiniExpl: Result = aShip::IsArtefactBoostEquipment(GetScanner()); break;
                case aConst::t_ArtPDTurret: Result = aShip::IsArtefactBoostEquipment(GetRadar()); break;
            }
            if (ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtefactNano)) {
                if (aShip::IsArtefactBoostEquipment(GetEngine()) || aShip::IsArtefactBoostEquipment(GetFuelTanks()) || aShip::IsArtefactBoostEquipment(GetRadar()) || aShip::IsArtefactBoostEquipment(GetScanner()) || aShip::IsArtefactBoostEquipment(GetRepairRobot()) || aShip::IsArtefactBoostEquipment(GetCargoHook()) || aShip::IsArtefactBoostEquipment(GetDefGenerator()) || aShip::IsArtefactBoostEquipment(Weapons[1]) || aShip::IsArtefactBoostEquipment(Weapons[2]) || aShip::IsArtefactBoostEquipment(Weapons[3]) || aShip::IsArtefactBoostEquipment(Weapons[4]) || aShip::IsArtefactBoostEquipment(Weapons[5])) {
                    Result = true;
                }
            }
            if (ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtDefToEnergy) || ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtEnergyPulse)) {
                if (aShip::IsArtefactBoostEquipment(Weapons[1]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[1]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy) || aShip::IsArtefactBoostEquipment(Weapons[2]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[2]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy) || aShip::IsArtefactBoostEquipment(Weapons[3]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[3]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy) || aShip::IsArtefactBoostEquipment(Weapons[4]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[4]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy) || aShip::IsArtefactBoostEquipment(Weapons[5]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[5]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkEnergy)) {
                    Result = true;
                }
            }
            if (ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtSplinter) || ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtDecelerate)) {
                if (aShip::IsArtefactBoostEquipment(Weapons[1]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[1]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter) || aShip::IsArtefactBoostEquipment(Weapons[2]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[2]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter) || aShip::IsArtefactBoostEquipment(Weapons[3]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[3]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter) || aShip::IsArtefactBoostEquipment(Weapons[4]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[4]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter) || aShip::IsArtefactBoostEquipment(Weapons[5]) && pas::contains(pas::load_unaligned<aGalaxyStruct::TDamageFlagSet>(&Weapons[5]->GetWeaponInfo()->DamageFlags), aGalaxyStruct::dkSplinter)) {
                    Result = true;
                }
            }
            if (ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtFastRacks)) {
                if (aShip::IsArtefactBoostEquipment(Weapons[1]) && pas::in_set<5, 7>(Weapons[1]->GetWeaponInfo()->ShotType) || aShip::IsArtefactBoostEquipment(Weapons[2]) && pas::in_set<5, 7>(Weapons[2]->GetWeaponInfo()->ShotType) || aShip::IsArtefactBoostEquipment(Weapons[3]) && pas::in_set<5, 7>(Weapons[3]->GetWeaponInfo()->ShotType) || aShip::IsArtefactBoostEquipment(Weapons[4]) && pas::in_set<5, 7>(Weapons[4]->GetWeaponInfo()->ShotType) || aShip::IsArtefactBoostEquipment(Weapons[5]) && pas::in_set<5, 7>(Weapons[5]->GetWeaponInfo()->ShotType)) {
                    return true;
                }
            }
            return Result;
        ...
        if (!aShip::IsArtefactBoostEquipment(Equipment)) {
            return Result;
        }
        switch (ArtefactType) {
        ...
        if (ArtefactType == static_cast<std::uint8_t>(aConst::t_ArtFastRacks) && !(pas::class_cast_if<aItem::TWeapon*>(Equipment) != nullptr && pas::in_set<5, 7>(reinterpret_cast<aItem::TWeapon*>(Equipment)->GetWeaponInfo()->ShotType))) {
            return Result;
        }
    ...
    void TShip::ApplyNanoArtefactRepair() {
        ...
        Item = nullptr;
        if (EquippedCount > 0) {
            aShip::SelectRepairable(EquippedCount, true, this, Item);
        }
        if (Item == nullptr && UnequippedCount > 0) {
            aShip::SelectRepairable(UnequippedCount, false, this, Item);
        }
        if (Item != nullptr) {
    ...
    void SelectRepairable(std::int32_t Count, std::uint8_t Equipped, TShip* Self, aItem::TEquipment*& Item) {
        ...
        do {
            ...
            aShip::SelectOrdinal(aMyFunction::SeededRandomIntRange(1, Count, aGalaxy::Galaxy->GenerationSeed + static_cast<std::uint32_t>(aGalaxy::Galaxy->CurrentTurn) + static_cast<std::uint32_t>(Attempt)), Equipped, Self, Item);
            if (Item == nullptr) {
                return;
            }
        } while (!(pas::contains(RepairableTypes, static_cast<std::uint8_t>(Item->ItemType)) && Item->ConditionPercent <= 6.0E+1L && !(pas::class_cast_if<aItem::TWeapon*>(Item) != nullptr && reinterpret_cast<aItem::TWeapon*>(Item)->GetWeaponInfo()->Availability == aGalaxyStruct::waNotSoldAndNodeRepair) && Self->CanRepairEquipmentTech(Item)));
    }
    ...
    pas::WideString TShip::LookupTalkText(const pas::WideString& Path) {
        pas::WideString Result{};
        std::int32_t Count{};
        pas::WideString Key{};
        ...
        if (pas::pos(u"Talk.", Path) == 1) {
            if (aPlayer::GetPlayer() != this) {
                Key = EC_Str::ReplaceAllWideString(Path, u"Talk."_wref.get(), aShip::GetTalkContextPrefix(this));
            } else if (Globals::TalkShip != nullptr) {
                Key = EC_Str::ReplaceAllWideString(Path, u"Talk."_wref.get(), aShip::GetTalkContextPrefix(Globals::TalkShip));
            } else {
                Key = Path;
            }
        } else {
        ...
        if (Count == 1) {
            ...
            Count = aMyFunction::SeededRandomIntRange(0, Count - 1, (aGalaxy::Galaxy->CurrentTurn + static_cast<std::int32_t>(Seed)) / 10);
            Result = Variants[Count];
        }
        if (HomePlanet != nullptr) {
            Result = aMyFunction::ReplaceColoredToken(Result, u"<HomePlanet>"_w, HomePlanet->Name, u"<color=255,240,100>"_w);
        }
        Result = ([&] {
        ...
        if (aPlayer::GetPlayer() == this && Globals::TalkShip != nullptr) {
    ...
    std::uint8_t TShip::OpenPlayerConversation(std::uint8_t RespectChameleon) {
        std::uint8_t Result{};
        pas::Array<WindowsImports::THandle, 0, 1> Handles{};
        if (GR_Main::ExitScreenLoop || static_cast<std::uint8_t>(aPlayer::GetPlayer()->InNormalSpace() ^ 1) || aPlayer::GetPlayer()->CurrentStar != CurrentStar || pas::in_set<0, 0, 2, 2, 4, 4, 6, 6>(aCalc::TurnCalculationPhase) || GlobalsV::CurrentScreenId != GlobalsV::screenStarMap || aGalaxy::Galaxy->SpecialSimulationMode != 0) {
            return false;
        }
        if ((static_cast<std::uint8_t>(RespectChameleon) & static_cast<std::uint8_t>(aPlayer::GetPlayer()->ChameleonActive)) != 0 && IsPlayerChameleonEffectiveAgainstSelf() && (ScriptShip == nullptr || static_cast<std::uint8_t>(HasScriptControl() ^ 1))) {
            return false;
        }
        Globals::TalkShip = this;
    ...
    std::uint8_t TShip::ShowPlayerDialogue(std::uint8_t Kind, const pas::WideString& Text, std::int32_t Amount) {
        ...
        std::uint8_t Result = 0;
        if (OpenPlayerConversation(true)) {
    ...
    void TShip::NotifyMoneyDemand(TShip* OtherShip, pas::WideString Response, std::int32_t Amount) {
        ...
        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", OtherShip->GetFullName(u" "_wref.get()), u"</color>"});
        Request = pas::concat_wide({u"- ", ([&] {
            pas::WideString lookupTalkText = LookupTalkText(u"Talk.Money.Send"_wref.get());
    ...
    void TShip::NotifyCargoDemand(TShip* OtherShip, pas::WideString Response) {
        ...
        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", OtherShip->GetFullName(u" "_wref.get()), u"</color>"});
        Request = pas::concat_wide({u"- ", aMyFunction::WrapTextInColor(LookupTalkText(u"Talk.Goods.Send"_wref.get()), pas::WideString())});
        Response = pas::concat_wide({u"- ", Response});
        {
            Globals::TMessagePlayer* cpp_with = Globals::AddOrUpdatePlayerBubble(1, aGalaxy::Galaxy->CurrentTurn, pas::concat_wide({Header, u"\r\n", Request, u"\r\n", Response}), u""_wref.get());
            cpp_with->Targets[0].ShipId = Id;
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            ...
            if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(OtherShip->Position, aPlayer::GetPlayer()->Position)) {
                if ((aPlayer::GetPlayer() != OtherShip || static_cast<std::uint8_t>(aPlayer::GetPlayer()->ChameleonActive ^ 1) || static_cast<std::uint8_t>(IsPlayerChameleonEffectiveAgainstSelf() ^ 1)) && static_cast<std::uint8_t>(NoTalk ^ 1) && static_cast<std::uint8_t>(OtherShip->NoTalk ^ 1)) {
                    Request = LookupVisibleTalkText(u"Talk.DropGoodsInFear.Drop"_wref.get(), OtherShip);
    ...
    void TShip::NotifyTruceOffer(TShip* OtherShip, pas::WideString Response, std::int32_t Amount) {
        ...
        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", OtherShip->GetFullName(u" "_wref.get()), u"</color>"});
        Request = pas::concat_wide({u"- ", ([&] {
            pas::WideString lookupTalkText = LookupTalkText(pas::concat_wide({u"Talk.Truce.", GetTypeNameKey(), u"Send"}));
            pas::WideString intToStr = pas::wide_int_to_str(Amount);
            return aMyFunction::ReplaceColoredToken(std::move(lookupTalkText), u"<Money>"_w, std::move(intToStr), u"<color=255,240,100>"_w);
        }())});
        Response = pas::concat_wide({u"- ", Response});
    ...
    void TShip::NotifyAttackRequest(TShip* OtherShip, pas::WideString Response, TShip* Target) {
        ...
        Request = pas::concat_wide({u"- ", ([&] {
            ...
            pas::WideString lookupTalkText = LookupTalkText(pas::concat_wide({u"Talk.Attack.", GetTypeNameKey(), u"Send"}));
    ...
    void TShip::NotifyPartnershipOffer(TShip* OtherShip, pas::WideString Response, std::int32_t Amount) {
        ...
        Request = pas::concat_wide({u"- ", ([&] {
            pas::WideString intToStr = pas::wide_int_to_str(Amount);
            pas::WideString lookupTalkText = LookupTalkText(u"Talk.Partner.Send"_wref.get());
    ...
    void TShip::NotifyPartnerBreak(TShip* Leader) {
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Partner.MateBreak"_wref.get());
                    TShip* self = this;
                    self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerBreak, lookupTalkText, 0);
                }
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", Leader->GetFullName(u" "_wref.get()), u"</color>"});
                        Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.MateBreak"_wref.get())});
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.AnswerLiderBreak"_wref.get())});
                        if (Leader->NoTalk) {
            ...
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    ...
                    if (cpp_left > aPlayer::GetPlayer()->CountWingmen()) {
                        const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Partner.MateTheEnd"_wref.get());
                        TShip* self = this;
                        self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerEnd, lookupTalkText, 0);
                    } else {
                        const pas::WideString& lookupTalkText_2 = LookupTalkText(u"Talk.Partner.MateTheEndLowLeadership"_wref.get());
                        TShip* self_2 = this;
                        self_2->ShowPlayerDialogue(aGalaxyStruct::tkPartnerEnd, lookupTalkText_2, 0);
                    }
                }
                aPlayer::GetPlayer()->ScriptItemsAct(aConst::satOnShipTalkedWithPlayer, this, nullptr, 0);
            } else {
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        ...
                        if (aPlayer::GetPlayer() != Leader || ([&] {
                            ...
                            Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.MateTheEnd"_wref.get())});
                        } else {
                            Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.MateTheEndLowLeadership"_wref.get())});
                        }
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.AnswerLiderTheEnd"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
                        {
    ...
    void TShip::NotifyPartnerRebellion(TShip* Leader) {
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Partner.MateRiot"_wref.get());
                    TShip* self = this;
                    self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerRiot, lookupTalkText, 0);
                }
                aPlayer::GetPlayer()->ScriptItemsAct(aConst::satOnShipTalkedWithPlayer, this, nullptr, 0);
            } else {
                RadarSquared = pas::sqr(aPlayer::GetPlayer()->GetRadarRange());
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", Leader->GetFullName(u" "_wref.get()), u"</color>"});
                        Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.MateRiot"_wref.get())});
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Partner.AnswerLiderRiot"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
                        {
    ...
    void TShip::NotifyPiratePartnerRelationBreak(TShip* Leader) {
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Pirate.MateBreakRelation"_wref.get());
                    TShip* self = this;
                    self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerBreak, lookupTalkText, 0);
                }
                aPlayer::GetPlayer()->ScriptItemsAct(aConst::satOnShipTalkedWithPlayer, this, nullptr, 0);
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", Leader->GetFullName(u" "_wref.get()), u"</color>"});
                        Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.MateBreakRelation"_wref.get())});
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.AnswerLiderBreak"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
                        {
    ...
    void TShip::NotifyPiratePartnerRatingBreak(TShip* Leader) {
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Pirate.MateBreakRating"_wref.get());
                    TShip* self = this;
                    self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerBreak, lookupTalkText, 0);
                }
                aPlayer::GetPlayer()->ScriptItemsAct(aConst::satOnShipTalkedWithPlayer, this, nullptr, 0);
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", Leader->GetFullName(u" "_wref.get()), u"</color>"});
                        Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.MateBreakRating"_wref.get())});
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.AnswerLiderBreak"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
            ...
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    std::int32_t cpp_left = aPlayer::GetPlayer()->GetEffectiveSkillLevel(psLeadership, false);
                    if (cpp_left > aPlayer::GetPlayer()->CountWingmen()) {
                        const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Pirate.MateTheEnd"_wref.get());
                        TShip* self = this;
                        self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerEnd, lookupTalkText, 0);
                    } else {
                        const pas::WideString& lookupTalkText_2 = LookupTalkText(u"Talk.Pirate.MateTheEndLowLeadership"_wref.get());
                        TShip* self_2 = this;
                        self_2->ShowPlayerDialogue(aGalaxyStruct::tkPartnerEnd, lookupTalkText_2, 0);
                    }
                }
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        ...
                        if (aPlayer::GetPlayer() != Leader || ([&] {
                            ...
                            Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.MateTheEnd"_wref.get())});
                        } else {
                            Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.MateTheEndLowLeadership"_wref.get())});
                        }
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.AnswerLiderTheEnd"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
                        {
    ...
    void TShip::NotifyPiratePartnerRebellion(TShip* Leader) {
        ...
        if (aPlayer::GetPlayer()->CurrentStar == CurrentStar && aPlayer::GetPlayer()->InNormalSpace()) {
            if (aPlayer::GetPlayer() == Leader && static_cast<std::uint8_t>(aRanger::PlayerAutomaticControl ^ 1) && static_cast<std::uint8_t>(Leader->NoTalk ^ 1)) {
                {
                    const pas::WideString& lookupTalkText = LookupTalkText(u"Talk.Pirate.MateRiot"_wref.get());
                    TShip* self = this;
                    self->ShowPlayerDialogue(aGalaxyStruct::tkPartnerRiot, lookupTalkText, 0);
                }
                aPlayer::GetPlayer()->ScriptItemsAct(aConst::satOnShipTalkedWithPlayer, this, nullptr, 0);
                ...
                if (static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Position, aPlayer::GetPlayer()->Position) || static_cast<long double>(RadarSquared) >= aMyFunction::PointDistanceSquared(Leader->Position, aPlayer::GetPlayer()->Position)) {
                    if (static_cast<std::uint8_t>(NoTalk ^ 1) && (static_cast<std::uint8_t>(Leader->NoTalk ^ 1) || aPlayer::GetPlayer() == Leader)) {
                        Header = pas::concat_wide({u"<color=255,240,100>", GetFullName(u" "_wref.get()), u"</color>", LookupTalkText(u"Talk.To"_wref.get()), u"<color=255,240,100>", Leader->GetFullName(u" "_wref.get()), u"</color>"});
                        Request = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.MateRiot"_wref.get())});
                        Response = pas::concat_wide({u"\r\n- ", LookupTalkText(u"Talk.Pirate.AnswerLiderRiot"_wref.get())});
                        if (Leader->NoTalk) {
                            Response = pas::WideString();
                        }
                        {
    ...
    std::int32_t TShip::CalculatePartnershipMonths(std::int32_t Amount, TShip* OtherShip) {
        ...
        std::int32_t Result = System::Round(cpp_left * aMyFunction::RemapClamped(RelationToShip(OtherShip) & 0x0000007f, 5.0E+1, 1.0E+2, 0.7, 1.5));
    ...
    pas::WideString TShip::GetGreetingText() {
        pas::WideString Result{};
        pas::WideString Key{};
        std::uint8_t Series{};
        ...
        if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr) {
            ...
            if (aPlayer::GetPlayer() == PartnerShip) {
                ...
                if (GetRelationLevelToShip(aPlayer::GetPlayer()) < aGalaxyStruct::rlNormal) {
                    Key = u"ShipGreetings.Standart.FemaleChameleonWar"_w;
                } else {
                    Key = u"ShipGreetings.Standart.FemaleChameleon"_w;
                }
            } else {
                Result = pas::checked_cast<aNormalShip::TNormalShip*>(this)->SelectSituationalMessage(false);
                if (Result != u"") {
                    return Result;
                }
                switch (static_cast<std::int32_t>(GetRelationLevelToShip(aPlayer::GetPlayer())) & 0x0000007f) {
        ...
        } else if (aPlayer::GetPlayer() == PartnerShip) {
            if (Chameleon && TypeId != aGalaxyStruct::stPirate) {
                Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"PartnerChameleon"});
            } else {
                Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Partner"});
            }
        } else if (pas::class_cast_if<aNormalShip::TNormalShip*>(this) != nullptr) {
            if (Chameleon && TypeId == aGalaxyStruct::stPirate && GetRelationLevelToShip(aPlayer::GetPlayer()) < aGalaxyStruct::rlNormal) {
                Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"ChameleonWar"});
            } else if (Chameleon && (TypeId == aGalaxyStruct::stPirate || IsPlayerChameleonEffectiveAgainstSelf())) {
                Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Chameleon"});
            } else {
                if (LiberationGroup != nullptr && GetRelationLevelToShip(aPlayer::GetPlayer()) > aGalaxyStruct::rlHostile) {
                    Result = pas::checked_cast<aGroup::TGroup*>(LiberationGroup)->GetShipGreeting(this);
                    if (Result != u"") {
                        return Result;
                    }
                }
                ...
                switch (static_cast<std::int32_t>(GetRelationLevelToShip(aPlayer::GetPlayer())) & 0x0000007f) {
                    case 0: Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"War"}); break;
                    case 1: Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Bad"}); break;
                    case 2: Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Normal"}); break;
                    case 3: Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Good"}); break;
                    case 4: Key = pas::concat_wide({u"ShipGreetings.Standart.", GetTypeNameKey(), u"Best"}); break;
                }
            }
        } else if (pas::class_cast_if<aKling::TKling*>(this) != nullptr) {
            if (static_cast<std::uint8_t>(aPlayer::GetPlayer()->HasProgram(aGalaxyStruct::prgIntercom) ^ 1) || static_cast<std::uint8_t>(aPlayer::GetPlayer()->CanResolveObjectWithScanner(this) ^ 1)) {
                Key = u"ShipGreetings.Dominator.Rnd"_w;
            } else if (pas::checked_cast<aKling::TKling*>(this)->ActiveProgramAppliedTurn > 0) {
                Key = u"ShipGreetings.Dominator.ProgrammRun"_w;
            } else {
                Series = static_cast<std::uint8_t>(pas::checked_cast<aKling::TKling*>(this)->DominatorSeries);
                switch (Series) {
                    case 0: {
        ...
        if (pas::class_cast_if<aKling::TKling*>(this) != nullptr && static_cast<aKling::TKling*>(this)->ActiveProgramAppliedTurn > 0) {
            aMyFunction::ReplaceTextToken(Result, u"<Name>"_w, (static_cast<void>(aPlayer::GetPlayer()), aRanger::TRanger::GetProgramName(pas::checked_cast<aKling::TKling*>(this)->ActiveProgramId)), u"<color=255,240,100>"_w);
        }
        return Result;
    }
                        ...
                        for (auto cpp_range_2 = pas::for_to<std::int32_t>(0, BindingCount - 1); cpp_range_2.next(BindingIndex); ) {
                            ...
                            if (reinterpret_cast<TShip*>(reinterpret_cast<std::uint8_t*>(this) + 0)->CurrentStar == OtherBinding->Ship->CurrentStar && OtherBinding->Ship->InNormalSpace()) {
                                ...
                                if (GroupIndex < GroupCount) {
                                    ...
                                    if (static_cast<long double>(pas::sqr(GetWeaponRange(Weapon))) >= Distance && Distance < BestDistance) {
                                        BestDistance = Distance;
    ...
    void TShip::UpdateScriptStateCompletionAndPickups() {
        ...
        if (InNormalSpace() && IsEquipmentUsable(GetCargoHook())) {
            ...
            if (State->PickUpNearbyItems) {
                for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CurrentStar->Items) - 1); cpp_range.next(I); ) {
                    Item = pas::list_at<aItem::TItem>(CurrentStar->Items, I);
                    if (ShouldPickUpItem(Item) && IsItemInPickupRange(Item)) {
                        AddPickupTarget(Item, false);
                    }
                }
            }
    ...
    std::int32_t TShip::CalculateSpeed() {
        ...
        if (aPlayer::GetPlayer() == this) {
            if (!CanUseEquipmentTech(GetFuelTanks())) {
                return Result;
            }
            if (!CanUseEquipmentTech(GetEngine())) {
                return Result;
            }
        }
        ...
        if (AfterburnerActive && IsEquipmentUsable(GetEngine()) && GetSlotCount(aConst::sskAfterburner) > 0) {
            CombinedFactor = static_cast<long double>(CombinedFactor) * aConst::AfterburnerSpeedFactor;
        }
        if (IsHealthEffectActive(17)) {
            CombinedFactor = CombinedFactor * 1.3L;
        }
        double NewSpeed = CalculateEngineSpeed(GetEngine(), false);
    ...
    std::uint8_t IsBetterInterceptorTarget(TShip* Current, TShip* Candidate, aItem::TInterceptorTargetingStrategy Strategy, TShip* Self) {
        switch (Strategy) {
            ...
            case aItem::itsFewestHullPoints: return Candidate->GetHull()->HullPoints < Current->GetHull()->HullPoints;
            ...
            case aItem::itsNearest: {
                return aMyFunction::PointDistanceSquared(Candidate->Position, Self->Position) < aMyFunction::PointDistanceSquared(Current->Position, Self->Position);
            }
    ...
    std::uint8_t TShip::HasNoUsableWeapons() {
        ...
        std::uint8_t Result = true;
        {
            ...
            if (1 <= cpp_last) {
                for (I = 1; I <= cpp_last; ++I) {
                    if (Weapons[I]->EquippedFlag != 0 && (static_cast<std::uint8_t>(pas::in_range(Weapons[I]->GetWeaponInfo()->ShotType, static_cast<std::int32_t>(aGalaxyStruct::wstTorpedo), static_cast<std::int32_t>(aGalaxyStruct::wstRocket)) ^ 1) || Weapons[I]->Ammo > 0) && IsEquipmentUsable(Weapons[I])) {
                        Result = false;
                        break;
                    }
                }
    ...
    void TShip::DecayCombatStatusEffects() {
        ...
        while (I >= 0) {
            ...
            switch (Entry->EffectType) {
                case cseShock: Amount = GetShockStatusDecay(Entry->Strength); break;
    ...
    float TShip::GetShockStatusDecay(float Strength) {
        std::int32_t I{};
        float Result = 0.1L * Strength + 5.0L;
        for (auto cpp_range = pas::for_to<std::int32_t>(1, CountActiveArtefacts(aConst::t_ArtefactHull)); cpp_range.next(I); ) {
            Result = Result * (aConst::HullArtefactStatusDecayFactor + static_cast<long double>(static_cast<std::int8_t>(CanBoostArtefact(aConst::t_ArtefactHull, nullptr, false))) * aConst::HullArtefactBoostStatusDecay);
        }
        if (TypeId == aGalaxyStruct::stKling && pas::checked_cast<aKling::TKling*>(this)->KlingType == 0) {
            return Result * 2.0L;
        }
    ...
    pas::WideString TShip::GetCombatStatusDescription(std::int32_t& Count, std::uint8_t ShowStrength) {
        ...
        if (CombatStatusEffects != nullptr && pas::list_count(CombatStatusEffects) > 0) {
            ...
            if (Strength >= 1) {
                aShip::AppendStatusLine(u"FormInfo.ISEBWBuff"_w, Count, ShowStrength, Result, Strength);
            }
            Strength = System::Round(GetCombatStatusStrength(cseAcid));
            if (Strength >= 1) {
                aShip::AppendStatusLine(u"FormInfo.ISEAcid"_w, Count, ShowStrength, Result, Strength);
            }
            Strength = System::Round(GetCombatStatusStrength(cseShock));
            if (Strength >= 1) {
                aShip::AppendStatusLine(u"FormInfo.ISECharged"_w, Count, ShowStrength, Result, Strength);
            }
            Strength = System::Round(GetCombatStatusStrength(cseMagnetic));
            if (Strength >= 1) {
                aShip::AppendStatusLine(u"FormInfo.ISEMagnetic"_w, Count, ShowStrength, Result, Strength);
            }
        }
        for (auto cpp_range = pas::for_to<std::int32_t>(0, pas::list_count(CustomShipInfos) - 1); cpp_range.next(I); ) {
            ...
            if (Info->StatusEffect && static_cast<std::uint8_t>(Info->DeleteQueued ^ 1)) {
                ...
                if (Strength >= 1) {
                    aShip::AppendStatusLine(pas::concat_wide({u"ShipInfo.AddInfo.CustomInfos.", Info->TypeName, u".StatusEffect"}), Count, ShowStrength, Result, Strength);
                }
            }
        }
        return Result;
    ...
    void TShip::RefreshCurrentStanding() {
    ...
    std::uint8_t TShip::CanDock(TShip* Ship) {
        return false;
    }
    ...
    std::uint8_t TShip::RelationToShip(TShip* Ship) {
        if (Ship == this) {
            return 100;
        }
        if (pas::class_cast_if<aTranclucator::TTranclucator*>(this) != nullptr && static_cast<aTranclucator::TTranclucator*>(this)->OwnerShip != nullptr) {
            return pas::checked_cast<aTranclucator::TTranclucator*>(this)->OwnerShip->RelationToShip(Ship);
        }
        if (aGalaxy::Galaxy->SpecialSimulationMode != 0) {
            if (aPlayer::GetPlayer() == Ship || aPlayer::GetPlayer() == this) {
                return 100;
            }
            return 0;
        }
        std::uint8_t Result = 0;
        if (Ship == EnemyShip || Ship->EnemyShip == this || (CurrentStanding == aGalaxyStruct::ssDominator) != (Ship->CurrentStanding == aGalaxyStruct::ssDominator) || (CurrentStanding == aGalaxyStruct::ssCustom) != (Ship->CurrentStanding == aGalaxyStruct::ssCustom)) {
            return Result;
        }
        if (CurrentStanding == aGalaxyStruct::ssCustom) {
            if (ScriptShip != nullptr && Ship->ScriptShip != nullptr && reinterpret_cast<aScript::TScriptShip*>(ScriptShip)->StateText == reinterpret_cast<aScript::TScriptShip*>(Ship->ScriptShip)->StateText) {
                return 100;
            }
            return Result;
        }
        if (Ship != TruceShip && Ship->TruceShip != this) {
            if (CurrentStanding == aGalaxyStruct::ssCoalitionMilitary && pas::in_range(Ship->CurrentStanding, aGalaxyStruct::ssPirateActive, aGalaxyStruct::ssPirateMilitary) || CurrentStanding == aGalaxyStruct::ssPirateMilitary && pas::in_range(Ship->CurrentStanding, aGalaxyStruct::ssCoalitionMilitary, aGalaxyStruct::ssCoalitionActive)) {
                return Result;
            }
        }
        if (CurrentStanding != aGalaxyStruct::ssDominator && Ship->TypeId == aGalaxyStruct::stKling && Ship->CurrentStanding == aGalaxyStruct::ssNeutral) {
            return 100;
        } else if (pas::class_cast_if<aRanger::TRanger*>(Ship) != nullptr) {
            return RelationToRanger(Ship);
            ...
            return RelationToShip(pas::checked_cast<aTranclucator::TTranclucator*>(Ship)->OwnerShip);
        } else {
            return RelationToNonRanger(Ship);
        }
    }
